...
function ItineraryView({route,onSeeReports,onContribute,myItin,onSaveMyItin,crewsForRoute,onShareItinToCrew,notify}){
...
const mtn=(function(){const _s=MOUNTAINS.find(m=>m.id===route.mountainId);if(_s&&_s.name)return _s;const _d=route._dbArea;if(_d&&_d.name)return _d;return Object.assign({},_d||{},{name:"this area"});})(),dc=CAT[catOf(route)]||{color:C.textSub,bg:C.surface,icon:"",label:catOf(route)};const partners=useMemo(()=>ALL_CLIMBERS.filter(c=>c.objectiveIds.includes(route.id)),[route.id]);const avgS=useMemo(function(){var rated=activity.filter(function(a){return a&&a.stars>0;});return rated.length?rated.reduce(function(s,a){return s+a.stars;},0)/rated.length:(route.stars||0);},[activity,route.stars]);const rkw=useMemo(()=>routeKw(route),[route.id,route.activity,route.hazards]);const rkFlagged=useMemo(()=>rkw.hazards.filter(h=>!h.official&&h.count>=2),[rkw.hazards]);const cragSibs=useMemo(()=>USE_DB&&dbSibs?dbSibs.map(dbRouteToCamel):ROUTES.filter(x=>x.mountainId===route.mountainId),[route.mountainId,dbSibs,USE_DB]);const routePhotos=useMemo(()=>(route.activity||[]).flatMap(a=>(a.photos||[]).map(u=>({url:u,alt:(a.photoAlts&&a.photoAlts[u])||null,by:a.user,avatar:a.avatar,date:a.date,ascent:a,cat:(a.condTags||[]).some(function(t){return isHazardTag(t);})?"Hazards":"Conditions"}))).concat((route.photos||[]).map(u=>({url:u,cat:"Conditions"}))),[route.id,route.activity,route.photos]);
