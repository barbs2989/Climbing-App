...
/* A STUB IS MEASURED IN METRES, NOT IN TENTHS OF A KILOMETRE. _gapDist above is for the kilometre
   scale a partial track is short by; feeding it seven metres prints "0 km", which undercuts the one
   fact the sentence carries. uImp() is the real boolean here too — uDistMi is a FORMATTER and is
   always truthy, the #641 trap this file records. */
function _shortDist(m){return uImp()?(Math.round(m*3.28084)+" ft"):(Math.round(m)+" m");}
/* A distance in the climber's units, switching to mi/km past a kilometre so "1,998 m" reads as "1.2 mi". */
function _baseDist(m){return m>=1000?(uImp()?((m/1609.34).toFixed(1)+" mi"):((m/1000).toFixed(1)+" km")):_shortDist(m);}
/* A request that never reached the server — the one failure worth keeping on the phone for later. */
function _isNoSignal(e){if(typeof navigator!=="undefined"&&navigator.onLine===false)return true;var m=String((e&&e.message)||e||"");return /failed to fetch|network|load failed|fetch failed/i.test(m);}
/* "I'm at the base" (0188). The phone judges whether its own GPS fix is good, steady and still; the
   DATABASE judges whether that point is at this climb, because a rule the browser applies can be
   skipped by a crafted request. So the screen reports the server's answer, never its own guess.
   Other climbers see each checked-in spot and how many climbers stood there — never who, never when. */
function BaseCheckinPanel({route,who}){
  const q=useRouteBaseCheckins(USE_DB?route.id:null);
  const [st,setSt]=useState({phase:"idle"});const [prog,setProg]=useState(null);
  const area=route._dbArea||{};
  const hasAnchor=(area.lat!=null&&area.lng!=null&&Number.isFinite(+area.lat)&&Number.isFinite(+area.lng))||(route.waypoints||[]).some(function(w){var t=String((w&&w.type)||"").toLowerCase();return wpPlaced(w)&&/^(base|start|route[ _-]?start|route base)$/.test(t);});
  /* Check-ins taken with no signal wait on this phone. Send them whenever this panel mounts with a
     connection or the browser comes back online; the server still judges each one. */
  const refetch=q.refetch;
  const flush=useCallback(function(){if(!who||!USE_DB)return;var list=pendingBaseCheckins(who);if(!list.length)return;var keep=[];var chain=Promise.resolve();list.forEach(function(p){chain=chain.then(function(){return checkInAtRouteBase(p.routeId,p.fix).then(function(res){if(p.routeId===route.id)setSt(res.ok?{phase:"done",late:true}:{phase:"refused",res:res});}).catch(function(e){if(_isNoSignal(e))keep.push(p);});});});chain.then(function(){savePendingBaseCheckins(who,keep);if(refetch)refetch();});},[who,route.id,refetch]);
  useEffect(function(){flush();if(typeof window==="undefined")return;window.addEventListener("online",flush);return function(){window.removeEventListener("online",flush);};},[flush]);
  if(!USE_DB)return null;
  const pts=q.data||[];const clusters=clusterBasePoints(pts);const mineAny=pts.some(function(p){return p.mine===true;});
  if(!hasAnchor&&!clusters.length)return null;
  const go=function(){if(!who||st.phase==="locating"||st.phase==="sending")return;setSt({phase:"locating"});setProg(null);
    acquireBaseFix(typeof navigator!=="undefined"?navigator.geolocation:null,{onProgress:setProg}).then(function(fix){
      if(!fix.ok){setSt({phase:"refused",res:fix});return;}
      setSt({phase:"sending"});
      checkInAtRouteBase(route.id,fix).then(function(res){setSt(res.ok?{phase:"done"}:{phase:"refused",res:res});if(res.ok&&refetch)refetch();}).catch(function(e){
        if(_isNoSignal(e)){var list=pendingBaseCheckins(who).filter(function(p){return p.routeId!==route.id;}).concat([{routeId:route.id,fix:fix}]);setSt(savePendingBaseCheckins(who,list)?{phase:"queued"}:{phase:"error"});}
        else setSt({phase:"error"});});});};
  const withdraw=function(){if(!who)return;setSt({phase:"withdrawing"});withdrawBaseCheckin(who,route.id).then(function(){setSt({phase:"idle"});if(refetch)refetch();}).catch(function(){setSt({phase:"error",withdraw:true});});};
  const busy=st.phase==="locating"||st.phase==="sending"||st.phase==="withdrawing";
  const top=clusters[0];
  return <div style={{background:C.card,borderRadius:12,padding:"12px 14px",border:`1px solid ${C.border}`,marginBottom:12}}>
    <CardHead style={{marginBottom:8}}>BASE OF THE CLIMB</CardHead>
    {q.isError?<div style={{fontSize:12.5,color:C.textSub,marginBottom:8}}>Couldn’t load check-ins at the base — try again in a moment.</div>
    :top?<div style={{marginBottom:9}}>
      <div style={{fontSize:13,color:C.text,fontWeight:600,marginBottom:3}}>{top.lat.toFixed(5)+", "+top.lng.toFixed(5)}</div>
      <div style={{fontSize:12,color:C.textSub,marginBottom:5}}>{"Checked in on-site by "+top.n+" climber"+(top.n!==1?"s":"")+(top.mine?", including you":"")+(clusters.length>1?" · "+(clusters.length-1)+" other spot"+(clusters.length-1!==1?"s":"")+" reported":"")}</div>
      <a href={"https://www.google.com/maps/search/?api=1&query="+top.lat.toFixed(6)+","+top.lng.toFixed(6)} target="_blank" rel="noopener noreferrer" style={{fontSize:12.5,color:C.blue,fontWeight:600,textDecoration:"none"}}>Open the base in maps ›</a>
    </div>
    :q.isPending?<div style={{fontSize:12.5,color:C.textMuted,marginBottom:8}}>Loading check-ins…</div>
    :<div style={{fontSize:12.5,color:C.textSub,marginBottom:8}}>Nobody has checked in at the base yet.</div>}
    {st.phase==="done"?<div role="status" style={{fontSize:12.5,color:C.green,fontWeight:600,marginBottom:8}}>{st.late?"✓ Your saved check-in was sent and accepted — thanks for marking the start.":"✓ You’re checked in at the base — thanks for marking the start for the next party."}</div>
    :st.phase==="refused"?<div role="status" style={{fontSize:12.5,color:C.amber,lineHeight:1.45,marginBottom:8}}>{baseCheckinMessage(st.res,_baseDist)}</div>
    :st.phase==="queued"?<div role="status" style={{fontSize:12.5,color:C.textSub,lineHeight:1.45,marginBottom:8}}>No signal here — your GPS reading is saved on this phone and will be sent, and checked, the next time you open this climb with a connection.</div>
    :st.phase==="error"?<div role="status" style={{fontSize:12.5,color:C.red,marginBottom:8}}>{st.withdraw?"Couldn’t remove your check-in — try again.":"Couldn’t check you in — try again."}</div>
    :st.phase==="locating"?<div role="status" style={{fontSize:12.5,color:C.textSub,marginBottom:8}}>{"Getting a steady GPS fix — stand still"+(prog?" ("+prog.good+" good reading"+(prog.good!==1?"s":"")+")":"")+"…"}</div>
    :st.phase==="sending"?<div role="status" style={{fontSize:12.5,color:C.textSub,marginBottom:8}}>Checking your position against this climb…</div>:null}
    {who?<div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
      <button onClick={go} disabled={busy} aria-busy={busy?"true":undefined} style={{padding:"8px 12px",background:busy?C.border:C.blueSolid,color:"#fff",border:"none",borderRadius:9,fontSize:12.5,fontWeight:700,cursor:busy?"default":"pointer"}}>{mineAny?"I’m at the base — check in again":"I’m at the base — check in"}</button>
      {mineAny?<button onClick={withdraw} disabled={busy} style={{padding:"8px 12px",background:C.surface,color:C.textSub,border:`1px solid ${C.border}`,borderRadius:9,fontSize:12.5,fontWeight:600,cursor:busy?"default":"pointer"}}>Remove my check-in</button>:null}
    </div>:<div style={{fontSize:12,color:C.textMuted}}>Sign in to check in when you’re standing at the start of the climb.</div>}
    {who?<div style={{fontSize:11.5,color:C.textMuted,lineHeight:1.45,marginTop:8}}>Uses your phone’s GPS, and only works at the start of the climb: your position is checked against where this climb is on file. Other climbers see the spot and how many climbers stood there — never who, or when.</div>:null}
  </div>;
}
function gearKey(s){
...
function RopeworkBox({route,onEdit}){
  ...
  return <><RouteRackBox route={route} activity={activity} onEditRack={onEditRack} rack={rack} onSeeReports={onSeeReports} rackGeneric={rackGeneric}/><RopeworkBox route={route} onEdit={onEditRopeNote}/><RouteGearEssentialsBox route={route} essentials={essentials} onEdit={onEditEssentials}/></>;
}
...
function PitchConsensus({p,onSuggestFix}){
  ...
  return <div style={{background:C.surface,borderRadius:8,padding:"10px 11px",marginBottom:9,border:"1px solid "+C.border}}>
    <CardHead tone={C.green} style={{marginBottom:8}}>Pitch gear</CardHead>
...
/* THE STATE IS `aria-expanded`, NOT WORDS IN THE NAME. These rows are disclosures — tapping one
   ...
   (it was first written down on the route-tag chips, since removed): `aria-expanded` rather than
   `aria-pressed`, because this is a disclosure, not a toggle that changes anything. The stage row's old ▸ carried it and lost it when the whole row became
          ...
          <div style={{flex:1,minWidth:0,background:C.surface,border:"1px solid "+C.border,borderLeft:"3px solid "+accent,borderRadius:10}}>
            <div {...(hasMore?clickable(function(){setOpen(isOpen?null:r._i);}):{})} aria-expanded={hasMore?isOpen:undefined} aria-label={hasMore?(isPitch?pitchRowName(r):stageRowName(r)):undefined} style={{padding:"9px 11px",cursor:hasMore?"pointer":"default"}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,flexWrap:"wrap",marginBottom:r._note?5:0}}>
                <span style={{fontSize:13,fontWeight:700,color:isPitch?C.amber:C.text,minWidth:0,wordBreak:"break-word",overflowWrap:"anywhere"}}>{isPitch?(r.grade||("Pitch "+r._badge)):r._label}{r.crux?<span style={{...SUB_LABEL,color:C.red,marginLeft:6,whiteSpace:"nowrap"}}>CRUX</span>:null}</span>
    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,marginBottom:8}}>
      <div style={{display:"flex",gap:8,minWidth:0,alignItems:"flex-start"}}>
        ...
        <div style={{minWidth:0}}>
          <div style={{...SUB_LABEL,marginBottom:2}}>TRAILHEAD</div>
...
function RappelTable({route,onEdit}){
  ...
  return <div style={{background:C.card,borderRadius:12,padding:"12px 14px",border:`1px solid ${C.border}`,marginTop:12}}>
    <div style={SZ4}><div style={{display:"flex",alignItems:"center",gap:10,minWidth:0}}><div style={{display:"flex",alignItems:"center",gap:7,minWidth:0}}><CardHead tone={C.red}>{rappelHeaderLabel(route)}</CardHead><ProvChip prov={sectionProvenance(route,"rappels")}/></div>{total>0?<div style={{fontSize:12,color:C.textMuted}}>{uLen(total)+(partial?(" across "+known.length+" of "+raps.length):" total")}</div>:null}</div>{onEdit?<EditIconButton onClick={onEdit} title="Edit rappel information"/>:null}</div>
...
export function ClimbingRouteTable({route,onEdit}){
  ...
  return <div style={{background:C.card,borderRadius:12,padding:"12px 14px",border:`1px solid ${C.border}`,marginTop:12}}>
    <div style={SZ4}><div style={{display:"flex",alignItems:"center",gap:10,minWidth:0}}><CardHead>{"CLIMBING ROUTE · "+list.length+" section"+(list.length!==1?"s":"")}</CardHead></div>{onEdit?<EditIconButton onClick={onEdit} title="Edit the climbing route"/>:null}</div>
...
function OvernightPermit({route}){
  ...
  return <div style={{background:C.surface,border:"1px solid "+C.border,borderRadius:10,padding:"9px 11px",marginBottom:10}}>
    <div style={{...SUB_LABEL,marginBottom:3}}>{"OVERNIGHT PERMIT"}</div>
...
function Calculator({route,activity,fit:fitProp,setFit:setFitProp,calc,onCalc}){
  ...
  /* `multiDay` WAS GATED ON `route.campOptions`, WHICH IS SEED-ONLY, SO THIS DISCLAIMER RENDERED
     FOR NOBODY IN PRODUCTION — on exactly the routes whose single-push estimate is least
     believable. The shared derivation and the measurement behind it are in lib/outing.js; note in
     particular that porting the old gate to `bivy` would have LOOKED like a fix and changed
     nothing, because 0 of the 796 WA routes carrying a bivy entry has a starred camp.
     THE COPY MOVED WITH THE GATE, and it had to: it used to say "use the Plan tab for a realistic
     day-by-day plan" while sitting ON the Plan tab, with <ItineraryView/>'s "Trip plan" rendered
     directly above it — a pointer past the very thing it was pointing at, invisible for as long
     as the box rendered for nobody. Ask what a newly-reachable surface SAYS, not only that it now
     reaches somebody. */
  const late=retH>18.5,sumLate=!publishedIsWholeDay&&sumH>13,multiDay=isMultiDayOuting(route);
  ...
  return <div style={{background:C.card,borderRadius:12,padding:"12px 14px",border:`1px solid ${C.border}`}}>
    ...
    {(function(){var _lt=loggedTimeStats(activity);if(!_lt)return null;var _spread=_lt.minMin!==_lt.maxMin?(fmtDurMin(_lt.minMin)+"–"+fmtDurMin(_lt.maxMin)):null;var _legLbl={approachMin:"approach",climbMin:"climb",descentMin:"descent"};var _legs=Object.keys(_lt.legs).map(function(k){return _legLbl[k]+" "+fmtDurMin(_lt.legs[k].medianMin);});
      return <div style={{background:C.greenBg,borderRadius:10,padding:"10px 12px",margin:"4px 0 12px",border:"1px solid "+C.greenDim}}>
        <div style={{...SUB_LABEL,marginBottom:4}}>{"WHAT PARTIES ACTUALLY TOOK"}</div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9,marginBottom:12}}>
      ...
      <div><div style={{...SUB_LABEL,marginBottom:3}}>PARTY SIZE</div><input aria-label="Party size" type="number" value={party} onChange={e=>setParty(Number(e.target.value))} style={{width:"100%",padding:"7px 8px",borderRadius:8,border:`1px solid ${C.border}`,background:C.surface,color:C.text,fontSize:13,boxSizing:"border-box",outline:"none"}}/></div>
...
/* ROAD CONDITIONS — what climbers who drove in said about the road, newest first. The catalog's
   own road block (GETTING THERE, on Plan) is researched once and goes stale with the season; this
   is the part that changes week to week, so it is read off reports, never off the catalog. A
   report counts when it answered any of ROAD TO THE TRAILHEAD in the log form, including the old
   open/closed chips, which are the same strings (see ROAD_STATUS in core). */
const ROAD_TONE=function(t){return t==="open"?[C.green,C.greenBg]:t==="gated"?[C.amber,C.amberBg]:t==="closed"?[C.red,C.redBg]:[C.textSub,C.surface];};
function roadReportsOf(activity){return (activity||[]).map(function(a){var r=roadReportOf(a);return r?Object.assign({},r,{user:a.user,date:a.date}):null;}).filter(Boolean).sort(function(a,b){return String(b.date||"").localeCompare(String(a.date||""));});}
function RoadReports({activity,reportsUnavailable,onReport}){
  const [all,setAll]=useState(false);
  const reps=useMemo(function(){return roadReportsOf(activity);},[activity]);
  const lastStatus=reps.find(function(r){return r.status;});
  const shown=all?reps:reps.slice(0,3);
  return <div style={{background:C.card,borderRadius:12,padding:"13px 15px",marginBottom:12,border:"1px solid "+C.border}}>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,marginBottom:4}}><CardHead tone={C.teal}>ROAD CONDITIONS</CardHead>{onReport?<button onClick={onReport} style={{flexShrink:0,fontSize:11.5,fontWeight:800,color:C.green,background:C.greenBg,border:"1px solid "+C.greenDim,borderRadius:8,padding:"5px 11px",cursor:"pointer"}}>Report the road</button>:null}</div>
    <div style={{fontSize:11.5,color:C.textMuted,marginBottom:reps.length?10:0,lineHeight:1.5}}>{reps.length?"From climbers who drove in — newest first.":reportsUnavailable?"Couldn’t load road reports — try again in a moment.":"No one has reported the road yet. Driving in? Say whether it was open and what it took — the next party checks this before they leave home."}</div>
    {lastStatus?(function(){var tc=ROAD_TONE(lastStatus.tone);return <div style={{marginBottom:10}}><span style={{display:"inline-block",fontSize:13,fontWeight:800,color:tc[0],background:tc[1],borderRadius:8,padding:"4px 10px"}}>{lastStatus.status}</span><span style={{fontSize:12,color:C.textMuted,marginLeft:8}}>{"reported "+ago(lastStatus.date)}</span>{!isRecent(lastStatus.date)?<div style={{fontSize:11.5,color:C.amber,marginTop:6,lineHeight:1.5}}>{"That is the newest report, and it is more than "+RECENT_DAYS+" days old. Roads change with the season — check with the land manager before you drive."}</div>:null}</div>;})():null}
    {shown.map(function(r,i){var bits=[r.status,r.vehicle].concat(r.issues).filter(Boolean);return <div key={i} style={{borderTop:"1px solid "+C.border,padding:"8px 0"}}><div style={{fontSize:11.5,color:C.textMuted,marginBottom:3}}>{(r.date?shortDate(r.date)+" · ":"")+(r.user||"A climber")}</div>{bits.length?<div style={{fontSize:12.5,color:C.textSub,lineHeight:1.5}}>{bits.join(" · ")}</div>:null}{r.note?<div style={{fontSize:12.5,color:C.text,lineHeight:1.5,marginTop:bits.length?3:0}}>{r.note}</div>:null}</div>;})}
    {reps.length>3?<button onClick={function(){setAll(!all);}} aria-expanded={all} style={{marginTop:4,background:"transparent",border:"none",color:C.blue,fontSize:12,fontWeight:700,cursor:"pointer",padding:"4px 0"}}>{all?"Show fewer":"Show all "+reps.length+" road reports"}</button>:null}
  </div>;
}
export function ConsensusPanel({route,activity,hzVotes,onVote,reportsUnavailable}){
  /* Road tags are left out of THIS panel's tallies: ROAD CONDITIONS sits directly beneath it and
     says the same thing as sentences, so on a route with few reports TOP CONDITIONS REPORTED filled
     up with "Road/trailhead closed · Road: washout" and pushed the route's own conditions off it.
     Overview's one-line summary still counts them — the road has no section of its own there. */
  const activityAll=useMemo(function(){return (activity||route.activity||[]).map(function(a){return (a&&(a.condTags||[]).some(isRoadTag))?Object.assign({},a,{condTags:a.condTags.filter(function(t){return !isRoadTag(t);})}):a;});},[activity,route.activity]);
  ...
  return <div style={{background:C.card,borderRadius:12,padding:"12px 14px",border:`1px solid ${C.border}`,marginBottom:14}}>{/* A PARTIAL CONSENSUS IS NOT A CONSENSUS. `activity` is route.activity + myReports + dbReports, so a failed reports read silently drops the DB half and buildConsensus runs on what is left — the panel then presents a DERIVED SAFETY JUDGEMENT as if it had seen everything filed. `reportsUnavailable` was already passed in and was consulted ONLY on the empty branch, so exactly this case said nothing. Found by walking the Conditions sub-tab in check:outage for the first time: under a blanket outage the screen CHANGED (3,388 -> 3,384 chars) and acknowledged nothing, which is rule 1. */}{reportsUnavailable?<div style={{fontSize:11.5,color:C.amber,background:C.amberBg,border:"1px solid "+C.amber,borderRadius:8,padding:"7px 9px",marginBottom:10,lineHeight:1.45}}>{"Some reports couldn’t load, so this is based on the ones that did — not on everything filed for this route."}</div>:null}{_fresh?<div style={{display:"flex",alignItems:"center",gap:8,marginBottom:11,fontSize:11.5,fontWeight:600}}><span style={{width:6,height:6,borderRadius:"50%",background:C.green,flexShrink:0,animation:"cm-pulse 2s infinite"}}/><span style={{color:C.textMuted}}>{_fresh}</span><span style={{color:_trend.c,fontWeight:700}}><span className="cm-pop" style={{display:"inline-block"}}>{_trend.a}</span>{" "+_trend.l}</span></div>:null}
    <div style={SZ3}><div style={{fontSize:14,fontWeight:700}}>Community Consensus</div><Pill label={`${c.confidence} confidence · ${c.reportCount} reports`} color={confCol} bg={`${confCol}22`} sm/></div>{c.hazards&&c.hazards.length?<div style={{background:C.redBg,border:`1px solid ${C.red}55`,borderRadius:10,padding:"10px 12px",marginBottom:12}}><div style={{...SUB_LABEL,marginBottom:6,color:C.red}}>Recent hazard reports</div>{c.hazards.map((h,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:7,marginBottom:i<c.hazards.length-1?6:0}}><Av src={h.avatar} size={20}/><div style={{flex:1,minWidth:0}}><div style={{fontSize:12,color:C.text,fontWeight:600}}>{h.tags.join(" · ")}</div><div style={{fontSize:12,color:C.textMuted}}>{h.user+" · "+h.date}</div></div>{h.trust!=null?<span style={{fontSize:12,color:trustTier(h.trust).color,fontWeight:700,flexShrink:0}}>{""+h.trust}</span>:null}</div>)}</div>:null}{c.lastDate?<div style={{fontSize:12,color:C.textMuted,marginBottom:10}}>Most recent report: <span style={{color:C.textMuted}}>{ago(c.lastDate)+" · "+c.lastDate}</span>{c.verifiedCount?" · "+c.verifiedCount+" of "+c.reportCount+" reports from verified climbers":""}</div>:null}
    {(kh.length||kc.length||k.conflicts.length)?<div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:"11px 12px",marginBottom:13}}><div style={{display:"flex",alignItems:"center",gap:7,marginBottom:9,flexWrap:"wrap"}}><span style={{fontSize:13,fontWeight:700,color:C.text,textTransform:"uppercase",borderLeft:"3px solid "+C.blue,paddingLeft:9,letterSpacing:0.5,flex:1}}>What trip reports consistently mention</span><span style={{fontSize:12,fontWeight:700,color:k.confidence==="High"?C.green:k.confidence==="Medium"?C.amber:C.red,background:(k.confidence==="High"?C.green:k.confidence==="Medium"?C.amber:C.redSolid)+"22",borderRadius:6,padding:"2px 7px"}}>{k.confidence+" confidence in these tags"}</span></div>{k.conflicts.length?<div style={{background:C.amberBg,border:`1px solid ${C.amber}55`,borderRadius:8,padding:"8px 10px",marginBottom:10,fontSize:11.5,color:C.amber,lineHeight:1.45}}>{k.conflicts[0]}</div>:null}{kh.length?<div style={{marginBottom:kc.length?11:0}}><div style={{...SUB_LABEL,marginBottom:7}}>Hazards climbers report</div>{kh.map(h=><CRow key={h.label} item={h} kind="h" routeId={route.id} hzVotes={hzVotes} onVote={onVote} openK={openK} setOpenK={setOpenK}/>)}</div>:null}{kc.length?<div><div style={{...SUB_LABEL,marginBottom:7}}>Conditions climbers report</div>{kc.slice(0,6).map(co=><CRow key={co.label} item={co} kind="c" routeId={route.id} hzVotes={hzVotes} onVote={onVote} openK={openK} setOpenK={setOpenK}/>)}</div>:null}<div style={{fontSize:12,color:C.textMuted,lineHeight:1.5,marginTop:9,paddingTop:9,borderTop:`1px solid ${C.borderLight}`}}>From {k.total} report{k.total!==1?"s":""}; a signal needs 2+ to show. Tap a row for its sources. <span style={{color:C.green,fontWeight:700}}>✓</span> official · <span style={{color:C.amber,fontWeight:700}}>⚑</span> community-flagged · <span style={{color:C.purple,fontWeight:700}}>✦</span> verified across seasons · ● within last year.</div></div>:null}
    {c.conditions&&Object.keys(c.conditions).length?<div style={{marginBottom:14}}>{(function(){/* The consensus falls back to reports of ANY age when none is recent, which used to sit under "CONDITIONS NOW" looking current. Say which it is. */var ks=Object.keys(c.conditions),allOld=ks.every(function(k){return c.conditions[k].stale;});var newest=ks.map(function(k){return c.conditions[k].mostRecent;}).filter(Boolean).sort().slice(-1)[0];return <><div style={{fontSize:12,color:allOld?C.amber:C.blue,fontWeight:800,letterSpacing:0.4,marginBottom:6}}>{allOld?"LAST REPORTED CONDITIONS":"CONDITIONS NOW"}</div>{allOld?<div style={{fontSize:11.5,color:C.amber,lineHeight:1.45,marginBottom:8}}>{"No one has reported conditions in the last "+RECENT_DAYS+" days"+(newest?" — the newest of these is from "+ago(newest):"")+". Treat them as history, not as today."}</div>:null}</>;})()}{VOLATILE_DISC.indexOf(catOf(route))>=0?<div style={{display:"flex",alignItems:"center",gap:6,background:C.amberBg,border:"1px solid "+C.amber+"55",borderRadius:8,padding:"6px 9px",marginBottom:8,fontSize:11.5,color:C.amber,lineHeight:1.4}}><span>⚠</span><span>This route's conditions can change fast — verify before you go.</span></div>:null}<div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{[["carToCar","⏱ Car-to-car"],["tempF","🌡 Temp"],["snow","❄ Snow"],["snowDepth","📏 Postholing"],["freezingFt","🧊 Freezing lvl"],["freezing","🧊 Freezing lvl"],["water","💧 Water"],["bugs","🦟 Bugs"],["trail","🥾 Trail"],["seepage","💦 Seepage"],["mud","🟤 Mud"],["protection","🔩 Protection"],["crowds","👥 Crowds"]].map(function(p){var cd=c.conditions[p[0]];/* a legacy prose freezing level only speaks when no NUMBER has been reported */if(p[0]==="freezing"&&c.conditions.freezingFt)cd=null;return cd?<div key={p[0]} style={{background:C.surface,border:"1px solid "+C.border,borderRadius:8,padding:"5px 9px",fontSize:12}}><span style={{color:C.textMuted}}>{p[1]+" "}</span><span style={{color:C.text,fontWeight:700}}>{p[0]==="tempF"?uTempU(cd.value):p[0]==="freezingFt"?uElev(cd.value):cd.value}</span>{cd.n?<span style={{color:cd.stale?C.amber:C.textMuted,fontSize:11}}>{" · "+cd.n+(cd.mostRecent?" · "+ago(cd.mostRecent):"")}</span>:null}</div>:null;})}</div></div>:null}<div style={{display:"flex",gap:12,alignItems:"center",marginBottom:14}}>
      ...
      <div style={{flex:1}}><div style={{...SUB_LABEL,marginBottom:5}}>TOP CONDITIONS REPORTED</div>{c.topTags.map(t=><div key={t.tag} style={{marginBottom:9}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",gap:8,marginBottom:3}}><span style={{fontSize:12,color:C.text}}>{t.tag}</span><span style={{fontSize:12,fontWeight:700,color:C.teal,flexShrink:0}}>{t.pct}%</span></div><Bar val={t.pct} color={C.teal} h={6}/></div>)}</div>
    </div>
    <div style={{...SUB_LABEL,marginBottom:6}}>SEASONAL HEATMAP (user reports)</div>
    ...
    {c.faCredits&&c.faCredits.length?<div style={{marginTop:14,background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:"10px 12px"}}><CardHead tone={C.amber} style={{marginBottom:6}}>First Ascent Credit</CardHead><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{c.faCredits.map(name=><span key={name} style={{fontSize:12,color:C.amber,background:C.amberBg,border:`1px solid ${C.amber}55`,borderRadius:6,padding:"4px 9px",fontWeight:600}}>{name}</span>)}</div></div>:null}
    {c.isDeveloped?<div style={{marginTop:12,background:C.greenBg,border:`1px solid ${C.green}55`,borderRadius:10,padding:"10px 12px"}}><div style={{...SUB_LABEL,color:C.green}}>Route Development</div><div style={{fontSize:12.5,color:C.textSub,marginTop:4}}>This route has been bolted or developed by climbers in the community.</div></div>:null}
    {c.photos&&c.photos.length?<div style={{marginTop:14}}><CardHead style={{marginBottom:8}}>Recent Photos</CardHead><div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6}}>{c.photos.map((p,i)=><div key={i} style={{position:"relative",paddingBottom:"100%",borderRadius:8,overflow:"hidden",background:C.card,cursor:"pointer",border:`1px solid ${C.border}`}}><img loading="lazy" decoding="async" src={p.url} alt={"Recent condition photo"+(p.user?" by "+p.user:"")} style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover"}} onError={()=>{}} />{p.user?<div style={{position:"absolute",left:0,right:0,bottom:0,padding:"8px 6px 4px",background:"linear-gradient(transparent,rgba(0,0,0,0.72))",fontSize:11,color:"#fff",fontWeight:600,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{p.user.split(" ")[0]}</div>:null}</div>)}</div></div>:null}
  </div>;
}
...
// The per-discipline safety boxes ("Spotting & landing", "Clipping & lowering", "Gear & anchors"…
// and "Watch out for on this type of climb") were REMOVED: canned lines chosen by discipline
// alone, the same four sentences on every route of a kind, which is advice about climbing in
// general rather than about THIS climb. The Safety tab still carries the route's own hazards,
// the forecast links and the nearby-fire panel. What stays here is the one control that acts
// on this route: reporting a bad bolt or anchor on a sport climb.
...
function SafetyMatrix({route,activity,mountain,hzVotes,onVote,onOpenContribute,onOpenStartLoc,onOpenHazards,onOpenTurnaround}){
  const [openK,setOpenK]=useState(null);
  const cat=catOf(route);/* The avalanche panel used to appear for every ice/mixed/alpine/mountaineering route with an
...
const avyRelevant=["ice","mixed","alpine","mountaineering"].includes(cat)&&routeTerrain(route).avalanche!=="no";const realBail=(route.waypoints||[]).filter(w=>wpIs(w,"Bailout"));const bail=realBail.length?realBail:(route.waypoints||[]).filter(w=>["Junction","Campsite","Trailhead"].includes(w.type));const bailIsReal=realBail.length>0;/* `activity` is the merged list (seed + yours + every climber’s DB report); route.activity alone is only your own logs on a DB route, so no other climber could ever flag a hazard here. */const _acts=activity||route.activity||[];const kcon=routeKw(Object.assign({},route,{activity:_acts}));const flaggedHaz=kcon.hazards.filter(h=>!h.official&&(h.count>=2||(h.reports||[]).some(rp=>isRecent(rp.date))));const confHaz=hstr=>kcon.hazards.find(h=>h.official&&h.count>=2&&HAZ_KW[h.label].some(kk=>hstr.toLowerCase().indexOf(kk)>=0));const envRelevant=["alpine","mountaineering"].includes(cat);const _objHaz=Array.isArray(route.objHaz)?route.objHaz:(route.objHaz?[route.objHaz]:[]);/* The box merges hazards, objHaz and watchOut. It used to de-duplicate the first two by exact
  ...
  return <div>
    ...
    {(()=>{const sh=enrichRoute(route).seasonalHazards||{};const wx=sh.weather||{};
      ...
      if(!rows.length)return null;
      return <div style={{background:C.card,borderRadius:12,padding:"13px 15px",marginBottom:12,border:`1px solid ${C.border}`}}>
        <CardHead style={{marginBottom:8}}>SEASONAL HAZARDS</CardHead>
...
function PhotoRow({items,w,h}){
  ...
  return <div><div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:6}}>{items.map((it,i)=>{const url=typeof it==="string"?it:(it.u||it.url);const a=(it&&it.a)||null;const cap=(it&&it.caption)||null;return <div key={i} style={{flexShrink:0,width:W}}><div {...clickable(()=>setView(url))} style={{position:"relative",cursor:"pointer"}}><img loading="lazy" decoding="async" src={url} alt={cap||("Photo "+(i+1))} onError={onImgErr(FALLBACK_COVER)} style={{width:W,height:H,objectFit:"cover",borderRadius:11,border:`1px solid ${C.border}`}}/>{a&&a.user?<div style={{position:"absolute",left:6,bottom:6,display:"flex",alignItems:"center",gap:4,background:"rgba(0,0,0,0.55)",borderRadius:20,padding:"2px 8px 2px 2px"}}><Av src={a.avatar} size={18}/><span style={{fontSize:12,color:"#fff",fontWeight:600}}>{(a.user||"").split(" ")[0]}</span></div>:null}</div>{cap?<div style={{fontSize:12,color:C.textSub,marginTop:4,lineHeight:1.4}}>{cap}</div>:null}</div>;})}</div>{view?createPortal(<div onClick={()=>setView(null)} role="dialog" aria-label="Photo viewer" aria-modal="true" style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",zIndex:99999,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}><img loading="lazy" decoding="async" src={view} alt="Enlarged photo" onError={onImgErr(FALLBACK_COVER)} style={{maxWidth:"100%",maxHeight:"100%",borderRadius:12,objectFit:"contain"}}/><button onClick={()=>setView(null)}  style={Object.assign({},POP_CLOSE_MEDIA,{position:"absolute",top:16,right:16})} aria-label="Close">✕</button></div>,document.body):null}</div>;
}
...
function ItineraryView({route,activity,onSeeReports,onContribute,myItin,onSaveMyItin,crewsForRoute,onShareItinToCrew,notify}){
  const it=route.itinerary;const _acts=activity||route.activity||[];const k=routeKw(Object.assign({},route,{activity:_acts}));const repN=_acts.length;
    ...
    {onSaveMyItin?<div style={{marginTop:16,paddingTop:14,borderTop:"1px solid "+C.borderLight}}>
      ...
      {!showBuilder?<div>
        ...
        <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
          ...
          {(myItin&&myItin.days&&myItin.days.length)?<button onClick={function(){shareOrCopy(route.name+" — plan",itinShareText(myItin,route),notify);}} aria-label="Share my plan" style={{padding:"8px 12px",background:C.card,color:C.blue,border:"1px solid "+C.border,borderRadius:9,fontSize:12.5,fontWeight:600,cursor:"pointer"}}>{"Share…"}</button>:null}
          {(myItin&&myItin.days&&myItin.days.length)?<button onClick={function(){printSheet(route.name+" — plan",itinSheetHtml(myItin,route),notify);}} aria-label="Save my plan as a PDF" style={{padding:"8px 12px",background:C.card,color:C.blue,border:"1px solid "+C.border,borderRadius:9,fontSize:12.5,fontWeight:600,cursor:"pointer"}}>{"Save as PDF"}</button>:null}
          {(myItin&&myItin.days&&myItin.days.length&&crewsForRoute&&crewsForRoute.length)?(pickCrew?<div style={{display:"flex",gap:5,flexWrap:"wrap",alignItems:"center"}}>{crewsForRoute.map(function(c,ci){return <button key={c.id} onClick={function(){onShareItinToCrew(c.id,myItin);setPickCrew(false);}} style={{padding:"6px 10px",background:C.blueBg,color:C.blue,border:"1px solid "+C.blueDim,borderRadius:8,fontSize:11.5,fontWeight:700,cursor:"pointer"}}>{/* Crews have no name column, so every button used to read "→ Crew". The agreed day tells two crews on one route apart. */"→ "+(c.date?"Crew · "+shortDate(c.date):crewsForRoute.length>1?"Crew "+(ci+1):"Your crew")}</button>;})}</div>:<button onClick={function(){setPickCrew(true);}} style={{padding:"8px 12px",background:C.card,color:C.green,border:"1px solid "+C.green+"55",borderRadius:9,fontSize:12.5,fontWeight:600,cursor:"pointer"}}>Share with crew</button>):null}
        </div>
      </div>:<div>
        {avail.length?<div style={{marginBottom:11}}>
          <div style={{...SUB_LABEL,marginBottom:6}}>START FROM A SUGGESTED PLAN</div>
...
export function TopoSection({route}){
  const areaId=route.mountainId;const _topoSess=useSession();const _topoUid=_topoSess&&_topoSess.user&&_topoSess.user.id;
  ...
  const photos=USE_DB
    ?(dbTopos.data||[]).map(function(t){return {id:t.id,url:topoPhotoUrl(t.storage_path),db:true,storagePath:t.storage_path,alt:t.alt||null,mine:!!(_topoUid&&t.created_by===_topoUid),
    ...
    {drawFor?createPortal(<div onClick={function(){setDrawFor(null);}} role="dialog" aria-modal="true" style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:9500,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
      <div onClick={function(e){e.stopPropagation();}} style={{background:C.bg,borderTopLeftRadius:18,borderTopRightRadius:18,width:"100%",maxWidth:520,padding:"16px 16px 22px",border:"1px solid "+C.border,maxHeight:"92vh",overflowY:"auto",overscrollBehavior:"contain",boxSizing:"border-box"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          ...
          <button onClick={function(){setDrawFor(null);}} style={POP_CLOSE} aria-label="Close">✕</button>
        </div>
...
function TopoPhotoModal({photo,onClose,onDraw,onDescribe}){
  ...
  return createPortal(<div onClick={onClose} role="dialog" aria-modal="true" style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.9)",zIndex:9400,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
    <div onClick={function(e){e.stopPropagation();}} style={{background:C.bg,borderRadius:16,width:"100%",maxWidth:520,border:"1px solid "+C.border,overflow:"hidden"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 14px",borderBottom:"1px solid "+C.border}}>
        ...
        <button onClick={onClose} style={POP_CLOSE} aria-label="Close">✕</button>
      </div>
...
/* shortDate now lives in ClimbMatchCore beside DLOCALE: three more surfaces needed it, and two
   copies of a date formatter is how this codebase ended up with four grade parsers. Its catch was
   also DEAD -- toLocaleDateString on an Invalid Date returns "Invalid Date" rather than throwing --
   so the core copy is hardened rather than moved verbatim. */
...
/* Each variant chip reads as the whole grade ("5.9−", "5.9", "5.9+") rather than the bare
   suffix: the plain grade used to be a lone "·", which read as a stray dot, not as "5.9". */
const gradeChipLabel=function(g){return String(g).replace(/-$/,"−");};
...
function SuggestFix({route,onClose,onSubmit,onLog,scrollTo,pending,peakCoord,prefill}){
  ...
  const DISCS=[[DL.trad,"trad",{discipline:"rock",style:"Trad"}],[DL.sport,"sport",{discipline:"rock",style:"Sport"}],[DL.toprope,"toprope",{discipline:"toprope"}],[DL.bouldering,"bouldering",{discipline:"bouldering"}],[DL.alpine,"alpine",{discipline:"alpine"}],[DL.ice,"ice",{discipline:"ice"}],[DL.mixed,"mixed",{discipline:"mixed"}],[DL.aid,"aid",{discipline:"aid"}],[DL.mountaineering,"mountaineering",{discipline:"mountaineering"}],[DL.scrambling,"scrambling",{discipline:"scrambling"}]];
  ...
  /* Tap adds one, − takes one away. It used to cycle 1→5→0, so a route needing a sixth
     alpine draw or a triple set of cams could not be described. 60 is a sanity ceiling only. */
  const bumpRack=function(key,d){setRack(function(r){var n=Object.assign({},r);var q=Math.max(0,Math.min(60,(n[key]||0)+(d||1)));if(q===0)delete n[key];else n[key]=q;return n;});};
  /* The pitch editor opened on ONE BLANK ROW even when the route already had a full
     ...
     the editor had just written as pp.len. One shape means what gets submitted is what
     PitchTable already renders. Prefill is no longer strictly IDENTITY: it converts the stored
     metres into the CLIMBER'S OWN UNITS, because the box asks in those units while the column
     stays canonical. The row SHAPE is still one shape -- only the length's unit differs between
     draft and column, exactly as itinDaysToDraft puts metres under the key `gainFt` for a
     metric climber. Reverting any one of those edges is silent; see the injection suite. */
  ...
  const routePitches=(route.pitchDetail&&route.pitchDetail.length)
    ?route.pitchDetail.map(function(p,i){return {pitch:p.pitch!=null?p.pitch:(p.n!=null?p.n:i+1),grade:p.grade||"",lengthM:p.lengthM!=null?uLenN(p.lengthM):"",gear:p.gear||"",notes:p.note!=null?p.note:(p.notes||""),anchor:p.anchor||"",bolts:p.bolts!=null?p.bolts:"",crux:!!p.crux};}).sort(function(a,b){return a.pitch-b.pitch;})
    :[blankPitch(1)];
  ...
  const pitchStr=function(ps){return ps.map(function(pp,i){var bits=[pp.grade,pp.lengthM?pp.lengthM+uLenUnit():"",pp.anchor?("anchor: "+pp.anchor):"",(pp.bolts!==""&&pp.bolts!=null)?(pp.bolts+" bolts"):"",pp.crux?"crux":"",pp.gear,pp.notes].filter(Boolean);return bits.length?("P"+(pp.pitch!=null?pp.pitch:i+1)+": "+bits.join(", ")):"";}).filter(Boolean).join("; ");};
...
rack:(boulder||cat==="sport"),protRating:!(cat==="trad"||cat==="sport"),/* `draws:true` made this the one field in the form unreachable for EVERY discipline — it sat in
  ...
  const renderInput=function(f){var v=valOf(f);
    ...
    if(OBJ_KEYS[f.type]){var _ks=OBJ_KEYS[f.type],_cur=(route[f.type]||{}),_val=OBJ_STATE[f.type][0],_set=OBJ_STATE[f.type][1];
      ...
      return <div>{_ks.map(function(k){var on=_dget(_cur,k[0]);if(on!=null&&typeof on==="object")on=objLabel(k,on);else if(on!=null&&k[3]==="enum")on=objLabel(k,on);return <div key={k[0]} style={{marginBottom:9}}>
        <div style={{...SUB_LABEL,marginBottom:4}}>{k[1]}</div>
  ...
  if(f.type==="pitches")return (vals.pitchDetail||[]).map(function(p,i){
    ...
    /* What was typed is read in the climber's OWN units and stored CANONICAL (metres) —
       the same conversion-at-the-edges the variants and waypoint branches apply. */
    var _len=uLenIn(p.lengthM),_bolts=parseInt(p.bolts,10);
    return {n:i+1,grade:p.grade||"",lengthM:(_len!=null&&isFinite(_len))?_len:null,gear:p.gear||"",note:p.notes||"",bolts:isFinite(_bolts)?_bolts:0,anchor:String(p.anchor||"").trim(),crux:!!p.crux,photos:[],comments:[]};
...
var _pv=(k[3]==="num")?(isFinite(parseFloat(v))?parseFloat(v):undefined):(k[3]==="enum")?((k[4]||[]).some(function(o){return typeof o[0]==="number";})?(isFinite(parseFloat(v))?parseFloat(v):undefined):v):((k[0]==="group_limit"&&/^[0-9]+$/.test(v))?parseInt(v,10):v);if(_pv===undefined)return;/* dotted keys write nested, so `fitnessSpec.hiking` lands inside the object the panel reads */if(String(k[0]).indexOf(".")>=0){_o=_dset(_o,k[0],_pv);}else{_o[k[0]]=_pv;}});return _o;}if(CANON[f.k])return CANON[f.k](filledStr(f));return filledStr(f);};
  ...
  /* A STRAY TAP OUTSIDE THE PANEL MUST NOT DESTROY TYPED BETA. This backdrop closed the form
     unconditionally, and the form holds up to 35 inputs — so one tap beside the panel discarded
     everything a climber had written, with no warning and nothing to undo it. The inner panel
     already shields the click, so only a genuine outside tap gets here.
     Gated on the form's OWN emptiness signal rather than a new one: `canSubmit` is
     `totalUpdates > 0`, which the submit button already computes. An untouched form still
     dismisses on a backdrop tap, which is what makes an accidental open cheap to leave; once
     there is work to lose, leaving is a deliberate act via the ✕ or Cancel, both of which are
     right there. The two mistakes do not cost the same. */
  return createPortal(<div onClick={canSubmit?undefined:onClose} role="dialog" aria-modal="true" aria-label="Contribute info" style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:100000,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"72px 12px 32px",overflowY:"auto",overscrollBehavior:"contain"}}>
    <div onClick={function(e){e.stopPropagation();}} style={{background:C.surface,borderRadius:18,width:"100%",maxWidth:460,border:"1px solid "+C.border,overflowY:"auto",overscrollBehavior:"contain",maxHeight:"92vh",display:"flex",flexDirection:"column"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px",borderBottom:"1px solid "+C.border,flexShrink:0}}><div style={{fontSize:16,fontWeight:700,color:C.text}}>{sent?"Thanks for contributing":"Add or fix info"}</div><button onClick={onClose} aria-label="Close" style={POP_CLOSE}>{"✕"}</button></div>
      ...
      <div ref={scrollRef} style={{minHeight:0,padding:16,overflowY:"auto",flex:1}}>
        ...
        <div style={{background:"linear-gradient(180deg,"+C.blueBg+","+C.card+")",border:"1px solid "+C.blue+"55",borderRadius:14,padding:"13px 14px 14px",marginBottom:18}}>
          <div style={{display:"flex",alignItems:"baseline",justifyContent:"space-between",gap:8,marginBottom:10}}><CardHead>{"Discipline"}</CardHead>{discChanged?<span style={{fontSize:11,fontWeight:700,color:C.amber}}>{"Will update the route"}</span>:null}</div>
        ...
        <div id="sf-section-bailout" style={{background:C.card,border:"1px solid "+C.green+"40",borderLeft:"4px solid "+C.green,borderRadius:12,padding:"11px 14px 14px",marginBottom:18}}>
          <CardHead tone={C.green} style={{margin:"2px 0 6px"}}>Bailout / retreat points</CardHead>
        ...
        <div id="sf-section-startLocation" style={{background:C.card,border:"1px solid "+C.blue+"40",borderLeft:"4px solid "+C.blue,borderRadius:12,padding:"11px 14px 14px",marginBottom:18}}>
          <CardHead style={{margin:"2px 0 6px"}}>Start of climb</CardHead>
        ...
        <div style={{marginBottom:4}}><div style={{...SUB_LABEL,margin:"2px 0 6px"}}>{"Why / source (optional)"}</div><input aria-label="Why / source (optional)" value={note} onChange={function(e){setNote(e.target.value);}} placeholder="e.g. climbed it last week; guidebook lists WI4" style={fld}/></div>
      </div>
...
const mtn=(function(){const _s=MOUNTAINS.find(m=>m.id===route.mountainId);if(_s&&_s.name)return _s;const _d=route._dbArea;if(_d&&_d.name)return _d;return Object.assign({},_d||{},{name:"this area"});})(),dc=CAT[catOf(route)]||{color:C.textSub,bg:C.surface,icon:"",label:catOf(route)};const partners=useMemo(()=>ALL_CLIMBERS.filter(c=>c.objectiveIds.includes(route.id)),[route.id]);/* REAL climbers who saved this route (0211). The list below was the example profiles only, so a real climber with this route on their objectives was invisible on its own page. */const _rpRes=usePartnersByObjective(USE_DB&&who?[route.id]:null,null,null,null,USE_DB&&!!who);const realPartners=((_rpRes&&_rpRes.data)||[]).filter(function(p){return p&&p.id&&String(p.id)!==String(who)&&!(blocked||[]).some(function(b){return String(b&&b.id)===String(p.id);});});const _rpObj=useObjectivesOfUsers(realPartners.map(function(p){return p.id;}));const avgS=useMemo(function(){var rated=activity.filter(function(a){return a&&a.stars>0;});return rated.length?rated.reduce(function(s,a){return s+a.stars;},0)/rated.length:(route.stars||0);},[activity,route.stars]);const rkw=useMemo(()=>routeKw(Object.assign({},route,{activity})),[route.id,activity,route.hazards]);const rkFlagged=useMemo(()=>rkw.hazards.filter(h=>!h.official&&h.count>=2),[rkw.hazards]);const cragSibs=useMemo(()=>USE_DB&&dbSibs?dbSibs.map(dbRouteToCamel):ROUTES.filter(x=>x.mountainId===route.mountainId),[route.mountainId,dbSibs,USE_DB]);const routePhotos=useMemo(()=>activity.flatMap(a=>(a.photos||[]).map(u=>({url:u,alt:(a.photoAlts&&a.photoAlts[u])||null,by:a.user,avatar:a.avatar,date:a.date,ascent:a,cat:(a.condTags||[]).some(function(t){return isHazardTag(t);})?"Hazards":"Conditions"}))).concat((route.photos||[]).map(u=>({url:u,cat:"Conditions"}))),[route.id,activity,route.photos]);
  /* Owned HERE rather than inside FloatPlan because the Safety tab is a conditional branch:
     `{tab==="safety"?<div>...</div>:null}` unmounts the form, and React discards the state of
  ...
  return <div ref={topRef}>{fixOpen?<SuggestFix route={route} pending={route._pendingEdits||{}} scrollTo={fixOpenSection} prefill={fixPrefill} peakCoord={mtn.lat!=null?{lat:mtn.lat,lng:mtn.lng,name:mtn.name}:null} onClose={()=>{setFixOpen(false);setFixOpenSection(null);}} onSubmit={onContribute} onLog={onLog?function(){setFixOpen(false);setFixOpenSection(null);onLog(route);}:undefined}/>:null}
    <div style={{minHeight:188,overflow:"hidden",position:"relative",display:"flex",flexDirection:"column",justifyContent:"space-between",padding:"11px 11px 14px",boxSizing:"border-box"}}>
      ...
      <div style={{position:"relative",zIndex:5,display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8}}><button onClick={onBack} style={POP_BACK_MEDIA}>← Back</button><div style={{display:"flex",flexDirection:"row",alignItems:"center",justifyContent:"flex-end",flexWrap:"wrap",gap:6}}><button onClick={()=>setShareOpen(true)} style={{background:"rgba(13,17,23,0.85)",color:"white",border:"1px solid "+C.border,borderRadius:8,padding:"9px 11px",fontSize:12,cursor:"pointer",fontWeight:700}}>↗ Share</button><button onClick={onToggleSave} title={saved?"Saved to your objectives — tap to remove":"Save to your objectives"} style={{background:saved?C.amberBg:"rgba(13,17,23,0.85)",color:saved?C.amber:"white",border:"1px solid "+(saved?C.amber:C.border),borderRadius:8,padding:"9px 11px",fontSize:12,cursor:"pointer",fontWeight:700}}>{saved?"Saved":"Save"}</button><button onClick={onToggleOffline} title={offlineSaved?"Saved on this device — tap to remove":offlinePending?"Saving this climb to your device…":"Save this climb's beta to this device"} style={{background:offlineSaved?C.green:"rgba(13,17,23,0.85)",color:offlineSaved?"#04110a":"white",border:"1px solid "+(offlineSaved?C.green:C.border),borderRadius:8,padding:"9px 11px",fontSize:12,cursor:"pointer",fontWeight:700}}>{offlineSaved?"✓ In pack":offlinePending?"Saving…":"Trip pack"}</button></div>
      </div>
...
{(route.descent||route.descentText||rxOf(route.id).retreat)?<div style={{marginBottom:14}}><SL action={<EditIconButton onClick={()=>{setFixOpenSection("descentText");setFixOpen(true);}} title="Edit descent information"/>} prov={sectionProvenance(route,"descentText")}>DESCENT</SL>{[["Descent",descentBeta(route)]].concat((!cragOnly&&rxOf(route.id).retreat)?[["Retreat / bail",rxOf(route.id).retreat]]:[]).filter(x=>x[1]).map(x=><div key={x[0]} style={{background:C.card,borderRadius:10,padding:"10px 12px",marginBottom:8,border:`1px solid ${C.border}`}}><div style={{fontSize:12,fontWeight:700,color:C.blue,marginBottom:5}}>{x[0]}</div>{splitParagraphs(x[1]).map((p,i)=><p key={i} style={{fontSize:13,color:C.textSub,lineHeight:1.7,margin:i===0?"0 0 8px":"8px 0 0"}}>{p}</p>)}</div>)}</div>:<GapNote what="No descent recorded" why="Nothing says how you get off this route — and on many climbs the way down is the committing part." cta="Describe the descent" onFix={()=>{setFixOpenSection("descentText");setFixOpen(true);}}/>}{/* The rappel table and its prose/count summary, immediately after the descent they belong
    ...
    they used to sit. */}
{(function(){
  /* What climbers who logged it said about the rappels. Shown UNDER the catalog's own
     account and never in place of its headline count. Shown even where the catalog says
     there are no rappels: a party that rapped is evidence the catalog should hear. */
  const crep=rappelReportsOf(activity);
  const crepCard=crep?<div style={{background:C.card,borderRadius:10,padding:"10px 12px",border:`1px solid ${C.border}`,marginTop:8}}><div style={{...SUB_LABEL,marginBottom:4}}>{"CLIMBERS REPORTED · "+crep.n+(crep.n===1?" report":" reports")}</div><div style={{fontSize:12.5,color:C.text,lineHeight:1.6}}>{[(crep.min===crep.max?crep.min:(crep.count.value+" most often ("+crep.min+"–"+crep.max+")"))+" rappel"+(crep.min===crep.max&&crep.min===1?"":"s"),crep.longestM?"longest "+uLenN(crep.longestM)+" "+uLenUnit():null,crep.rope?crep.rope.value+(crep.rope.n>1?" ("+crep.rope.n+")":""):null].filter(Boolean).join(" · ")}</div>{crep.latest?<div style={{fontSize:11.5,color:isRecent(crep.latest)?C.textMuted:C.amber,marginTop:3}}>{"latest "+ago(crep.latest)}</div>:null}</div>:null;
  if(rappelsAreNone(route))return crepCard?<div style={{marginBottom:12}}><SL prov={sectionProvenance(route,"rappels")}>RAPPELS</SL>{crepCard}</div>:null;
  ...
  if(!hasTable&&!heading&&!prose&&!crepCard)return null;
  ...
  return <div style={{marginBottom:12}}>
    ...
    confirm CAMPING & BIVY still reaches the screen. */}{(["alpine","mountaineering","ice","mixed"].includes(route.discipline)||(route.gainFt||0)>=3000||(route.routeFt||0)>=1000)?<div style={{marginBottom:14}}><div style={{background:C.amberBg,border:`1px solid ${C.amber}55`,borderRadius:12,padding:"11px 13px",marginBottom:10,fontSize:12.5,color:C.text,lineHeight:1.5}}><b style={{color:C.amber}}>Committing objective.</b> {(rxOf(route.id).comms||{}).coverage==="good"?"File a float plan below in case something still goes wrong.":"File a float plan below before you lose cell service."}</div><FloatPlan plan={floatPlan} onPlan={setFloatPlan} who={who} scope={route.id?"route:"+route.id:null} defaults={{route:route.name}} coords={(mtn&&mtn.lat!=null&&mtn.lng!=null)?{lat:mtn.lat,lng:mtn.lng,name:mtn.name}:null}/></div>:null}<WeatherPanel waypoints={route.waypoints} showPlan={showPlan} packId={offlineSaved?route.id:null}/><div style={{marginTop:14,marginBottom:14}}><SL>Weather & mountain forecasts</SL><div style={{fontSize:12,color:C.textMuted,margin:"-4px 0 9px",lineHeight:1.5}}>Cross-check several forecasts before committing — mountain weather turns fast.</div><div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,overflow:"hidden"}}>{(mtn.lat!=null&&mtn.lng!=null?[{n:"NWS point forecast",u:"https://forecast.weather.gov/MapClick.php?lon="+mtn.lng+"&lat="+mtn.lat},{n:"NWS hourly graph",u:"https://forecast.weather.gov/MapClick.php?lon="+mtn.lng+"&lat="+mtn.lat+"&FcstType=graphical"}]:[]).concat([{n:"Mountain-Forecast.com",u:"https://www.google.com/search?q="+encodeURIComponent("site:mountain-forecast.com "+mtn.name)},{n:"OpenSnow forecast",u:"https://opensnow.com/"},{n:"Fire & smoke — AirNow",u:"https://www.airnow.gov/"},{n:"Active fires — InciWeb",u:"https://inciweb.wildfire.gov/"}]).concat(mtn.avyZone?[{n:"Avalanche forecast ("+mtn.avyZone+")",u:avyCenterFor(mtn)[2]}]:[]).map(function(a,i,arr){return <a key={i} href={a.u} target="_blank" rel="noreferrer" style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 13px",borderBottom:i<arr.length-1?`1px solid ${C.borderLight}`:"none",textDecoration:"none"}}><span style={{fontSize:13,color:C.text}}>{a.n}</span><span style={{fontSize:12,color:C.blue,fontWeight:700}}>Open →</span></a>;})}</div></div><SafetyMatrix route={route} activity={activity} mountain={mtn} hzVotes={hzVotes} onVote={onVoteHazard} onOpenContribute={()=>{setFixOpenSection("bailout");setFixOpen(true);}} onOpenStartLoc={()=>{setFixOpenSection("startLocation");setFixOpen(true);}} onOpenHazards={(seed)=>{setFixOpenSection("haz");setFixPrefill(seed?{haz:seed}:null);setFixOpen(true);}} onOpenTurnaround={()=>{setFixOpenSection("turn");setFixOpen(true);}}/>{(()=>{const cm=rxOf(route.id).comms;/* Cell coverage is contributable (the `comms` field) and this was the only panel on the Safety tab with no way to correct what it says — every neighbour here has an EditIconButton. Coverage is also the one fact on this screen that changes with a carrier and a new tower rather than with the rock, so it is exactly the kind of thing the last party knows better than the catalog. */
        if(!cm)return route.comms?<div style={{background:C.card,borderRadius:10,padding:"11px 12px",marginTop:14,border:`1px solid ${C.border}`}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,marginBottom:5}}><CardHead>Cell / sat coverage</CardHead><EditIconButton onClick={()=>{setFixOpenSection("comms");setFixOpen(true);}} title="Edit cell / satellite coverage"/></div><div style={{fontSize:12.5,color:C.textSub,lineHeight:1.5}}>{route.comms}</div></div>:null;const COV={none:{t:"No service",c:C.red,bg:C.redBg},spotty:{t:"Spotty signal",c:C.amber,bg:C.amberBg},good:{t:"Good signal",c:C.green,bg:C.greenBg}};return <div style={{background:C.card,borderRadius:10,padding:"11px 12px",marginTop:14,border:`1px solid ${C.border}`}}><div style={{display:"flex",alignItems:"center",gap:7,marginBottom:cm.note?5:0}}><CardHead>Cell coverage</CardHead><span style={{fontSize:11.5,fontWeight:700,color:(COV[cm.coverage]||COV.none).c,background:(COV[cm.coverage]||COV.none).bg,padding:"2px 8px",borderRadius:10}}>{(COV[cm.coverage]||COV.none).t}</span></div>{cm.note?<div style={{fontSize:12.5,color:C.textSub,lineHeight:1.5}}>{cm.note}</div>:null}</div>;})()}</div>:null}
...
function WeatherPanel({waypoints,showPlan,packId}){
  ...
  useEffect(function(){
    ...
    points.forEach(function(w){
      ...
      // length fields) from meters to feet, so freezeMax arrives in CANONICAL
      // FEET — never scale it by 3.28 again. DISPLAYING it still goes through
      // uElev(), exactly like every other tile in this panel: converting the
    ...
    <div style={{display:"flex",flexDirection:"column",gap:10}}>{points.map(function(w){const k=w.type+"_"+w.name;const d=data[k];const expDate=expandedDay[k];const _wty=wpType(w);const wCol=wpColor(_wty);return <div key={k} style={{background:C.card,border:"1px solid "+C.border,borderRadius:14,padding:"13px 14px"}}>
      ...
      {!d?<div style={{fontSize:12,color:C.textMuted}}>Loading forecast…</div>:d.error?<div style={{fontSize:12,color:C.textMuted}}>Forecast unavailable — check your connection.</div>:(d.savedAt&&!d.days.length)?<div style={{fontSize:12,color:C.amber,lineHeight:1.45}}>{"No signal, and the forecast saved with your trip pack ("+savedAgo(d.savedAt)+") has no days left in it."}</div>:<>{d.savedAt?<div style={{fontSize:11.5,color:C.amber,lineHeight:1.45,marginBottom:7}}>{"No signal — this is the forecast saved with your trip pack "+savedAgo(d.savedAt)+". Conditions may have changed since."}</div>:null}<div style={{display:"flex",gap:9,overflowX:"auto",paddingBottom:3}}>{d.days.map(function(dy,di){
        ...
        return <div key={dy.date} style={{flexShrink:0,width:272,display:"flex",flexDirection:"column",background:C.surface,borderRadius:12,padding:"12px 13px",border:"1px solid "+C.border,borderTop:"3px solid "+wxTempColor(dy.tempLo)}}>
          ...
          <div style={{fontSize:17,fontWeight:800,marginBottom:3}}><span style={{color:wxTempColor(dy.tempHi)}}>{"High "+uTemp(dy.tempHi)}</span><span style={{color:C.textMuted,fontWeight:600}}>{"  ·  "}</span><span style={{color:wxTempColor(dy.tempLo)}}>{"Low "+uTemp(dy.tempLo)}</span></div>
          <div style={{fontSize:11,fontWeight:frostbite?700:400,color:frostbite?C.red:chilly?C.blue:C.textMuted,marginBottom:10}}>{"Feels like: High "+uTemp(dy.feelsHi)+" · Low "+uTemp(dy.feelsLo)+(chilly?" (wind chill)":"")+(frostbite?" — frostbite risk in 30 min":"")}</div>
          {dy.parts.length?<div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6,marginBottom:10}}>{dy.parts.map(function(p){return <div key={p.label} style={{background:C.card,borderRadius:9,padding:"7px 6px",border:"1px solid "+C.border}}>
            ...
            <div style={{fontSize:13.5,fontWeight:800,color:wxTempColor(p.temp)}}>{uTemp(p.temp)}</div>
