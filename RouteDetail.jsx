...
function RouteGearEssentialsBox({route,essentials,onEdit}){const [gearTier,setGearTier]=useState("midweight");const disc=catOf(route);const _itDays=(route&&route.itinerary&&route.itinerary.days&&route.itinerary.days.length)||0;const _multiDay=_itDays>1;const assumed=assumedFor(route,disc).concat(["First aid kit"]).concat(_multiDay?["Tent / shelter","Sleeping bag","Sleeping pad","Stove + fuel","Extra food (overnight)"]:[]);const normGear=s=>(s||"").toLowerCase().trim().replace(/\s+/g," ");const asmNorm=new Set(assumed.map(normGear));const ess=(essentials||[]).filter(x=>!asmNorm.has(normGear(x)));if(!assumed.length&&!ess.length)return null;return <div style={{background:C.card,borderRadius:12,padding:"12px 14px",border:`1px solid ${C.border}`,marginTop:12}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,marginBottom:3}}><span style={{fontSize:13,fontWeight:700,color:C.text}}>GEAR & ESSENTIALS</span>{onEdit?<EditIconButton onClick={function(){onEdit();}} title={ess.length?"Suggest a correction to this route's essentials":"Add gear this route needs beyond the standard kit"}/>:null}</div><div style={{fontSize:12,color:C.textMuted,marginBottom:10,lineHeight:1.45}}>{"Standard kit for "+(DISC[disc]&&DISC[disc].label?DISC[disc].label.toLowerCase():disc)+" — check it against this route's own notes above, which take precedence."}</div><div style={{display:"flex",flexDirection:"column",gap:5,marginBottom:11}}>{assumed.map((gi,i)=><div key={i} style={{display:"flex",gap:8,fontSize:12.5,color:C.text,lineHeight:1.45}}><span style={{color:C.blue,flexShrink:0}}>•</span><span>{gi}</span></div>)}</div>{(function(){const cond=conditionalFor(route,disc);if(!cond)return null;return <div style={{paddingTop:10,borderTop:`1px solid ${C.borderLight}`,marginBottom:11}}><div style={{fontSize:11.5,fontWeight:700,color:C.amber,marginBottom:3}}>Only if there is snow</div><div style={{fontSize:11.5,color:C.textMuted,marginBottom:7,lineHeight:1.45}}>This route mentions snow gear conditionally, not as standard kit — check current conditions before you carry it.</div><div style={{display:"flex",flexDirection:"column",gap:5,marginBottom:cond.quote?8:0}}>{cond.items.map((gi,i)=><div key={i} style={{display:"flex",gap:8,fontSize:12.5,color:C.text,lineHeight:1.45}}><span style={{color:C.amber,flexShrink:0}}>?</span><span>{gi}</span></div>)}</div>{cond.quote?<div style={{fontSize:11.5,color:C.textSub,lineHeight:1.5,background:C.surface,borderRadius:8,padding:"7px 9px",fontStyle:"italic"}}>{"“"+cond.quote+"”"}</div>:null}</div>;})()}{ess.length?<div style={{paddingTop:10,borderTop:`1px solid ${C.borderLight}`,marginBottom:11}}><div style={{fontSize:11.5,fontWeight:700,color:C.green,marginBottom:3}}>Specific to this route</div><div style={{fontSize:11.5,color:C.textMuted,marginBottom:7,lineHeight:1.45}}>From this route's notes on file — on top of the standard kit.</div><div style={{display:"flex",flexDirection:"column",gap:5}}>{ess.map((gi,i)=><div key={i} style={{display:"flex",gap:8,fontSize:12.5,color:C.text,lineHeight:1.45}}><span style={{color:C.green,flexShrink:0}}>▸</span><span>{gi}</span></div>)}</div></div>:null}<div style={{paddingTop:10,borderTop:`1px solid ${C.borderLight}`}}><div style={{fontSize:11.5,fontWeight:700,color:C.textSub,marginBottom:7}}>Margin</div><div style={{display:"flex",gap:5,marginBottom:8}}>{GEAR_MARGIN_TIERS.map(t=>{const on=gearTier===t[0];return <button key={t[0]} onClick={()=>setGearTier(t[0])} style={{flex:1,padding:"9px 4px",borderRadius:8,border:"1px solid "+(on?C.blue:C.border),background:on?C.blueBg:C.surface,color:on?C.blue:C.textSub,fontSize:11.5,fontWeight:700,cursor:"pointer"}}>{t[1]}</button>;})}</div>{(()=>{const margin=(GEAR_MARGIN_TIERS.find(t=>t[0]===gearTier)||GEAR_MARGIN_TIERS[1])[2];return margin.length?<div style={{display:"flex",flexDirection:"column",gap:5}}>{margin.map((gi,i)=><div key={i} style={{display:"flex",gap:8,fontSize:12.5,color:C.textSub,lineHeight:1.45}}><span style={{color:C.amber,flexShrink:0}}>+</span><span>{gi}</span></div>)}</div>:<div style={{fontSize:12,color:C.textMuted,lineHeight:1.5}}>Core only.</div>;})()}</div></div>;}
function RouteGearCheck({route,onSeeReports,rack,onEditRack,onEditEssentials,essentials,rackGeneric}){const disc=catOf(route);
  ...
  return <><RouteRackBox route={route} onEditRack={onEditRack} rack={rack} onSeeReports={onSeeReports} rackGeneric={rackGeneric}/><RouteGearEssentialsBox route={route} essentials={essentials} onEdit={onEditEssentials}/></>;
}
...
function rappelLabel(route){if(!_rapEdited(route)&&route&&route.rappelDetail&&route.rappelDetail.length)return route.rappelDetail.length+"x";var r=route&&route.rappels;if(r==null)return null;if(typeof r==="object")return r.count!=null?r.count+"x":null;var t=String(r).trim();if(/^\d+$/.test(t))return t+"x";var m=t.match(/^~?\s*(\d+)\s*[-\u2013]\s*(\d+)/);if(m)return m[1]+"\u2013"+m[2];var n=rappelCount(route);return n!=null?"~"+n+"x":null;}
...
function PitchTable({route,focus,onEdit,comments,onCommentAdd}){
  ...
  return <div style={{background:C.card,borderRadius:12,padding:"12px 14px",border:`1px solid ${C.border}`}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8,gap:10,flexWrap:"wrap"}}><div style={{fontSize:12,fontWeight:700,color:C.blue,whiteSpace:"nowrap"}}>{lbl+" · "+((route.pitches&&route.pitches>pitches.length)?(pitches.length+" of "+route.pitches):pitches.length)}</div><div style={{display:"flex",alignItems:"center",gap:8,marginLeft:"auto"}}><div style={{fontSize:11.5,fontWeight:700,color:C.blue,background:C.blueBg,border:"1px solid "+C.blueDim,borderRadius:7,padding:"4px 9px",whiteSpace:"nowrap"}}>Tap for detail</div>{onEdit?<EditIconButton onClick={onEdit} title="Edit pitch descriptions"/>:null}</div></div>
        ...
        return <div key={i} style={{display:"flex",gap:10,marginBottom:i<rows.length-1?10:0}}>
          ...
          <div style={{flex:1,minWidth:0,background:C.surface,border:"1px solid "+C.border,borderRadius:10,padding:"9px 11px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,flexWrap:"wrap",marginBottom:r._note?5:0}}>
              ...
              <span style={{display:"flex",alignItems:"center",gap:7,flexShrink:0,marginLeft:"auto"}}>
                {r.grade?<span style={{fontSize:11.5,fontWeight:700,color:C.amber,background:C.amberBg,border:"1px solid "+C.amber+"55",borderRadius:6,padding:"2px 7px",whiteSpace:"nowrap"}}>{r.grade}</span>:null}
...
function TrailheadCard({route,onEdit}){
  ...
  const wp=(route.waypoints||[]).find(w=>w&&w.type==="Trailhead")||null;
  ...
  const copy=function(){
    if(!hasCoord)return;
    try{navigator.clipboard.writeText(lat.toFixed(5)+", "+lng.toFixed(5));setCopied(true);setTimeout(()=>setCopied(false),1600);}catch(e){}
  };
...
function ConsensusPanel({route,activity,hzVotes,onVote}){
  ...
  if(!c)return <div style={{color:C.textMuted,fontSize:13,padding:10}}>No reports yet — be the first to log this climb.</div>;const _dates=(activityAll||[]).map(function(a){return a.date;}).filter(Boolean).sort();const _latest=_dates[_dates.length-1];const _dago=_latest?Math.round((new Date()-new Date(String(_latest).slice(0,10)+"T12:00:00"))/86400000):null;const _fresh=_dago==null?"":(_dago<=0?"Updated today":_dago===1?"Updated yesterday":(_dago<14?"Updated "+_dago+"d ago":(_dago<60?"Updated "+Math.round(_dago/7)+"w ago":"Updated "+Math.round(_dago/30)+"mo ago")));const _rc=(activityAll||[]).slice().sort(function(a,b){return String(b.date).localeCompare(String(a.date));})[0];const _rtx=_rc?(((_rc.condTags||[]).join(" ")+" "+(_rc.text||"")).toLowerCase()):"";const _good=/(dry|perfect|firm|bluebird|tacky|cool|crisp|prime|stellar)/.test(_rtx);const _bad=/(wet|greasy|humid|snow|ice|verglas|slick|warm|hot|damp|muddy|seep|soaked)/.test(_rtx);const _trend=(_good&&!_bad)?{a:"↗",l:"trending good",c:C.green}:((_bad&&!_good)?{a:"↘",l:"trending poor",c:C.red}:{a:"→",l:"holding steady",c:C.textSub});
...
rack:(boulder||cat==="sport"),protRating:!(cat==="trad"||cat==="sport"),/* `draws:true` made this the one field in the form unreachable for EVERY discipline — it sat in
  ...
  const pendStr=function(f,v){if(v==null)return "";if(UNCANON[f.k]&&v!==""&&!isNaN(parseFloat(v)))return UNCANON[f.k](parseFloat(v));if(f.type==="waypoints")return wpStr(v);if(f.type==="pitches")return pitchStr(v||[]);if(f.type==="itinerary")return itinSummaryStr(v);if(Array.isArray(v))return v.join(", ");return String(v);};
    ...
    <div onClick={function(e){e.stopPropagation();}} style={{background:C.surface,borderRadius:18,width:"100%",maxWidth:460,border:"1px solid "+C.border,overflowY:"auto",overscrollBehavior:"contain",maxHeight:"92vh",display:"flex",flexDirection:"column"}}>
      ...
      <div ref={scrollRef} style={{padding:16,overflowY:"auto",flex:1}}>
        ...
        <div style={{background:"linear-gradient(180deg,"+C.blueBg+","+C.card+")",border:"1px solid "+C.blue+"55",borderRadius:14,padding:"13px 14px 14px",marginBottom:18}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:7}}>{DISCS.map(function(d){var on=disc===d[1];return <button key={d[1]} onClick={function(){setDisc(d[1]);}} style={{padding:"8px 14px",borderRadius:11,border:"1px solid "+(on?C.blue:C.border),background:on?C.blueSolid:C.surface,color:on?"#fff":C.textSub,fontSize:13,fontWeight:700,cursor:"pointer",boxShadow:on?"0 1px 6px "+C.blue+"66":"none"}}>{d[0]}</button>;})}</div>
        </div>
...
function descentBeta(r){if(!r)return "";const a=String(r.descent||"").trim(),b=String(r.descentText||"").trim();return b.length>a.length?b:a;}
...
function RouteDetail({route,presence,autoFix,onAutoFixDone,contribs,onSubTab,initialSubTab,myReports,onBack,onPlan,onViewProfile,connections,onConnect,friendState,saved,onToggleSave,onLog,onOpenReport,onOpenRoute,hzVotes,onVoteHazard,offlineSaved,onToggleOffline,onShareRoute,onContribute,sunReports,onRequestJoin,requested,onFindPartners,diffRatings,setDiffRatings,logged,onOpenCrag,comments,onCommentAdd,onCommentEdit,onCommentDelete,onCommentLike,onCommentReply,onAddTopo,topoConsensus,myItin,onSaveMyItin,crewsForRoute,onShareItinToCrew,myStars,setMyStars,onOpenFireMap}){
  ...
  const tripQ=useRouteTripReports(USE_DB?route.id:null);
...
{(function(){
  ...
  return <div style={{marginBottom:12}}>
    {hasTable?<RappelTable route={route} onEdit={openEdit}/>:<>
      <SL action={<EditIconButton onClick={openEdit} title="Edit rappel information"/>}>{"RAPPELS"+(heading?" · "+heading:"")}</SL>
