...
function BaseCheckinPanel({route,who}){
  ...
  return <div style={{background:C.card,borderRadius:12,padding:"12px 14px",border:`1px solid ${C.border}`,marginBottom:12}}>
    <div style={{fontSize:12,fontWeight:700,color:C.blue,marginBottom:8}}>BASE OF THE CLIMB</div>
...
function RoadReports({activity,reportsUnavailable,onReport}){
  ...
  return <div style={{background:C.card,borderRadius:12,padding:"13px 15px",marginBottom:12,border:"1px solid "+C.border}}>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,marginBottom:4}}><div style={{fontSize:12,fontWeight:700,color:C.teal}}>ROAD CONDITIONS</div>{onReport?<button onClick={onReport} style={{flexShrink:0,fontSize:11.5,fontWeight:800,color:C.green,background:C.greenBg,border:"1px solid "+C.greenDim,borderRadius:8,padding:"5px 11px",cursor:"pointer"}}>Report the road</button>:null}</div>
