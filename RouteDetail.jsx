...
rack:(boulder||cat==="sport"),protRating:!(cat==="trad"||cat==="sport"),/* `draws:true` made this the one field in the form unreachable for EVERY discipline — it sat in
  ...
  const pendStr=function(f,v){if(v==null)return "";if(UNCANON[f.k]&&v!==""&&!isNaN(parseFloat(v)))return UNCANON[f.k](parseFloat(v));if(f.type==="waypoints")return wpStr(v);if(f.type==="pitches")return pitchStr(v||[]);if(f.type==="itinerary")return itinSummaryStr(v);if(f.type==="road")return objStr(ROAD_KEYS,v);if(f.type==="access")return objStr(ACCESS_KEYS,v);if(Array.isArray(v))return v.join(", ");return String(v);};
  ...
  const filledStr=function(f){var v=vals[f.k];if(f.type==="multi")return (v&&v.length)?v.join(", "):"";if(f.type==="rack")return v?rackStr(v):"";if(f.type==="pitches")return v?pitchStr(v):"";if(f.type==="waypoints")return v?wpStr(v):"";if(f.type==="itinerary")return v?itinSummaryStr(v):"";if(f.type==="road")return v?objStr(ROAD_KEYS,v):"";if(f.type==="access")return v?objStr(ACCESS_KEYS,v):"";return v!=null?String(v).trim():"";};
  ...
  const renderInput=function(f){var v=valOf(f);
    ...
    if(f.type==="road"||f.type==="access"){var _ks=f.type==="road"?ROAD_KEYS:ACCESS_KEYS,_cur=(f.type==="road"?(route.road||{}):(route.access||{})),_val=f.type==="road"?road:access,_set=f.type==="road"?setRoad:setAccess;
