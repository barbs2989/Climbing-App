...
function gradeNum(g, s) { if (!g) return null; let m; if (s === "yds" && (m = g.match(/5\.(\d+)([a-d]?)/))) return parseInt(m[1]) + ("abcd".indexOf(m[2]) + 1) / 4; if (s === "v" && (m = g.match(/V(\d+)/))) return parseInt(m[1]); if (s === "wi" && (m = g.match(/WI(\d+)/i))) return parseInt(m[1]); if (s === "m" && (m = g.match(/M(\d+)/))) return parseInt(m[1]); if (s === "aid" && (m = g.match(/[AC](\d)/))) return parseInt(m[1]); if ((m = g.match(/5\.(\d+)([a-d]?)/))) return parseInt(m[1]) + ("abcd".indexOf(m[2]) + 1) / 4; if ((m = g.match(/\b(?:WI|AI)(\d+)/i))) return parseInt(m[1]); if ((m = g.match(/\bV(\d+)/))) return parseInt(m[1]); if ((m = g.match(/\b(TD|PD|AD|ED)\b/)) || (m = g.match(/^(D|F)[+-]?$/))) { const ag = { F:1, PD:2, AD:3, D:4, TD:5, ED:6 }; return ag[m[1]]; } if ((m = g.match(/class\s*(\d)/i)) || (m = g.match(/(\d)\s*(?:rd|th|nd)?\s*class/i))) return parseInt(m[1]); if ((m = g.match(/^\s*(VII|VI|IV|III|II|I|V)\b/))) { const rm = { I:1, II:2, III:3, IV:4, V:5, VI:6, VII:7 }; return rm[m[1]]; } return null; }
