-- 0143 — the pitch table, buildable at creation time.
--
-- `pitch_detail` is the most conspicuous thing a route page shows that the add-a-route form
-- could not supply: a climber could read a pitch-by-pitch table and had no way to write one.
-- The form now has a row builder for it (pitch label, grade, length, notes) and this is the
-- write half. Rule 4 of check:approve-route-columns failed the build until it existed.
--
-- The builder's four keys are MEASURED, not chosen: across 1,155 live entries `notes` appears
-- on 100%, `pitch` on 99.7%, `grade` on 99.0% and `lengthM` on 32.4%. `crux`/`anchor`/`bolts`
-- sit around 27% each and are deliberately left to SuggestFix, which already has a pitch
-- builder for an existing route — seven inputs per row would not survive a 390px phone, which
-- check:overflow guards.
--
-- `pitch` is a LABEL, not a number. On walk-ups these rows are stages ("Trailhead to Yellow
-- Aster meadows"), which is why neither the builder nor this function derives `pitches` from
-- the row count.
--
-- Generated from 0142's body by insertion, not retyping. Idempotent.
