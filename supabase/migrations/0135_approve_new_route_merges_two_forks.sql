-- 0135 — two migrations forked `approve_new_route`, and neither fork was complete.
--
-- 0128 and 0132 both rewrote this function, from the SAME ancestor (0127), a few hours apart.
-- Each added what it cared about and silently dropped what the other added:
--
--     0128  approve_new_route(uuid, numeric)   +grade_num        NO six columns
--     0132  approve_new_route(uuid)            +six columns      NO grade_num
--
-- 0132 says so in its own comment — "0127 introduced this. Replaced whole rather than
-- patched" — which is the tell: it was based on 0127, not on the 0128 that had already
-- superseded 0127. Nothing failed. Both files are green, both are guarded, both are
-- idempotent, and `check:migrations` is satisfied because they carry different numbers.
--
-- MEASURED LIVE before writing this, rather than inferred from the file order. The
-- discriminator needs two calls, because one cannot separate the cases:
--
--     rpc {p_contribution_id}                 -> 400 P0001  (admin gate)
--     rpc {p_contribution_id, p_grade_num}    -> 400 P0001  (admin gate)
--
-- A 1-arg call answering P0001 rather than `42725 function ... is not unique` proves exactly
-- ONE overload exists; the 2-arg call answering P0001 rather than PGRST202 proves that one
-- takes two arguments. So the live function is 0128's — and 0132's six columns are written by
-- nothing. All six exist on `routes` and all six hold **0 populated rows**.
--
-- WHY IT IS INVISIBLE. The six are wired everywhere else. `var SS={…}` in ClimbMatch.jsx
-- allow-lists all of them (`protRating`, `startType`, `landing`, `pads`, `rock`, `crux`), the
-- form collects them, the submit half stores them verbatim in the proposal jsonb,
-- `dbRouteToCamel` maps every one (`protRating:r.prot_rating` … `rockType: r.rock`), and four
-- of them already render in the TECH STATS tiles. Storage, form and display are all correct.
-- Only the write between them is missing, so every layer reviews as finished.
--
-- NO DATA WAS LOST, and that is luck rather than design: `contributions` holds **0**
-- new_route rows in any status and `routes` holds **0** rows with `source='community'`, so
-- the flow has never once run end to end. This is a latent defect caught before first use.
-- Had a climber submitted a boulder problem, the pad count, the landing and the start type --
-- the three fields that make a boulder problem legible -- would have vanished on approval and
-- the tiles would have rendered blank, with the proposal jsonb in `verif.proposal` as the only
-- surviving copy.
--
-- THE DROP IS LOAD-BEARING, for the reason 0128 already documented and 0132 then demonstrated:
-- a differing argument list is a DIFFERENT function, so `create or replace` cannot replace
-- across signatures. Re-applying 0132 today would put its 1-arg version back alongside this
-- one, and a 1-arg call would then be `42725` ambiguous rather than resolving to the default --
-- or worse, resolve to the fork that writes nulls. 0132 also states no `grant`/`revoke` of its
-- own, so a freshly-created 1-arg version would carry Postgres's default EXECUTE-to-PUBLIC
-- instead of 0127's explicit revoke. The admin gate makes that unexploitable, not correct.
--
-- Everything else is unchanged from 0128: same admin gate, same `(area_id, name)` duplicate
-- refusal, same id minting through `route_slug`, same `verif.proposal` fallback, same reliance
-- on the `routes` trigger to maintain `route_count`. `grade_num` still arrives as an argument
-- computed by `lib/grade.js` rather than parsed here -- see 0128 for why a fifth parser in a
-- fourth language is the drift this repo keeps paying for.
--
-- Idempotent: `drop ... if exists` then `create or replace`, so a re-run is a no-op.
-- `check:approve-route-columns` (added with this migration) is what stops a third fork.

-- Clears any stale 1-arg overload. A no-op on the live DB today, and the whole point on any
-- database where 0132 ran after 0128.
