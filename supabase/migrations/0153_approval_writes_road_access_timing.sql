-- 0153 — road, access and timing, buildable at creation time.
--
-- The last three jsonb objects a route page shows that the add-a-route form could not supply.
-- A climber could read who manages the land, what the road is like and how long the day takes,
-- and had no way to write any of it when proposing a climb.
--
-- The form now renders all three from lib/objKeys.js — the SAME lists SuggestFix renders. They
-- moved out of RouteDetail.jsx for this: ClimbMatchCore cannot import RouteDetail (lazy-loaded,
-- and it already imports FROM core, so a static import would both cycle and drag the route page
-- into the main bundle). Same reasoning that moved the rack helpers into lib/rack.js. One list,
-- two forms, so they cannot drift on which sub-key is canonical.
--
-- That last point is the reason this matters rather than being cosmetic: `access` carries TWO
-- land-manager spellings — `land_manager` on 399 of 400 sampled rows and `landManager` on 8 —
-- and display reads `ac.land_manager||ac.landManager`. A form writing the wrong one is accepted,
-- reaches consensus, and changes nothing on screen.
--
-- Rule 4 of check:approve-route-columns failed the build until this existed, naming all three.
-- Fourth time it has caught work in progress rather than in hindsight.
--
-- Generated from 0143's body by insertion, not retyping. Idempotent.
