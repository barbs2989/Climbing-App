...
-- function is the only other writer — measured, not assumed: of every plpgsql/sql function in
-- `public`, `approve_new_route` is the ONLY one whose body still names the column once
-- comments are stripped.
--
-- So route approval was BROKEN on the live database between 0155 and this migration. The insert
-- named a column that had been dropped, and plpgsql resolves column names when the statement
-- first runs rather than when the function is created — so nothing failed at deploy time, no
-- guard went red, and the failure was reserved for the first admin to approve a proposed route:
--   ERROR: column "source" of relation "routes" does not exist
-- That is the worst possible place for it. Approval is the consume half of add-a-route, it is
-- admin-only, and it is exercised by hand rather than by CI.
--
-- No value moves anywhere. The column held an INGEST TAG ('wa-enrich-batch', 'mountainproject'),
-- and this function wrote the constant 'community' into it — a provenance marker for the pipeline,
-- never anything a climber typed. The climber's own "where did you learn this" answer has always
-- lived in `verif.proposal`, which is untouched, so nothing a contributor submitted is lost.
--
-- Generated from 0154's body by DELETION, not retyping, and the generator refuses to write a file
-- whose column count and value count disagree — dropping one of a 52-item pair by hand is exactly
-- how the two forks of this function diverged in the first place. Idempotent.
