// Generates supabase/seed-utah.sql from the OpenBeta full-Utah catalog files.
// Maps catalog -> Phase-0 schema (areas + routes). Run: node supabase/gen-seed-lcc.cjs
const fs = require("fs");
const path = require("path");
const root = path.join(__dirname, "..");
const A = JSON.parse(fs.readFileSync(path.join(root, "catalog/utah/utah_areas.json"), "utf8"));
const R = JSON.parse(fs.readFileSync(path.join(root, "catalog/utah/utah_routes.json"), "utf8"));
...
let sql = `-- ClimbMatch — Utah statewide seed (full state) (OpenBeta, CC0)\n-- ${ordered.length} areas, ${R.length} routes. Facts only; route_count + path are set by triggers.\nbegin;\ntruncate routes, areas restart identity cascade;\n\n`;
...
fs.writeFileSync(path.join(__dirname, "seed-utah.sql"), sql);
console.log(`wrote supabase/seed-utah.sql: ${ordered.length} areas, ${R.length} routes`);
