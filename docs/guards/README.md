...
- **[The route page](route-page.md)** — **`check:bare`**, **`check:provenance`**, **`check:access-checked-line`**, **`check:trailhead-directions`**, **`check:trailhead-direction-shape`**, **`check:approach-section`**, `check:area-name-embed`, **`check:crew-gear`**, `check:area-surfaces`, `check:photo-contract`, **`check:fire`**, **`check:field-renders`**, **`check:summit-briefing`**, **`check:token-boxes`**, **`check:pitch-split`**
...
- **[Database, migrations and git history](database-and-history.md)** — **`check:search-norm`**, **`check:approve-route-columns`**, **`check:counts`**, **`check:function-columns`**, **`check:function-drift`**, **`check:column-drift`**, **`check:migration-claims`**, `check:sql`, `check:merge-survival`, **`audit:silent-reverts`**, `check:schema`, `check:migrations`, **`check:rls`**
