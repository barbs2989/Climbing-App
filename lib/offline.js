...
export async function downloadStateOffline(stateName, onProgress) {
  ...
  await idbPutAll("meta", [{ key: "state:" + st.id, name: stateName, areaCount, routeCount, savedAt: Date.now() }]);
...
export async function offlineAreaChildren(parentId) {
  ...
  return rows.sort((a, b) => (b.route_count || 0) - (a.route_count || 0) || String(a.name).localeCompare(String(b.name)));
}

export async function offlineArea(id) {
  return (await idbGet("areas", id)) || null;
}

export async function offlineAreaRoutes(areaId) {
  const rows = await idbIndexAll("routes", "area_id", areaId);
...
export async function offlineStates() {
  return (await idbGetAll("areas")).filter((a) => a.area_type === "state").sort((a, b) => String(a.name).localeCompare(String(b.name)));
}

export async function offlineDownloads() {
  try { return await idbGetAll("meta"); } catch (e) { return []; }
}
