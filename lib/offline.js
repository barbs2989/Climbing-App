...
/* The scale a row's grade_num is on — `r.grade_system = grade_sys` in routes_in_subtree (0189).
   A state downloaded BEFORE 0189 stores the old labels (ice/mixed/aid rows graded "5.9" said 'wi',
...
function routeMatches(r, f) {
  ...
  // `name_search` is on rows packed after that migration; older packs derive it here.
  if (f.q && !searchMatches(f.q, r.name, r.name_search)) return false;
...
export async function offlineAreaSearch(rootId, q, lim) {
  ...
  const hits = scope.areas.filter((a) => a.id !== rootId && scope.ids.has(a.id)
    && searchMatches(q, a.name, a.name_search));
