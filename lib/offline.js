...
import { searchMatches, searchCanon, searchBare } from "./search";
...
// A MAP of state id -> when its download COMPLETED (savedAt on the complete meta row). Every
// reader but one only asks `.has`; offlineRoutesByIds also reads the time, to decide whether a
// packed copy or a later state download is the fresher answer for one route.
async function completeStates() {
  try {
    const metas = await idbGetAll("meta");
    return new Map(metas.filter((m) => m.complete && String(m.key).startsWith("state:")).map((m) => [m.key.slice("state:".length), m.savedAt || 0]));
...
//
// A REPACK IS A REFRESH, not a new pack: `packedAt` (when the climber chose it) and the
// snapshots riding along with the row (`_snap`, `_wx` — see below) survive it, and `refreshedAt`
// records when this copy of the route was last read from the catalog. The screen shows that age,
// because a copy served with no signal and no date is one the climber cannot weigh.
export async function packRouteOffline(routeId, opts) {
  let prev = null;
  try { prev = await idbGet("pack", routeId); } catch (e) { prev = null; }
  const now = Date.now();
  const keep = { packedAt: (prev && prev.packedAt) || now, refreshedAt: now };
  if (prev && prev._snap) keep._snap = prev._snap;
  if (prev && prev._wx) keep._wx = prev._wx;
  if (opts && opts.seed) {
    await idbPutAll("pack", [{ id: routeId, seed: true, ...keep }]);
    return { seed: true };
  }
  if (!supabase) throw new Error("Catalog backend not configured");
  ...
  await idbPutAll("pack", [{ ...data, ...keep }]);
  return { seed: false };
}

// ── what rides along with a packed route ─────────────────────────────────────
// The route row alone left a packed climb missing things the page shows ONLINE, two of them
// silently: other climbers' AGREED CORRECTIONS (the page fell back to the raw catalog value with no
// notice) and — via the itinerary cache further down — the climber's own plan. Plus the latest TRIP
// REPORTS with who wrote them, and a FORECAST. Each is stored with the time it was read, and each
// reader that falls back to it gets that time too, so the page can say "saved with your trip pack
// N ago" instead of passing an old copy off as live.
//
// `_snap` = { at, contribs, reports, profiles }. Photos are NOT in `contribs`: a photo row is a URL
// to storage that needs a signal anyway, and serving the row without the image would render a
// broken tile (or, filtered out, an empty gallery that reads as "no photos").
// `_wx` = { at, points:[{k,lat,lng,elev}], raw:{[k]:[openMeteo, nws, met]} } — the three services'
// responses exactly as fetched, so the offline forecast is processed by the same code as a live one.
async function patchPackRow(routeId, patch) {
  const row = await idbGet("pack", routeId);
  if (!row) return false; // unpacked meanwhile — never resurrect it
  await idbPutAll("pack", [{ ...row, ...patch }]);
  return true;
}
export async function savePackSnapshot(routeId, snap) { return patchPackRow(routeId, { _snap: { ...snap, at: Date.now() } }); }
export async function savePackForecast(routeId, wx) { return patchPackRow(routeId, { _wx: { ...wx, at: Date.now() } }); }

async function packRow(routeId) { try { return (await idbGet("pack", routeId)) || null; } catch (e) { return null; } }

// Arrays come back STAMPED with `_packedAt` (the snapshot time). The hooks serving them turn off
// react-query's structural sharing so the stamp survives to the screen — it is how the page knows
// it is showing a saved copy. `undefined` (not []) when nothing was saved: orOfflineExact then
// rethrows the network error and the page keeps its honest "couldn't load" branch.
const stamp = (arr, at) => { const a = arr.slice(); Object.defineProperty(a, "_packedAt", { value: at, enumerable: false }); return a; };
export async function offlinePackContribs(routeId) {
  const r = await packRow(routeId);
  return r && r._snap && Array.isArray(r._snap.contribs) ? stamp(r._snap.contribs, r._snap.at) : undefined;
}
export async function offlinePackReports(routeId) {
  const r = await packRow(routeId);
  return r && r._snap && Array.isArray(r._snap.reports) ? stamp(r._snap.reports, r._snap.at) : undefined;
}
// Reporter names for the saved reports. Public profile fields only (the columns useProfilesByIds
// reads), gathered across every packed route.
export async function offlinePackProfiles(ids) {
  const want = new Set(ids || []);
  if (!want.size) return undefined;
  const byId = {};
  try {
    for (const row of await idbGetAll("pack")) {
      ((row._snap && row._snap.profiles) || []).forEach((p) => { if (p && want.has(p.id)) byId[p.id] = p; });
    }
  } catch (e) { return undefined; }
  const out = Object.values(byId);
  return out.length ? out : undefined;
}
// How old a saved copy is, in words — ONE wording for every "saved with your trip pack N ago" line.
export function savedAgo(t) { if (!t) return "a while ago"; const m = Math.max(0, Math.round((Date.now() - t) / 60000)); if (m < 2) return "just now"; if (m < 60) return m + " min ago"; const h = Math.round(m / 60); if (h < 48) return h + " hr ago"; return Math.round(h / 24) + " days ago"; }
export async function packForecast(routeId) { const r = await packRow(routeId); return (r && r._wx) || null; }

// Ages for the screen: when each packed route was chosen, last refreshed, and what rides with it.
export async function packMeta() {
  try {
    return (await idbGetAll("pack")).map((r) => ({
      id: r.id, seed: !!r.seed, packedAt: r.packedAt || null, refreshedAt: r.refreshedAt || r.packedAt || null,
      snapAt: (r._snap && r._snap.at) || null, reports: r._snap && r._snap.reports ? r._snap.reports.length : null,
      corrections: r._snap && r._snap.contribs ? r._snap.contribs.filter((c) => c && c.kind === "field").length : null,
      wxAt: (r._wx && r._wx.at) || null, wxPoints: (r._wx && r._wx.points) || null,
      // "Updated N ago" is the age of the OLDEST part: a refresh whose snapshot failed rewrites the
      // row but not the reports, and a card reading "just now" over three-day-old reports over-claims.
      updatedAt: r._snap && r._snap.at ? Math.min(r.refreshedAt || r.packedAt || 0, r._snap.at) : (r.refreshedAt || r.packedAt || null),
    }));
  } catch (e) { return []; }
}
...
// The fallback behind useRoutesByIds. Two sources, and THE FRESHER ONE WINS. It used to be the
// pack, unconditionally — right when the pack was newer (it was fetched for that route on
// purpose), wrong when a climber packed a route in June and downloaded the whole state in
// September: the older copy hid the newer one, with no way to tell from a trailhead. So a
// packed row is compared, by the time it was last read from the catalog (refreshedAt, else
// packedAt), against the time its state's download COMPLETED; ties and unknown times keep the
// pack. Only the route ROW is chosen here — the pack's snapshot (reports, corrections,
// forecast) is served by its own readers either way.
//
// Rows from a state are gated on that state being marked COMPLETE, for the reason every other
// reader here is — a truncated catalog served to somebody with no signal is one they have no way
// to cross-check.
export async function offlineRoutesByIds(ids) {
  ...
  try {
    for (const row of await idbGetAll("pack")) {
      ...
      const { packedAt, refreshedAt, _snap: _d3, _wx: _d4, ...r } = row;
      packed.set(row.id, { row: r, at: refreshedAt || packedAt || 0 });
    }
  ...
  for (const id of want) {
    ...
    let row = null;
    if (ok.size) { try { row = await idbGet("routes", id); } catch (e) { row = null; } }
    const stateAt = row && ok.has(row._state) ? ok.get(row._state) : null;
    if (p && !(stateAt != null && stateAt > p.at)) { out.push(p.row); continue; }
    if (stateAt != null) out.push({ ...row, areas: await areaEmbed(row.area_id) });
  }
  return out;
}
...
// ── objective Target days ────────────────────────────────────────────────────
// Logbook › Objectives lets you set a Target day per saved climb, and it was a plain useState: gone
// on every reload, which Help had to admit ("it lasts for this session"). Device-local and keyed by
// account, like the bookmarks above, and for a privacy reason on top of the sync one: `objectives`
// is PUBLIC read (0031), and a date says when somebody plans to be away from home, so it does not
// belong beside it. Only YOUR date is ever read (partner search orders by it), so nothing needs it
// from another device's owner.
const OBJDATES_KEY = (who) => "objdates:" + (who || "anon");

export async function savedObjDates(who) {
  try { const row = await idbGet("meta", OBJDATES_KEY(who)); return row && row.dates && typeof row.dates === "object" ? row.dates : null; }
  catch (e) { return null; }
}

export async function saveObjDates(who, dates) {
  await idbPutAll("meta", [{ key: OBJDATES_KEY(who), dates: Object.assign({}, dates || {}), savedAt: Date.now() }]);
}
...
// ── my itineraries, on this device ─────────────────────────────────────────
// `user_itineraries` (0199) is owner-only and network-only, so a climber who packed a route and
// planned it lost their OWN plan at the trailhead — the Plan tab simply had none, with no notice.
// Every successful read is mirrored here, KEYED BY ACCOUNT for the float plan's reason: signing
// out must not hand one climber's plans to the next person on the same phone.
const ITIN_KEY = (who) => "itineraries:" + (who || "anon");
export async function cacheItineraries(who, map) {
  if (!who) return;
  await idbPutAll("meta", [{ key: ITIN_KEY(who), map: map || {}, savedAt: Date.now() }]);
}
// undefined = never cached for this account (the read's error stands); a stored {} is a real answer.
export async function cachedItineraries(who) {
  if (!who) return undefined;
  try { const row = await idbGet("meta", ITIN_KEY(who)); return row && row.map && typeof row.map === "object" ? row.map : undefined; }
  catch (e) { return undefined; }
}
...
/* The scale a row's grade_num is on — `r.grade_system = grade_sys` in routes_in_subtree (0196).
   A state downloaded BEFORE 0196 stores the old labels (ice/mixed/aid rows graded "5.9" said 'wi',
   'm' or 'aid'), so the migration's own relabel rule is applied here too: without it an old
   download would drop every 5.x aid route from a 5.x range that the same search online returns. */
const RELABELLED = { ice: 1, mixed: 1, aid: 1 };
const RX_YDS_LEAD = /^5\.\d/, RX_CLASS_LEAD = /^((class\s*\d)|((easy |low )?\d(st|nd|rd|th)( class)?))/i;
export function rowGradeSystem(r) {
  if (RELABELLED[r.discipline] && typeof r.grade === "string") {
    if (RX_YDS_LEAD.test(r.grade)) return "yds";
    if (RX_CLASS_LEAD.test(r.grade)) return "class";
  }
  return r.grade_system || null;
}

function routeMatches(r, f) {
  // Same rule as routes_in_subtree (0190): every typed word, in any spelling, in any order.
  // Derived from the NAME, never the packed `name_search`: a pack keeps the spelling rule it was
  // saved under, so a pack from before 0201 would not know "dir" is "direct".
  if (f.q && !searchMatches(f.q, r.name)) return false;
  if (f.disc && r.discipline !== f.disc) return false;
  // WI / M / A-C ranges read that scale's own column (0206), since a route can carry several grades
  // at once; a row downloaded before 0206 has no such column, so it falls back to its primary grade
  // when that grade is on the requested scale. Any other scale is 0196's rule.
  const PER_SCALE = { wi: "ice_grade_num", m: "mixed_grade_num", aid: "aid_grade_num" };
  let gn;
  if (f.gradeSys && PER_SCALE[f.gradeSys]) {
    gn = numOrNull(r[PER_SCALE[f.gradeSys]]);
    if (gn == null && rowGradeSystem(r) === f.gradeSys) gn = numOrNull(r.grade_num);
    if (gn == null) return false;
  } else {
    if (f.gradeSys && rowGradeSystem(r) !== f.gradeSys) return false;
    gn = numOrNull(r.grade_num);
  }
...
// "Mount Baker" as an exact match rather than as a mid-word hit. Compared on CANONICAL forms
// (0190), so "mt" and "mount" are one honorific.
const deHonorific = searchBare;
...
export async function offlineAreaSearch(rootId, q, lim) {
  ...
  const scored = hits.map((a) => {
    const lname = searchCanon(a.name);
