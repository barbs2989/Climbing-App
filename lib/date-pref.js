...
// duplicated try/catch starts to be able to drift.
//
...
export function saveDateFmt(f) {
  if (VALID.indexOf(f) < 0) return;
  try { localStorage.setItem(KEY, f); } catch { /* full or blocked — the in-memory choice stands for this session */ }
}
