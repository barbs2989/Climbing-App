...
function useMyHomeStatePath() {
  const { data: uid } = useQuery({
    ...
    queryFn: async () => { const { data } = await supabase.auth.getUser(); return (data && data.user && data.user.id) || null; },
  });
  ...
  // Same query key as useStates, so this shares that cache entry rather than adding a fetch.
...
async function fetchAreaPath(id) {
  ...
  while (currentId && guard++ < 12) {
    // `area_type` and `route_count` come back too. DbAreaTree roots its hierarchy at the STATE
    // ancestor, which it cannot find without the type — and because the chain is returned
    // root-first, a missing `area_type` does not fail loudly: `.find(a => a.area_type === "state")`
    // simply misses and the caller falls back to chain[0], the COUNTRY. The tree then renders,
    // populated and plausible, rooted one level too high. Additive for every existing caller,
    // which reads `id`/`name` only.
    const { data, error } = await supabase.from("areas").select("id,name,parent_id,area_type,route_count").eq("id", currentId).maybeSingle();
    if (error) throw error;
    if (!data) break;
    path.unshift({ id: data.id, name: data.name, area_type: data.area_type, route_count: data.route_count });
