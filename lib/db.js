...
export function useRouteSearch(q, lim = 8) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return ranked.slice(0, lim).map(dbRouteToCamel);
    },
  });
}
...
export function dbRouteToCamel(r) {
  ...
  return {
    ...
    rockType: r.rock,
    alpineGrade: r.alpine_grade, rockGrade: r.rock_grade, iceGrade: r.ice_grade,highPointFt:r.high_point_ft,aidGrade:r.aid_grade,descentText:r.descent_text,protRating:r.prot_rating,startType:r.start_type,landing:r.landing,pads:r.pads,rock:r.rock,crux:r.crux,pitchDetail:pitchArr(r.pitch_detail),rappelDetail:r.rappel_detail,rappelCountNote:r.rappel_count_note,lists:r.lists,permits:r.permit,
...
// Report a photo (0155). Files into content_reports, whose SELECT policy an admin can read --
// which is the whole reason it is not user_reports, see 0155's header.
//
...
// which cascades the report away with it (0155) -- so there is no state to forget to clear.
