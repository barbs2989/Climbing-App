...
export function dbRouteToCamel(r) {
  ...
  return {
    ...
    timing:r.timing, detailedRack:r.detailed_rack, whatToBring:toArr(r.what_to_bring), proTips:toArr(r.pro_tips), watchOut:toWarnArr(r.watch_out), proNeeds:r.pro_needs, bestSeason:r.best_season, beta:Array.isArray(r.beta)?r.beta:(r.beta?[r.beta]:[]),
    ...
    // Same story as `verif` above, and measured before adding: neither of these was mapped,
    // so both were unreachable by every screen. `auto_generated` is populated on 100% of WA
    // routes (5.4% true) and `gear_confidence` on 5.2% (verified/inferred) — between them they
    // are the only per-SECTION provenance signals the routes table actually carries, which is
    // why sectionProvenance() needs them. Do NOT reach for data_quality.confidence instead:
    // it is 94.0% "MEDIUM" across 8,367 WA routes, so it cannot distinguish anything.
    autoGenerated: r.auto_generated, gearConfidence: r.gear_confidence,
