...
export function dbRouteToCamel(r) {
  ...
  return {
    ...
    alpineGrade: r.alpine_grade, rockGrade: r.rock_grade, iceGrade: r.ice_grade,highPointFt:r.high_point_ft,aidGrade:r.aid_grade,descentText:r.descent_text,protRating:r.prot_rating,startType:r.start_type,landing:r.landing,pads:r.pads,rock:r.rock,crux:r.crux,pitchDetail:r.pitch_detail,rappelDetail:r.rappel_detail,rappelCountNote:r.rappel_count_note,lists:r.lists,permits:r.permit,
...
export async function approveNewRoute(contributionId) {
  if (!supabase) return null;
  const { data, error } = await supabase.rpc("approve_new_route", { p_contribution_id: contributionId });
  if (error) throw error;
  return data;
}
