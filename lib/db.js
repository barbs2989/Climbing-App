...
export function useAdminQueueCounts(enabled) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      for (const x of [r, p, c]) if (x.error) throw x.error;
      const reports = r.count || 0, photos = p.count || 0, proposals = c.count || 0;
      return { reports, photos, proposals, total: reports + photos + proposals };
    },
  });
}
