...
// ============================================================================
// FEEDBACK LOOP: CREWS, LOGS, VOUCHES, & TRUST (Phase 2-3)
// ============================================================================

// Query: Get user's active crews
export function useUserCrews(userId) {
  return useQuery({
    queryKey: ["user-crews", userId],
    enabled: !!supabase && !!userId,
    queryFn: async () => {
      const { data, error } = await supabase
        .from("crews").select("*")
        .eq("user_id", userId)
        .in("status", ["forming", "ready", "climbing"])
        .order("created_at", { ascending: false });
      if (error) throw error;
      return data || [];
    },
...
// Query: Get user's archived crews
export function useUserArchivedCrews(userId) {
  return useQuery({
    queryKey: ["user-archived-crews", userId],
    enabled: !!supabase && !!userId,
    queryFn: async () => {
      const { data, error } = await supabase
        .from("crews").select("*")
...
// Query: Get crews for a specific route
export function useRoutCrews(routeId) {
  return useQuery({
    queryKey: ["route-crews", routeId],
    enabled: !!supabase && !!routeId,
    queryFn: async () => {
      const { data, error } = await supabase
        .from("crews").select("*")
        .eq("route_id", routeId)
        .in("status", ["forming", "ready", "climbing", "completed"])
        .order("created_at", { ascending: false });
      if (error) throw error;
      return data || [];
    },
...
// Mutation: Create a new crew
export async function createCrew(userId, routeId, members = []) {
  if (!supabase) return null;
  const { data, error } = await supabase
    .from("crews").insert({
      user_id: userId,
...
// Mutation: Update crew (status, dates, meet place/time, etc.)
export async function updateCrew(crewId, fields) {
  if (!supabase) return null;
  const { data, error } = await supabase
    .from("crews").update({ ...fields, updated_at: new Date().toISOString() })
    .eq("id", crewId).select().single();
  if (error) throw error;
  return data;
}

// Mutation: Archive a crew
export async function archiveCrew(crewId) {
  if (!supabase) return null;
  return updateCrew(crewId, { status: "archived", archived_at: new Date().toISOString() });
}

// Mutation: Delete crew (if not yet started)
...
export async function addVerification(userId, verificationType) {
  if (!supabase) return null;
  const { data, error } = await supabase
    .from("verification_records").insert({
      user_id: userId,
