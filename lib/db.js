...
export function useProfilesByIds(ids) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("profiles").select("id, name, avatar, show_name").in("id", ids);
      if (error) throw error;
      return data;
    },
  });
  ...
  useEffect(() => {
    ...
    channel.on(
      ...
      async (payload) => {
        ...
        if (msg.user_id) {
          const { data: user } = await supabase.from("profiles").select("id,name,avatar,show_name").eq("id", msg.user_id).single();
    ...
    channel.on(
      ...
      async (payload) => {
        ...
        if ((msg.sender_id === userId && msg.recipient_id === partnerId) || (msg.sender_id === partnerId && msg.recipient_id === userId)) {
          // Fetch full user profile for the sender
          if (msg.sender_id) {
            const { data: sender } = await supabase.from("profiles").select("id,name,avatar,show_name").eq("id", msg.sender_id).single();
