...
// "mount goode", so "mt goode" is an exact match for either rather than a miss (0189).
...
export function useRouteSearch(q, lim = 8) {
  ...
  return withPending(useQuery({
    ...
    queryFn: async () => {
      ...
      // WORDS, NOT A PHRASE (0189). Each leg requires every typed word somewhere in
      ...
      // when no leg found a single row, ask search_names_fuzzy (0189, trigram word similarity)
