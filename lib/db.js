...
import { offlineAreaChildren, offlineArea, offlineAreaRoutes, offlineStates, offlineRoutesByIds, offlineSubtreeRoutes, offlineSubtreeRouteCount, offlineAreaSearch, offlineAreasByIds, offlineAreaNamesByIds, ROUTE_AREA_EMBED, packRouteOffline, savePackSnapshot, offlinePackContribs, offlinePackReports, offlinePackProfiles, packMeta, cacheItineraries, cachedItineraries } from "./offline";
import { tidyWaypoints } from "./waypoints";
import { usableFa } from "./fa";
import { withoutLocation } from "./photoPrivacy";
import { searchCanon, searchBare, searchNorm, searchTokens } from "./search";

// SEARCH IS DEBOUNCED HERE, ONCE, rather than at each of the eleven boxes that call these
// three hooks. Every keystroke used to be its own query key, so typing "north ridge" fired
// eleven fetches — and route search runs TWO legs per call, so twenty-two — of which only
// the last answered anything the climber asked. 250ms is below the point where a pause
// reads as lag.
//
// Three properties, each load-bearing:
//   - clearing the box is NOT delayed: an empty term disables the query at once;
//   - the previous results stay on screen while the next term settles, but NEVER after the
//     box is cleared (keepPreviousData alone would do that — a disabled query is pending,
//     and pending is exactly when placeholderData applies);
//   - while the debounce is pending the hook reports isLoading, because the query itself is
//     idle then, and a caller reading "not loading, no rows" would print "No results" for a
//     term nobody has searched yet.
const SEARCH_DEBOUNCE_MS = 250;
function useDebouncedTerm(value) {
  const [v, setV] = useState(value);
  useEffect(() => {
    if (!value) { setV(value); return; }
    const t = setTimeout(() => setV(value), SEARCH_DEBOUNCE_MS);
    return () => clearTimeout(t);
  }, [value]);
  const term = value ? v : value;
  return [term, !!value && term !== value];
}
const holdWhileTyping = term => prev => (term ? prev : undefined);
function withPending(res, pending) {
  return pending ? { ...res, isLoading: true, isFetching: true } : res;
}
...
// The same thing for a query whose answer can legitimately be EMPTY. orOffline treats an empty
// result as "nothing is stored", which is right for a route list or a state list — those are
// unfiltered, so empty means undownloaded. A FILTERED search is different: "no route in this
// area is under 5.6" and "this area was never downloaded" are different facts, and reporting the
// first as a failed read is a false claim of its own kind. A count of 0 is the sharpest case,
// because 0 is falsy and orOffline would rethrow on it every time.
//
// So the fallback signals absence with `undefined` and anything else — including `[]` and `0` —
// is a real answer. Kept as a sibling rather than a widening of orOffline: that helper's
// emptiness rule is load-bearing for the readers already using it, and loosening it there would
// let an undownloaded area serve an empty catalog as though it were the whole one.
async function orOfflineExact(networkFn, offlineFn) {
  try { return await networkFn(); }
  catch (err) {
    try { const local = await offlineFn(); if (local !== undefined) return local; }
    catch (e) { /* fall through to the network error */ }
    throw err;
  }
}
...
export function useAreaNamesByIds(ids) {
  ...
  return useQuery({
    ...
    // orOfflineExact, because the answer is a MAP: an empty one is truthy, so orOffline
    // would substitute it for a failed read and every name would degrade to its placeholder
    // as though it had been resolved. A partial map is a real answer and is kept.
    queryFn: () => orOfflineExact(async () => {
      const { data, error } = await supabase.from("areas").select("id,name").in("id", ids);
      if (error) throw error;
      const map = {};
      (data || []).forEach(a => { map[a.id] = a.name; });
      return map;
    }, () => offlineAreaNamesByIds(ids)),
  });
}

// The same lookup, plus `area_type` — backs the Logbook's Saved view, which files a saved PEAK
// apart from a saved crag or range. A peak IS an area row, so "Save" on a peak page has always
// written the same bookmark; without the type the two could only render as one undifferentiated
// "Saved areas" list. A map of {name, type}, with the same undefined-not-{} offline rule as above.
export function useAreaCardsByIds(ids) {
  const key = (ids || []).slice().sort().join(",");
  return useQuery({
    queryKey: ["area-cards", key],
    enabled: !!supabase && !!(ids || []).length,
    queryFn: () => orOfflineExact(async () => {
      const { data, error } = await supabase.from("areas").select("id,name,area_type").in("id", ids);
      if (error) throw error;
      const map = {};
      (data || []).forEach(a => { map[a.id] = { name: a.name, type: a.area_type }; });
      return map;
    }, async () => {
      const rows = await offlineAreasByIds(ids);
      if (!rows.length) return undefined;
      const map = {};
      for (const a of rows) map[a.id] = { name: a.name, type: a.area_type };
      return map;
    }),
  });
}
...
// Every string here is CANONICAL (lib/search.js): "Mt. Goode" and "Mount Goode" are both
// "mount goode", so "mt goode" is an exact match for either rather than a miss (0190).
function routeSearchScore(row, needle) {
  const rn = searchCanon((row && row.name) || "");
  const an = searchCanon((row && row.areas && row.areas.name) || "");
...
export function useRouteSearch(q, lim = 8) {
  const [qq, typing] = useDebouncedTerm((q || "").trim());
  const homeState = useMyHomeStatePath();
  return withPending(useQuery({
    queryKey: ["route-search", qq, lim, homeState || ""],
    enabled: !!supabase && !!qq,
    placeholderData: holdWhileTyping(qq),
    queryFn: async () => {
      const SEL = "*, areas(name,area_type,region,lat,lng,elevation_ft,prominence_ft,avy_zone,blurb,parent:parent_id(name))";
      const needle = searchCanon(qq);
      // WORDS, NOT A PHRASE (0190). Each leg requires every typed word somewhere in
      // `name_search`, which carries every spelling of each word — so "mt baker", "baker mt",
      // "Mount Baker" and "mount-baker" are one query, and "bobs wall" reaches "Bob's Wall".
      // It used to be `ilike("name", "%mt baker%")`: one verbatim substring, which found a
      // highway called "Bellingham and Mt Baker Hwy" and never the mountain.
      const toks = searchTokens(qq);
      if (!toks.length) return [];
      const words = (b) => toks.reduce((acc, t) => acc.ilike("name_search", `%${t}%`), b);
      // The prefix leg's job is to put rows whose name STARTS with the query in front of the
      // scorer. `name_search` leads with each word's canonical form, so the first word is
      // matched canonically ("mt baker" -> "mount%") OR as typed — "st" may be the start of
      // "Stuart", not "saint", and must not be rewritten into a word the climber never meant.
      const firstForms = [...new Set([toks[0], searchCanon(toks[0])])];
      const startsWith = (b) => b.or(firstForms.map((f) => `name_search.ilike.${f}%`).join(","));
      ...
      const [byPrefix, byName, areaHit, areaExact] = await Promise.all([
        words(startsWith(supabase.from("routes").select(SEL))).limit(lim * 2),
        words(supabase.from("routes").select(SEL)).limit(lim * 2),
        words(supabase.from("areas").select("id,name,path")).order("route_count", { ascending: false }).limit(5),
        // Exact = the same words in any spelling: name_search is a pure function of the words,
        // so equal name_search means "Mt. Baker" and "mount baker" name the same place.
        supabase.from("areas").select("id,name,path").eq("name_search", searchNorm(qq)).order("route_count", { ascending: false }).limit(25),
      ]);
      ...
      // A TYPO GETS A SECOND CHANCE, AND ONLY A TYPO. "mt rainer", "shucksan", "forbiden":
      // when no leg found a single row, ask search_names_fuzzy (0190, trigram word similarity)
      // for the nearest names instead of printing nothing. Never blended into a query that
      // matched — a near-miss must not displace a real hit. Every such row scores the scorer's
      // floor, so the similarity order set here survives the stable sort below.
      if (!merged.length) {
        const fz = await supabase.rpc("search_names_fuzzy", { q: qq, lim });
        if (fz.error) throw fz.error;
        const simOf = new Map(); // route id -> the similarity that brought it in
        const fzAreas = (fz.data || []).filter((x) => x.kind === "area").slice(0, 3);
        const fzRoutes = (fz.data || []).filter((x) => x.kind === "route");
        for (const x of fzRoutes) simOf.set(x.id, Number(x.sim) || 0);
        // A matched area is usually a CONTAINER with no direct routes (see the expansion note
        // above), so reach its populated descendants the same way.
        const fzKids = await Promise.all(fzAreas.map((a) =>
          supabase.from("areas").select("id").filter("path", "cd", a.path).gt("route_count", 0)
            .order("route_count", { ascending: false }).limit(25)));
        const areaSim = new Map();
        fzKids.forEach((k, i) => {
          if (k.error) throw k.error;
          for (const a of k.data || []) if (!areaSim.has(a.id)) areaSim.set(a.id, Number(fzAreas[i].sim) || 0);
        });
        const [byFzRoute, byFzArea] = await Promise.all([
          fzRoutes.length ? supabase.from("routes").select(SEL).in("id", fzRoutes.map((x) => x.id)) : { data: [] },
          areaSim.size ? supabase.from("routes").select(SEL).in("area_id", [...areaSim.keys()]).limit(lim * 2) : { data: [] },
        ]);
        if (byFzRoute.error) throw byFzRoute.error;
        if (byFzArea.error) throw byFzArea.error;
        const sim = (r) => Math.max(simOf.get(r.id) || 0, areaSim.get(r.area_id) || 0);
        for (const r of [...(byFzRoute.data || []), ...(byFzArea.data || [])]) {
          if (seen.has(r.id)) continue;
          seen.add(r.id); merged.push(r);
        }
        merged.sort((a, b) => sim(b) - sim(a));
      }
      ...
      const namedAll = merged.filter((r) => {
        const an = searchCanon((r.areas && r.areas.name) || "");
        return an === needle || searchBare(an) === needle;
      });
...
export async function fetchArea(id) {
  ...
  // Offline this is the FIRST half of opening a search hit: the hit carries no `path`, so
  // without the full row the breadcrumb below has nothing to walk and jumpToArea bails
  // after it has already switched screens.
  return orOffline(async () => {
...
export async function fetchAreaBreadcrumb(area) {
  ...
  // The SECOND half. The dropped leading label is the root country, which is the one
  // ancestor `downloadStateOffline` never stores (it keeps descendants of the state only) —
  // so the ids that survive that slice are exactly the ones a downloaded state holds.
  return orOffline(async () => {
...
// `gradeSys` restricts a grade RANGE to rows whose grade_system says which scale grade_num is on
// (0196) — WI4 and 5.4 are both 4, so a range is only meaningful within one scale.
export function useSubtreeRoutes(areaId, { q, disc, gradeSys, minGrade, maxGrade, minStars, minPitches, minLengthM, maxLengthM, sortBy, page = 0, pageSize = 40 } = {}) {
  return useQuery({
    queryKey: ["subtree-routes", areaId, q || "", disc || "", gradeSys || "", minGrade ?? "", maxGrade ?? "", minStars ?? "", minPitches ?? "", minLengthM ?? "", maxLengthM ?? "", sortBy || "", page, pageSize],
    enabled: !!supabase && !!areaId,
    retry: false, // re-fires on every keystroke — fail fast instead of a slow, flickering retry storm
    queryFn: () => orOfflineExact(async () => {
      const { data, error } = await supabase.rpc("routes_in_subtree", { root_id: areaId, q: q || null, disc: disc || null, grade_sys: gradeSys || null, min_grade: minGrade ?? null, max_grade: maxGrade ?? null, min_stars: minStars ?? null, min_pitches: minPitches ?? null, min_length_m: minLengthM ?? null, max_length_m: maxLengthM ?? null, sort_by: sortBy || "name", lim: pageSize, off: page * pageSize });
      if (error) throw error;
      return data;
    }, () => offlineSubtreeRoutes(areaId, { q, disc, gradeSys, minGrade, maxGrade, minStars, minPitches, minLengthM, maxLengthM, sortBy, page, pageSize })),
  });
}

export function useSubtreeRouteCount(areaId, { q, disc, gradeSys, minGrade, maxGrade, minStars, minPitches, minLengthM, maxLengthM } = {}) {
  return useQuery({
    queryKey: ["subtree-route-count", areaId, q || "", disc || "", gradeSys || "", minGrade ?? "", maxGrade ?? "", minStars ?? "", minPitches ?? "", minLengthM ?? "", maxLengthM ?? ""],
    enabled: !!supabase && !!areaId,
    retry: false,
    queryFn: () => orOfflineExact(async () => {
      const { data, error } = await supabase.rpc("routes_in_subtree_count", { root_id: areaId, q: q || null, disc: disc || null, grade_sys: gradeSys || null, min_grade: minGrade ?? null, max_grade: maxGrade ?? null, min_stars: minStars ?? null, min_pitches: minPitches ?? null, min_length_m: minLengthM ?? null, max_length_m: maxLengthM ?? null });
      if (error) throw error;
      return data;
    }, () => offlineSubtreeRouteCount(areaId, { q, disc, gradeSys, minGrade, maxGrade, minStars, minPitches, minLengthM, maxLengthM })),
  });
}
...
export function useAreaSearch(rootId, q0, lim = 40) {
  const [q, typing] = useDebouncedTerm(q0 || "");
  return withPending(useQuery({
    queryKey: ["area-search", rootId, q || "", lim],
    enabled: !!supabase && !!rootId && !!(q || "").trim(),
    placeholderData: holdWhileTyping(q),
    retry: false,
    queryFn: () => orOfflineExact(async () => {
      const { data, error } = await supabase.rpc("areas_in_subtree", { root_id: rootId, q, lim });
      if (error) throw error;
      return data;
    }, () => offlineAreaSearch(rootId, q, lim)),
  }), typing);
}
...
// THE OFFLINE FALLBACK IS WHAT MAKES THE TRIP PACK REAL. Every route the pack, the wishlist
// and the logbook resolve comes through here, so with no signal this query failed and
// `dbRouteById` was empty — the pack rendered `routeById(id)` as undefined and dropped every
// row, i.e. a climber who had packed six climbs opened the app at the trailhead and saw an
// empty pack. Packing writes the row (lib/offline.js `packRouteOffline`); this is the read.
//
// Deliberately partial-tolerant, unlike the sibling readers: `offlineRoutesByIds` returns the
// ids it HAS, so six packed routes still resolve when a seventh wishlisted one was never
// stored. orOffline only substitutes on a non-empty result, so a query for ids none of which
// are on the device still surfaces the network error rather than reading as "no such routes".
export function useRoutesByIds(ids) {
  ...
  return useQuery({
    ...
    queryFn: () => orOffline(async () => {
      const { data, error } = await supabase
        .from("routes").select(ROUTE_AREA_EMBED)
        .in("id", ids);
      if (error) throw error;
      return data;
    }, () => offlineRoutesByIds(ids)),
  });
}
...
// With no signal a PACKED route answers from the copy saved with it (photos excluded — see
// lib/offline.js), stamped `_packedAt`; structuralSharing is off so that stamp reaches the page.
async function fetchRouteContributions(routeId) {
  const { data, error } = await supabase
...
export function useRouteContributions(routeId) {
  return useQuery({
    ...
    queryFn: () => orOfflineExact(() => fetchRouteContributions(routeId), () => offlinePackContribs(routeId)),
  });
}
...
// The areas where one climber tops the contributor leaderboard (0222), for the profile's
// Top Contributor badge. Keyed by uid; the old seed badge matched display names.
export function useUserTopContribAreas(uid) {
  return useQuery({
    queryKey: ["user-top-contrib-areas", uid],
    enabled: !!supabase && !!uid,
    queryFn: async () => {
      const { data, error } = await supabase.rpc("user_top_contributor_areas", { p_uid: uid });
      if (error) throw error;
      return data || [];
    },
  });
...
// ── Ranks (0194) ──
// The leaderboard is computed server-side: climb_logs is author-only under RLS, so no client
// can count anybody else's climbs. One row per rankable climber, every board's number in
// `stats.all` / `stats.backed` (backed = partner-confirmed or carrying a photo/GPS track).
// 0194's header states the counting rules; do not re-derive them here. `version` is a cheap
// signature of the caller's own logs, so logging a climb re-reads the board.
export function useLeaderboard({ scope, areaId, radiusMi, enabled, version }) {
  return useQuery({
    queryKey: ["leaderboard", scope, areaId || null, radiusMi || null, version || ""],
    enabled: !!supabase && !!enabled && (scope !== "area" || !!areaId),
    queryFn: async () => {
      const { data, error } = await supabase.rpc("leaderboard", {
        p_scope: scope, p_area_id: areaId || null, p_radius_mi: radiusMi || 120,
      });
      if (error) throw error;
      return data || [];
    },
...
// The "Top climbs" board: most climbers with a PUBLIC trip report on the route.
export function useTopClimbs({ scope, areaId, disc, radiusMi, enabled, version }) {
  return useQuery({
    queryKey: ["leaderboard-top-climbs", scope, areaId || null, disc || "all", radiusMi || null, version || ""],
    enabled: !!supabase && !!enabled && (scope !== "area" || !!areaId),
    queryFn: async () => {
      const { data, error } = await supabase.rpc("leaderboard_top_climbs", {
        p_scope: scope, p_area_id: areaId || null, p_disc: disc || "all", p_radius_mi: radiusMi || 120,
      });
      if (error) throw error;
      return data || [];
    },
...
// Climbs a connection tagged ME on ("Climbed with"), unanswered first. Confirming one is what
// makes it count fully on Ranks; denying one takes it off every board.
export function useLogTagsForMe(userId) {
  return useQuery({
    queryKey: ["log-tags-for-me", userId],
    enabled: !!supabase && !!userId,
    queryFn: async () => {
      const { data, error } = await supabase.rpc("log_tags_for_me");
      if (error) throw error;
      return data || [];
    },
  });
...
// The three trust-score inputs as the SERVER counted them (0203): climbs that count on Ranks,
// trip reports on those that say something, and catches the caught climber filed. The Profile's
// "what feeds your score" itemises from these rather than from raw rows, which the server no longer
// counts one-for-one. `version` refetches it when the caller's logs change.
export function useMyTrustCounts(userId, version) {
  return useQuery({
    queryKey: ["my-trust-counts", userId, version || ""],
    enabled: !!supabase && !!userId,
    queryFn: async () => {
      const { data, error } = await supabase.rpc("my_trust_counts");
      if (error) throw error;
...
// verdict: "confirmed" | "denied". Throws on refusal (not tagged, not a connection, blocked).
export async function respondToLogTag(logId, verdict) {
  if (!supabase) return null;
  const { data, error } = await supabase.rpc("respond_to_log_tag", { p_log_id: logId, p_verdict: verdict });
  if (error) throw error;
  return data;
}
...
// Several climbers' objectives in ONE read, keyed by user id — the partner rows (browse, near, search)
// score a match from them. `objectives` is public read (0031). A user with none is present as [], so
// the caller can tell "has no objectives" from "was not read" (undefined).
export function useObjectivesOfUsers(userIds) {
  const ids = [...new Set((userIds || []).filter(Boolean))].sort();
  return useQuery({
    queryKey: ["objectives-of-users", ids.join(",")],
    enabled: !!supabase && ids.length > 0,
    queryFn: async () => {
      const { data, error } = await supabase.from("objectives").select("user_id,route_id").in("user_id", ids).limit(5000);
      if (error) throw error;
      const by = {};
      ids.forEach(id => { by[id] = []; });
      (data || []).forEach(r => { (by[r.user_id] = by[r.user_id] || []).push(r.route_id); });
      return by;
    },
  });
}
...
// The crew's shared plan (0199). NOT through updateCrewRow: crews UPDATE is organiser-only, and
// the climber sharing a plan is often a member. The definer function admits the organiser or a
// confirmed member, for this one column only. `null` clears it.
export async function shareCrewItinerary(crewId, itinerary) {
  if (!supabase) return;
  const { error } = await supabase.rpc("share_crew_itinerary", { p_crew_id: crewId, p_itinerary: itinerary || null });
  if (error) throw error;
}
...
// What I am bringing to a crew, and the items I added to its list (0209). Through a definer rather
// than the row update: the membership policy refuses a confirmed member who joined by request, and
// that refusal is a 200 with zero rows, not an error. The function raises instead.
export async function setMyCrewGear(crewId, claims, extras) {
  if (!supabase) return;
  const { error } = await supabase.rpc("set_my_crew_gear", { p_crew_id: crewId, p_claims: claims || [], p_extras: extras || [] });
  if (error) throw error;
}
...
// `show_name` is on every list that becomes a CLIMBER OBJECT, because pubName() decides between
// the display name and the @handle from it — a select that omits it makes the column arrive
// undefined, which reads as false, which silently ignores the climber's own setting.
// availability / avail_week / hiking_speed_ft_hr (0209) are two of the four signals compat() scores;
// without them every real row counted as "not enough shared info". partners_near returns the same three.
const PARTNER_COLS = "id,name,username,avatar,bio,location,disciplines,sport_grade,trad_grade,boulder_grade,show_name,resume_public,availability,avail_week,hiking_speed_ft_hr";
...
// A climber's zip code, and "within N miles" of it (0189). The zip lives in `profile_zips`, which
// only its OWNER can read -- `profiles` is public-read, so a zip there would publish everyone's
// home area. Other climbers see a rounded distance, never the zip, and only through
// `partners_near`, which is the one door onto that table.
export function useMyZip(userId) {
  return useQuery({
    queryKey: ["my-zip", userId],
    enabled: !!supabase && !!userId,
    queryFn: async () => {
      const { data, error } = await supabase
        .from("profile_zips").select("zip, zip_centroids(lat,lng)").eq("user_id", userId).maybeSingle();
      if (error) throw error;
      if (!data) return null;
      const c = data.zip_centroids || {};
      return { zip: data.zip, lat: c.lat, lng: c.lng };
    },
  });
}

// Store (or clear, when blank) the caller's zip. A zip the Census has no centroid for is refused by
// the foreign key (23503) and reported as unrecognised rather than stored unplaceable. Re-reads the
// row it wrote: an RLS refusal is a 200 with nothing in it.
export async function saveMyZip(userId, zip) {
  if (!supabase || !userId) throw new Error("not signed in");
  const z = String(zip || "").trim();
  if (!z) {
    const { error } = await supabase.from("profile_zips").delete().eq("user_id", userId);
    if (error) throw error;
    return null;
  }
  if (!/^[0-9]{5}$/.test(z)) throw new Error("A zip code is five digits.");
  const { data, error } = await supabase.from("profile_zips")
    .upsert({ user_id: userId, zip: z, updated_at: new Date().toISOString() }).select("zip");
  if (error) {
    if (error.code === "23503") throw new Error("We don't recognise zip code " + z + ".");
    throw error;
  }
  if (!Array.isArray(data) || data.length !== 1) throw new Error("the zip was not saved");
  return data[0].zip;
}

// Discoverable climbers within `radiusMi` of an origin, nearest first, each with `dist_mi` rounded
// UP to the next 5 miles. The origin is used for this query and stored nowhere. The key rounds the
// origin to 2 decimals (~1 km) so a phone's jitter does not refetch on every reading.
export function usePartnersNear(origin, radiusMi, enabled) {
  const lat = origin && Number.isFinite(origin.lat) ? Math.round(origin.lat * 100) / 100 : null;
  const lng = origin && Number.isFinite(origin.lng) ? Math.round(origin.lng * 100) / 100 : null;
  return useQuery({
    queryKey: ["partners-near", lat, lng, radiusMi],
    enabled: !!supabase && !!enabled && lat != null && lng != null,
    queryFn: async () => {
      const { data, error } = await supabase.rpc("partners_near", { p_lat: lat, p_lng: lng, p_radius_mi: radiusMi, p_limit: 50 });
      if (error) throw error;
      return data || [];
    },
  });
...
// Discoverable climbers who saved one of `routeIds` as an objective, or any route filed under
// `areaId` (0211) -- My Objectives / By Route / By Area. Most shared objectives first. With an
// origin it is also a radius search on the same terms as partners_near, and climbers with no zip
// drop out; without one, dist_mi is null. Each row carries shared_route_ids (up to 20) and
// shared_count, so the card can say WHY this climber was found.
export function usePartnersByObjective(routeIds, areaId, origin, radiusMi, enabled) {
  const ids = Array.isArray(routeIds) && routeIds.length ? [...new Set(routeIds.map(String))].sort().slice(0, 500) : null;
  const area = areaId || null;
  const lat = origin && Number.isFinite(origin.lat) ? Math.round(origin.lat * 100) / 100 : null;
  const lng = origin && Number.isFinite(origin.lng) ? Math.round(origin.lng * 100) / 100 : null;
  return useQuery({
    queryKey: ["partners-by-objective", ids, area, lat, lng, lat != null ? radiusMi : null],
    enabled: !!supabase && !!enabled && (!!ids || !!area),
    queryFn: async () => {
      const args = { p_route_ids: ids, p_area_id: area, p_limit: 50 };
      if (lat != null && lng != null) Object.assign(args, { p_lat: lat, p_lng: lng, p_radius_mi: radiusMi });
      const { data, error } = await supabase.rpc("partners_by_objective", args);
      if (error) throw error;
      return data || [];
    },
  });
...
// Write the photo-strip visibility preference. Same shape as setDiscoverable above, and the
// same reason for re-reading what the server stored: an RLS-rejected PATCH is a 200 with an
// empty array, indistinguishable from success unless the row count is checked.
export async function setPhotosPublic(userId, value) {
  if (!supabase || !userId) throw new Error("not signed in");
  const { data, error } = await supabase
    .from("profiles").update({ photos_public: !!value }).eq("id", userId).select("id,photos_public");
  if (error) throw error;
  if (!Array.isArray(data) || data.length !== 1) throw new Error("the update matched no profile row");
  return data[0];
}
...
export function useProfilesByIds(ids) {
  ...
  return useQuery({
    ...
    // No signal: the reporters of a PACKED route's saved trip reports were stored with them, already
    // mapped by fetchProfilesByIds below, so their names render exactly as they do online.
    queryFn: () => orOfflineExact(() => fetchProfilesByIds(ids), () => offlinePackProfiles(ids)),
  });
}
export async function fetchProfilesByIds(ids) {
      const { data, error } = await supabase
        .from("profiles").select("id, name, avatar, show_name, username").in("id", ids);
      if (error) throw error;
      // NAME THE CLIMBER THE WAY THEY ASKED TO BE NAMED. 0175 made "Show my real name publicly"
      // real, and pubName() honours it — but it reads `showName`, and postgrest hands back
      // `show_name`, so every consumer of a RAW row silently answered "no" and fell through to a
      // handle DERIVED from the real name (username was not selected either). Consumers that skip
      // pubName and read `.name` did the opposite and published the real name whatever the switch
      // said — vouchRowsFrom did, and FullProfile builds ANOTHER climber's vouch rows with it.
      // Mapped ADDITIVELY: the snake_case fields stay, so nothing reading `p.show_name` breaks.
      return (data || []).map((p) => ({ ...p, showName: !!p.show_name }));
}
...
const CRAG_DISC = new Set(["trad", "sport", "toprope", "bouldering"]);
...
// A photo description (WCAG 1.1.1): trimmed, capped at the column's 300, and `null` when blank,
// so an undescribed photo stores nothing rather than an empty string that reads like an answer.
function cleanAlt(alt) {
  const d = String(alt == null ? "" : alt).trim();
  return d ? d.slice(0, 300) : null;
}

export async function uploadTopoPhoto(areaId, file, alt) {
  if (!supabase) return null;
  ...
  // The location the camera saved inside the photo is removed first: this bucket is public.
  file = await withoutLocation(file);
  const path = `${uid}/${crypto.randomUUID()}-${file.name}`;
  const { error: upErr } = await supabase.storage.from("topo-photos").upload(path, file);
  if (upErr) throw upErr;
  const { data, error } = await supabase
    .from("topos").insert({ area_id: areaId, storage_path: path, alt: cleanAlt(alt) }).select().single();
  if (error) throw error;
  return data;
}
...
// Take one photo back OFF the strip.
//
// The counterpart to uploadProfilePhoto + saveProfilePhotos, and the thing a climber had no
// way to do at all: PhotoStrip has offered `onAdd` and nothing to undo it, so the only thing
// you could do about a photo you regretted was HIDE it -- and 0174 is explicit that
// `photos_public` is surfacing only, not access control, so hiding never stopped the row being
// served to anyone holding the anon key.
//
// ORDER MATTERS, and it is deleteRoutePhoto's order for deleteRoutePhoto's reason: drop the
// REFERENCE first, then the storage object. Delete the object first and a surviving array
// entry points at a file that is gone, which is a broken image on the profile. This order can
// only ever leave an orphan object in a bucket nobody browses, which is invisible.
//
// Throws rather than resolving on a failed write, so a caller's "Photo removed" stays behind a
// write that landed. Returns the new array, so the caller renders what the database now holds
// rather than what it assumed it would hold.
// Describe a topo photo after it was uploaded. `topos own update` (0026) scopes this to the
// uploader, and an RLS refusal matches zero rows, which .single() surfaces as an error — so a
// caller's "Description saved" can only follow a write that landed.
export async function updateTopoAlt(topoId, alt) {
  if (!supabase) return null;
  if (!topoId) throw new Error("Nothing to describe.");
  const { data, error } = await supabase
    .from("topos").update({ alt: cleanAlt(alt) }).eq("id", topoId).select("id, alt").single();
  if (error) throw error;
  return data;
}

// Persist the descriptions of a climber's profile photos, and ONLY that column — the same
// one-column-in rule as saveProfilePhotos, for the same reason (check:profile-edit-gate).
// Entries for photos no longer on the strip are pruned here, so the map cannot grow stale.
export async function saveProfilePhotoAlts(userId, alts, photos) {
  if (!supabase) return null;
  if (!userId) throw new Error("Sign in to describe your profile photos.");
  const keep = new Set(Array.isArray(photos) ? photos : []);
  const next = {};
  Object.keys(alts || {}).forEach((u) => { const a = cleanAlt(alts[u]); if (a && keep.has(u)) next[u] = a; });
  const { data, error } = await supabase
    .from("profiles").update({ photo_alts: Object.keys(next).length ? next : null }).eq("id", userId).select("id, photo_alts").single();
  if (error) throw error;
  return data;
}

export async function removeProfilePhoto(userId, url, photos) {
  if (!supabase) return null;
  if (!userId) throw new Error("Sign in to change your profile photos.");
  const before = Array.isArray(photos) ? photos : [];
  const next = before.filter((p) => p !== url);
  // A no-op must not report success -- the swallowed-write-failure shape this repo keeps
  // finding. If the strip and this argument disagree about what is on the profile, say so,
  // rather than writing the array back unchanged under a "Photo removed" message.
  if (next.length === before.length) throw new Error("That photo is not on your profile any more — reload and try again.");
  await saveProfilePhotos(userId, next);
  const key = photoStorageKey(url);
  // Removal is by VALUE, so if a URL somehow appeared twice both entries have just gone and
  // nothing on the profile still points at the object. A "is anything still referencing it?"
  // test here could never be false, and a condition that cannot fail reads as a safety check
  // while being none -- so the reason is written down instead.
  //
  // `key` is null for a `blob:` preview or a seed https:// photo, neither of which is an object
  // of ours to delete. Reading null as a key would ask storage to remove the whole URL.
  if (key) {
...
// Take the AVATAR back off, and the reason this exists is the same one removeProfilePhoto
// records: the editor offered "Change photo" and nothing to undo it, so a climber who regretted
// their profile picture could replace it and never clear it. The in-app privacy sheet meanwhile
// says you can "edit or clear anything from your profile and settings at any time" — and
// check:policy-claims exists precisely because a legal surface must not claim a control the app
// lacks. This is the control that sentence was describing.
//
// IT TAKES A userId RATHER THAN READING THE SESSION, and that choice was made FOR TESTABILITY.
// uploadProfilePhoto asks `auth.getSession()` itself, and mirroring that read cleanly — this is
// its counterpart, after all. It also makes the function unexecutable by check:photo-removal,
// whose whole method is running the real removal against a stubbed transport: with no session in
// node it would throw "Sign in" before touching anything, so every property below could only be
// asserted as SOURCE. Its sibling removeProfilePhoto(userId, …) is executable precisely because
// it is handed the id. One prop on EditProfileScreen buys the same, and a prop is cheap next to a
// guard that can only read the code it is guarding.
//
// ONE COLUMN IN, ONE COLUMN OUT — deliberately NOT routed through `saveProfile`, which PATCHes
// whatever object it is handed. That is saveProfilePhotos' stated reasoning and it applies here
// unchanged: handing that write a partial object is the shape check:profile-edit-gate exists for.
//
// ORDER MATTERS and it is removeProfilePhoto's order: drop the REFERENCE first, then the storage
// object. Deleting the object first leaves `profiles.avatar` pointing at a file that is gone,
// which is a broken image everywhere a climber's face appears. This order can only ever leave an
// orphan object in a bucket nobody browses.
//
// Throws rather than resolving on a failed write, so a caller's "Photo removed" stays behind a
// write that landed. `.single()` errors on zero rows, so an RLS refusal — which rejects by
// matching nothing rather than by erroring — surfaces here instead of reading as success.
// The climber's pinned states (0187). ACCOUNT-level, unlike the offline catalog for each state,
// which lives in the device's IndexedDB — so a pin made on one browser shows up on the next one
// with a Download button beside it instead of not at all.
//
// ONE COLUMN IN, ONE COLUMN OUT, never through saveProfile, which PATCHes whatever it is handed
// (the check:profile-edit-gate shape). Throws on a refusal — `.single()` errors on zero rows, so
// an RLS rejection cannot read as a saved pin.
export async function savePinnedStates(userId, names) {
  if (!supabase) return null;
  if (!userId) throw new Error("Sign in to pin a state to your account.");
  const clean = [...new Set((names || []).filter((n) => typeof n === "string" && n))];
  const { error } = await supabase
    .from("profiles").update({ pinned_states: clean }).eq("id", userId).select("id").single();
  if (error) throw error;
  return clean;
}
...
export async function removeProfileAvatar(userId, url) {
  ...
  if (error) throw error;
  // null for a `blob:` preview or a seed https:// avatar — neither is an object of ours, and
  // reading null as a key would ask storage to remove the whole URL. Avatars go into the same
  // `topo-photos` bucket as the strip, so the same helper resolves them.
  const key = photoStorageKey(url);
  if (key) {
    // NOT rethrown: the column is already null, so the photo has left every screen, and telling
    // the climber their removal failed would be false. removeProfilePhoto's reasoning verbatim.
    await supabase.storage.from("topo-photos").remove([key]).catch(() => {});
  }
  return true;
}
...
export async function submitRoutePhoto(routeId, file, alt) {
  if (!supabase) return null;
  ...
  const url = topoPhotoUrl(path);
  if (!url) throw new Error("The photo uploaded but its address could not be resolved.");
  const a = cleanAlt(alt);
  return submitContribution({ route_id: routeId, kind: "photo", value: a ? { url, alt: a } : { url } });
}
...
// The storage object key behind one of our public photo URLs. Everything after
// `/topo-photos/` IS the object key -- getPublicUrl only appends -- so this is the reversal of
// a known transformation rather than a guess at a URL shape.
//
// Returns null for anything that is not one of ours, which is what a `blob:` preview and a
// seed https:// photo both look like. A caller must read null as "there is no object of ours
// to remove", never as a key it can pass on.
export function photoStorageKey(url) {
...
// fa_ascent/developed/sun_vote/sun_note/itinerary: RouteDetail has always MAPPED all five into
// the report another climber opens, and nothing ever SELECTED them -- so the FA panel could only
// credit yourself and a logged day-by-day plan was never offered to anyone. gpx_track stays out:
// up to 500 points per row, on a read of 60 rows.
const TRIP_REPORT_COLS = "id, user_id, stars, cond_tags, date_climbed, discipline, tick_type, notes, photos, beta, gear_beta, outcome_reasons, outcome_note, approach_minutes, climb_minutes, descent_minutes, car_to_car_minutes, snow_condition, freezing_level_ft, water_level, bug_pressure, trail_condition, party_size, protection_quality, anchor_quality, crowd_level, fa_ascent, developed, sun_vote, sun_note, itinerary, created_at";
...
async function fetchRouteTripReports(routeId, limit) {
  const run = (cols) => supabase
    ...
    .order("date_climbed", { ascending: false })
    .limit(limit);
  let { data, error } = await run(TRIP_REPORT_COLS + ", temp_f, snow_depth, seepage, mud, road_note, rappel_count, rappel_longest_m, rappel_rope");
  if (isUnknownColumn(error)) ({ data, error } = await run(TRIP_REPORT_COLS));
  if (error) throw error;
  return data || [];
}
// The Today tab's "Recent friend activity": your connections' shared climbs, newest first, in ONE
// read. It was built from the seed ROUTES' `activity`, so a real account's friends never appeared
// in it. Same columns and the same visibility filter as the route page, for the same reasons —
// 0081's policy decides what a friend's row shows you; the filter keeps it to what they shared.
async function fetchSharedLogs(col, ids, limit, sinceDays) {
  const run = (cols) => {
    let q = supabase
      .from("climb_logs")
      .select("route_id, " + cols)
      .in(col, ids)
      .in("trip_report_visibility", ["public", "crew"]);
    if (sinceDays) q = q.gte("date_climbed", new Date(Date.now() - sinceDays * 86400000).toISOString().slice(0, 10));
    return q.order("date_climbed", { ascending: false }).limit(limit);
  };
  let { data, error } = await run(TRIP_REPORT_COLS + ", temp_f, snow_depth, seepage, mud, road_note, rappel_count, rappel_longest_m, rappel_rope");
  if (isUnknownColumn(error)) ({ data, error } = await run(TRIP_REPORT_COLS));
  if (error) throw error;
...
export function useFriendsActivity(userIds, limit = 40) {
  const ids = [...new Set((userIds || []).filter(Boolean))].sort();
  return useQuery({
    queryKey: ["friends-activity", ids, limit],
    enabled: !!supabase && ids.length > 0,
    queryFn: () => fetchSharedLogs("user_id", ids, limit),
  });
}
// ...and the Logbook's "Recent condition reports": the last 90 days of shared reports on the
// routes in your objectives. It read each route's seed `activity`, which a catalog route never has.
export function useRecentReportsOnRoutes(routeIds, limit = 60) {
  const ids = [...new Set((routeIds || []).filter(Boolean))].sort();
  return useQuery({
    queryKey: ["recent-reports-on-routes", ids, limit],
    enabled: !!supabase && ids.length > 0,
    queryFn: () => fetchSharedLogs("route_id", ids, limit, 90),
  });
}
// No signal: a PACKED route answers with the reports saved when it was packed or last refreshed,
// stamped `_packedAt` (structuralSharing off so the stamp reaches the page, which says how old).
export function useRouteTripReports(routeId, limit = 60) {
  return useQuery({
    queryKey: ["route-trip-reports", routeId, limit],
    enabled: !!supabase && !!routeId,
    structuralSharing: false,
    queryFn: () => orOfflineExact(() => fetchRouteTripReports(routeId, limit), () => offlinePackReports(routeId)),
  });
}

// ── packing a route: the row AND what the page needs beside it ──────────────
// packRouteOffline stores the route row; this adds the snapshot (agreed corrections, the latest
// reports and their authors' public names). The pack itself is the promise the button makes, so a
// failed SNAPSHOT does not fail it — the result says so and the caller words its toast from it.
// Also the REFRESH: a repack keeps packedAt and replaces the snapshot with a fresh one.
export const PACK_REPORT_LIMIT = 20;
export async function packRouteWithSnapshot(routeId, opts) {
  const res = await packRouteOffline(routeId, opts);
  if (res.seed || !supabase) return { ...res, snapshot: false };
  try {
    const [contribs, reports] = await Promise.all([fetchRouteContributions(routeId), fetchRouteTripReports(routeId, PACK_REPORT_LIMIT)]);
    const ids = [...new Set((reports || []).map((r) => r.user_id).filter(Boolean))];
    const profiles = ids.length ? await fetchProfilesByIds(ids) : [];
    await savePackSnapshot(routeId, { contribs: (contribs || []).filter((c) => c.kind !== "photo"), reports: reports || [], profiles });
    return { ...res, snapshot: true };
  } catch (e) {
    return { ...res, snapshot: false };
  }
}

// When each packed route was chosen and last refreshed — the ages the pack's rows print. Device-only,
// so it runs with or without a backend; callers refetch it after a pack, refresh or forecast save.
export function usePackMeta() {
  return useQuery({ queryKey: ["pack-meta"], queryFn: packMeta, staleTime: 0 });
}
...
// road_note (0205) rides the same degrade: the road-conditions section of the log form writes it,
// and a database without the column must still save the rest of the log.
// rappel_count / rappel_longest_m / rappel_rope (0207) ride it too: the log form's RAPPELS answers.
const LOG_COLS_0121 = ["temp_f", "snow_depth", "seepage", "mud", "road_note", "rappel_count", "rappel_longest_m", "rappel_rope"];
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: profs, error: pErr } = await supabase
        .from("profiles").select("id, name, username, avatar, location, show_name, resume_public").in("id", others);
      if (pErr) throw pErr;
      const byId = {};
...
// Mutual friends. `mutualIds()` in core took no arguments and returned `[]`, so every consumer
// rendered `mutualCount(...) ? control : null` and the feature was invisible — including the
// Mutual friends sheet, which no entry point in the app could open.
//
// IT HAS TO BE AN RPC, and that is not a preference. `0087` restricts reading `connections` to
// the two people on the row ("A connection is not public"), so the other climber's edges are
// unreadable from here by construction — no client-side query can compute an intersection with
// them. `0182` supplies a SECURITY DEFINER function; see its header for what that discloses and
// why blocks are honoured inside it.
//
// ONE CALL FOR MANY CLIMBERS, because the friends list renders several rows at once and a
// per-row hook would be N round trips for one screen. The array is capped at 64 in SQL, so the
// caller slices rather than discovering the cap as a silently short answer.
// "I'm at the base" check-ins (0188). The write goes ONLY through check_in_at_route_base(), which
// re-applies the proximity rules server-side; a refusal comes back as {ok:false, reason} rather
// than an error, because "you are 2 km from this crag" is an answer, not a failure. A real failure
// (network, auth) throws, so no caller can report a check-in that did not land.
export async function checkInAtRouteBase(routeId, fix) {
  if (!supabase) throw new Error("no database");
  const { data, error } = await supabase.rpc("check_in_at_route_base", {
    p_route_id: routeId, p_lat: fix.lat, p_lng: fix.lng,
    p_accuracy_m: fix.accuracy, p_samples: fix.samples || 1, p_fixed_at: fix.fixedAt,
  });
  if (error) throw error;
  if (!data || typeof data.ok !== "boolean") throw new Error("unexpected check-in answer");
  return data;
}

// Withdraw your own check-in. Selects the row back so a delete RLS refused (zero rows, 200) is not
// read as success.
export async function withdrawBaseCheckin(userId, routeId) {
  if (!supabase) throw new Error("no database");
  const { data, error } = await supabase.from("route_base_checkins").delete()
    .eq("route_id", routeId).eq("user_id", userId).select("id");
  if (error) throw error;
  if (!data || !data.length) throw new Error("nothing was removed");
  return data.length;
}

// Unattributed points: {lat, lng, mine}. No identity and no time ever leave the database.
export function useRouteBaseCheckins(routeId) {
  return useQuery({
    queryKey: ["route-base-checkins", routeId],
    enabled: !!supabase && !!routeId,
    staleTime: 60000,
    queryFn: async () => {
      const { data, error } = await supabase.rpc("route_base_checkin_points", { p_route_id: routeId });
      if (error) throw error;
      return data || [];
    },
  });
...
export function useMutualConnections(userId, otherIds) {
  // The key is the SORTED id list: the same set asked in a different order is the same
  // question, and keying on the raw array would refetch on every render that rebuilt it.
  const ids = [...new Set((otherIds || []).filter(Boolean))].sort();
  const key = ids.join(",");
  return useQuery({
    queryKey: ["mutual-connections", userId, key],
    enabled: !!supabase && !!userId && ids.length > 0,
    queryFn: async () => {
      const { data, error } = await supabase.rpc("mutual_connections", { others: ids.slice(0, 64) });
      if (error) throw error;
      const map = {};
      (data || []).forEach((r) => {
        if (!r || !r.other_id || !r.mutual_id) return;
        (map[r.other_id] = map[r.other_id] || []).push(r.mutual_id);
      });
      return map;
    },
  });
...
export function useMyBlocked(userId) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: profs, error: pErr } = await supabase
        .from("profiles").select("id, name, username, avatar, show_name").in("id", rows.map((r) => r.blocked));
      if (pErr) throw pErr;
      const byId = {};
      (profs || []).forEach((p) => { byId[p.id] = p; });
...
export async function fetchCrewMessages(crewId, limit = 100, offset = 0) {
  ...
  const { data, error } = await supabase
    .from("crews_messages").select("*, user:user_id(id,name,avatar,username,show_name)")
    .eq("crew_id", crewId)
    .order("created_at", { ascending: true })
...
export function useDirectMessages(userId, otherUserId, limit = 100, offset = 0) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("messages").select("*, sender:sender_id(id,name,avatar,username,show_name)")
  ...
  useEffect(() => {
    ...
    channel.on(
      ...
      async (payload) => {
        ...
        if (msg.user_id) {
          const { data: user } = await supabase.from("profiles").select("id,name,avatar,username,show_name").eq("id", msg.user_id).single();
    ...
    channel.on(
      ...
      async (payload) => {
        ...
        if ((msg.sender_id === userId && msg.recipient_id === partnerId) || (msg.sender_id === partnerId && msg.recipient_id === userId)) {
          // Fetch full user profile for the sender
          if (msg.sender_id) {
            const { data: sender } = await supabase.from("profiles").select("id,name,avatar,username,show_name").eq("id", msg.sender_id).single();
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: mem, error: mErr } = await supabase
        .from("group_members").select("group_id, user_id, role, status")
      ...
      return groups.map((g) => {
        ...
        // 0178 SPLIT "HAS A ROW" FROM "IS A MEMBER", and every count here means the second.
        // A `pending` row is a request and an `invited` row is somebody else's offer; treating
        // either as membership would list a group you merely asked about among your own, and
        // would count a requester against the group's capacity.
        ...
        return {
          ...
          ownerId: g.created_by, memberIds: active.map((m) => m.user_id),
          moderatorIds: active.filter((m) => m.role !== "member").map((m) => m.user_id),
          joined: !!mine && mine.status === "active",
          myStatus: mine ? mine.status : null,
          myRole: mine && mine.status === "active" ? mine.role : null, _db: true,
        };
      });
    },
  });
...
// REQUESTS WAITING ON YOU, for every group you manage. Before 0178 this list was `groupReqs`, a
// useState seeded only by DEMO_FILLERS — so a real organiser had nothing to approve and a real
// climber's request reached nobody.
//
// RLS does the scoping rather than a trusted client filter: reading `group_members` needs
// is_group_member() or a public group, and the manager test is re-applied by the approve policy
// anyway, so a caller who tampered with the group list would be refused at the write.
export function useGroupJoinRequests(userId) {
  return useQuery({
    queryKey: ["group-join-requests", userId],
    enabled: !!supabase && !!userId,
    queryFn: async () => {
      const { data: mine, error: mErr } = await supabase
        .from("group_members").select("group_id, role")
        .eq("user_id", userId).eq("status", "active").in("role", ["owner", "moderator"]);
      if (mErr) throw mErr;
      const ids = (mine || []).map((r) => r.group_id);
      if (!ids.length) return [];
      const { data, error } = await supabase
        .from("group_members").select("group_id, user_id, created_at:joined_at")
        .in("group_id", ids).eq("status", "pending");
      if (error) throw error;
      return data || [];
    },
  });
...
// Invitations waiting on YOU. `group_members read own` (0178) is what makes this readable at all:
// an invitation to a PRIVATE group sits on a roster the recipient cannot see, so without a
// row-level read of their own row the invite would be invisible to the one person it is for.
export function useMyGroupInvites(userId) {
  return useQuery({
    queryKey: ["my-group-invites", userId],
    enabled: !!supabase && !!userId,
    queryFn: async () => {
      const { data, error } = await supabase
        .from("group_members").select("group_id").eq("user_id", userId).eq("status", "invited");
      if (error) throw error;
      const rows = data || [];
      if (!rows.length) return [];
      // is_group_invited() (0178) is what lets these names resolve; without it an invitation
      // would name a group the recipient cannot read.
      const { data: gs, error: gErr } = await supabase
        .from("groups").select("id, name, visibility, policy").in("id", rows.map((r) => r.group_id));
      if (gErr) throw gErr;
      const byId = Object.fromEntries((gs || []).map((g) => [g.id, g]));
      return rows.map((r) => ({ groupId: r.group_id, group: byId[r.group_id] || null }));
    },
  });
}

// THE FOUR DOORS 0178 OPENED, so that tightening the gate did not simply forbid joining.
//
// Which one a caller reaches for is decided by the GROUP, not by the climber: an open public
// group takes `joinGroupRow`, a public gated one takes a request, and a private one takes an
// invitation from a manager. `groupJoinPath()` in the app makes that choice in one place so the
// two join handlers cannot disagree about it.
//
// Each of these is one row and one status transition, and every one is enforced by RLS as well —
// so a client that reaches for the wrong door is refused rather than quietly doing the other
// thing. `.select()` then a length check, because PostgREST answers a zero-row PATCH 200: a
// refusal and a success are the same status code, which this repo has now recorded three times.
export async function requestToJoinGroup(groupId, userId) {
  if (!supabase) return null;
  const { data, error } = await supabase
    .from("group_members").insert({ group_id: groupId, user_id: userId, status: "pending" }).select().single();
  if (error && error.code !== "23505") throw error;   // 23505: already asked, which is not a failure
  return data || null;
}
...
export async function inviteToGroup(groupId, userId) {
  if (!supabase) return null;
  const { data, error } = await supabase
    .from("group_members").insert({ group_id: groupId, user_id: userId, status: "invited" }).select().single();
  if (error && error.code !== "23505") throw error;
  return data || null;
}

// Accepting and approving are the same UPDATE from the database's point of view and two
// different permissions: the first is yours over your own invitation, the second a manager's over
// somebody else's request. Kept as two functions so a call site names which one it means.
export async function acceptGroupInvite(groupId, userId) {
  if (!supabase) return null;
  const { data, error } = await supabase
    .from("group_members").update({ status: "active" })
    .eq("group_id", groupId).eq("user_id", userId).eq("status", "invited")
    .select("group_id, user_id, status");
  if (error) throw error;
  if (!data || data.length !== 1) throw new Error("that invitation is no longer open");
  return data[0];
}
...
// Declining a request and withdrawing your own are the same DELETE, and 0090's delete policy
// already covers both (your own row, or a manager on any non-owner row) — so no new policy was
// needed for either. `removeGroupMember` is that DELETE and is reused rather than duplicated.
...
//
// Only own lists are fetched here. Other climbers' `shared` lists are read by usePublicLists
// (their profile) and useSharedList (a `?list=` link), below.
export function useMyLists(userId) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("user_lists").select("id, name, icon, description, route_ids, shared, created_at")
        .eq("user_id", userId).order("created_at", { ascending: true });
      if (error) throw error;
      return (data || []).map((l) => ({
        _dbId: l.id, id: l.id, name: l.name, icon: l.icon || "", description: l.description || "",
...
// Another climber's PUBLIC lists, for their profile. The 0092 select policy already admits any
// `shared = true` row; the `.eq("shared", true)` is still stated here, because a climber viewing
// THEIR OWN profile would otherwise be shown their private lists as if others could see them.
export function usePublicLists(userId) {
  return useQuery({
    queryKey: ["public-lists", userId],
    enabled: !!supabase && !!userId,
    queryFn: async () => {
      const { data, error } = await supabase
        .from("user_lists").select("id, user_id, name, description, route_ids")
        .eq("user_id", userId).eq("shared", true).order("created_at", { ascending: true });
      if (error) throw error;
      return (data || []).map((l) => ({ id: l.id, userId: l.user_id, name: l.name, description: l.description || "", routeIds: l.route_ids || [] }));
    },
  });
}

// One list, opened from a shared link (`?list=<id>`). RLS answers a PRIVATE or deleted list with
// zero rows and a 200, so `null` here means "not public, or gone" — distinct from a thrown error,
// which is a failed read and must not be shown as "this list is private".
export function useSharedList(listId) {
  return useQuery({
    queryKey: ["shared-list", listId],
    enabled: !!supabase && !!listId,
    queryFn: async () => {
      const { data, error } = await supabase
        .from("user_lists").select("id, user_id, name, description, route_ids, shared")
        .eq("id", listId).maybeSingle();
      if (error) throw error;
      if (!data) return null;
      return { id: data.id, userId: data.user_id, name: data.name, description: data.description || "", routeIds: data.route_ids || [], shared: !!data.shared };
    },
  });
}

// ── my itineraries (migration 0199) ──
// Before 0199 a climber's own day-by-day plan lived in one useState and a refresh erased it,
// under a toast that said "Saved your plan". One row per (user, route); owner-only RLS.
export function useMyItineraries(userId) {
  return useQuery({
    queryKey: ["my-itineraries", userId],
    enabled: !!supabase && !!userId,
    // Mirrored to this device on every good read (keyed by account), and served from there when the
    // read fails — a plan made for a packed route must still be on the Plan tab at the trailhead.
    queryFn: () => orOfflineExact(async () => {
      const { data, error } = await supabase.from("user_itineraries").select("route_id, itinerary").eq("user_id", userId);
      if (error) throw error;
      const out = {};
      (data || []).forEach((r) => { if (r.itinerary && Array.isArray(r.itinerary.days) && r.itinerary.days.length) out[r.route_id] = r.itinerary; });
      cacheItineraries(userId, out).catch(() => {});
      return out;
    }, () => cachedItineraries(userId)),
  });
}
// .select() so an RLS refusal (200 + zero rows) surfaces as a failure, not a silent success.
export async function saveItineraryRow(userId, routeId, itinerary) {
  if (!supabase) return null;
  const { data, error } = await supabase.from("user_itineraries")
    .upsert({ user_id: userId, route_id: routeId, itinerary, updated_at: new Date().toISOString() }, { onConflict: "user_id,route_id" })
    .select("route_id");
  if (error) throw error;
  if (!data || !data.length) throw new Error("That didn’t save — try again.");
  return data;
}
