...
export function useRouteSearch(q, lim = 8) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const [byPrefix, byName, areaHit] = await Promise.all([
      ...
      // contributing routes is not worth a fourth round trip on every keystroke.
      ...
      for (const k of kids) {
        ...
        for (const a of k.data || []) if (!ids.includes(a.id)) ids.push(a.id);
      }
      let byArea = { data: [] };
      if (ids.length) {
