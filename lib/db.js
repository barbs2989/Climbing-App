...
export async function updateCrewMemberStatus(crewId, userId, status) {
  if (!supabase) return;
  const { error } = await supabase.from("crew_members").update({ status }).eq("crew_id", crewId).eq("user_id", userId);
  if (error) throw error;
}
...
// Ordered newest-first so a growing offset walks further into the past, then
// reversed to chronological for the UI. Returns [] on failure rather than throwing,
// so a failed page-load can never blank the chat.
export async function fetchOlderCrewMessages(crewId, limit = 50, offset = 0) {
  if (!supabase || !crewId) return [];
  const { data, error } = await supabase
    .from("crews_messages").select("*, user:user_id(id,name,avatar)")
    .eq("crew_id", crewId)
...
export async function fetchOlderDirectMessages(userId, otherUserId, limit = 50, offset = 0) {
