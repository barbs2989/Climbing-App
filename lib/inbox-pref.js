...
//
// This was believed to be the LAST setting on the Settings screen that did not survive a page
// load — found by deriving the control list off the screen rather than from a list somebody
// typed, which is what showed the screen has twelve controls and the census covered six.
//
// IT WAS NOT THE LAST, AND THE DERIVATION HAD ITS OWN HOLE. The census reads controls one JSX
// site at a time and keys on a string-literal `aria-label`; the four switches under Notifications
// are rendered from a `.map` with a computed one, so the whole GROUP was dropped silently and the
// run reported "0 volatile" with four volatile switches on the screen. Fixing the POPULATION by
// deriving it fixed the list somebody had typed and left a blind spot of its own — a control
// group rendered from a loop. See lib/notif-pref.js, which is the fourth of these modules.
//
