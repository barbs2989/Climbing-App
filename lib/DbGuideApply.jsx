    ...
    <div style={{ position: "fixed", inset: 0, background: C.bg, zIndex: 1100, overflowY: "auto" }}>
      ...
      <div style={{ maxWidth: 560, margin: "0 auto", padding: "14px 16px 44px" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
          ...
          <select aria-label="Maximum party size" value={groupMax} onChange={e => setGroupMax(e.target.value)} style={{ ...inp, maxWidth: 180 }}>{["1", "2", "3", "4", "6"].map(n => <option key={n} value={n}>{"Up to " + n}</option>)}</select>
        </div>
        ...
        <select aria-label="Certification track" value={certTrack} onChange={e => setCertTrack(e.target.value)} style={inp}>{tracks.map(t => <option key={t} value={t}>{CERT_TRACK_LABELS[t]}</option>)}</select>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <select aria-label="Additional certification type" value={ccType} onChange={e => setCcType(e.target.value)} style={inp}>{CROSS_CUTTING.map(x => <option key={x[0]} value={x[0]}>{x[1]}</option>)}</select>
