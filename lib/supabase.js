...
//
// Written in terms of the three BUILD-TIME env strings rather than `!!supabase`, which is the
// same value (the client exists exactly when url && key) but is a runtime object the bundler
// cannot see through. As env strings it folds to a literal in a production build, and every
// `USE_DB ? <DbX/> : <SeedX/>` in the app then drops its dead half from the bundle — the seed
// components (check:seed-only-surfaces lists them) were being shipped to every visitor and
// rendered for none.
export const USE_DB = import.meta.env.VITE_USE_DB === "true" && !!url && !!key;
