...
//
// A POSITIONAL WORD IS NOT NOISE — IT IS USUALLY THE WHOLE DISAMBIGUATOR, and eight of them were
// stripped here. "Upper Boulder Field" and "Lower Boulder Field" on wa_bedal_peak_standard are two
// Hazard pins 435 m apart, and after stripping they were ONE key and one pin; the same for "Upper
// cliff band" / "Lower cliff band" on wa_davis_peak_nc_southwest, 184 m apart. A climber saw one
// hazard marker where the route records two.
//
// The defect this list exists for — "Forbidden Peak summit" vs "Summit" — is not handled here at
// all: a summit is a SINGLETON and merges on TYPE, before any name is compared. So the generic
// NOUNS still earn their place and the positional adjectives (upper/lower/west/east/north/south/
// true/main) do not. Measured across all WA routes: removing them keeps two genuine pins and costs
// no legitimate merge.
const STOP = /\b(the|mount|mt|peak|spire|summit|standard|route)\b/g;
...
//
// TRAILHEAD IS NOT ONE OF THEM, and it was here — merging two trailhead pins on TYPE ALONE,
// whatever their names or coordinates said. A route really does have one summit; it does not
// have one trailhead. CLAUDE.md names four WA peaks with two GENUINE approaches (Carru, Remmel,
// Howard, Stuart's North Ridge) and warns twice not to sweep them to one.
//
// The merge was worse than a drop, because mergePair() keeps the first pin's COORDINATE and the
// longer NAME. Remmel's Southeast Slope stores Thirtymile (48.8228,-120.0197) and Andrews Creek
// (48.7837,-120.1086), 7,829 m apart, and rendered as "Andrews Creek Trailhead" AT THIRTYMILE —
// one trailhead's label on the other's position, on the pin that drives the Directions button.
//
// Removing it costs nothing a trailhead needs: two pins for the same start still merge on
// sameSpot() (~30 m) or on nameKey(), which is how every other non-singleton type is handled.
// Measured across all 8,365 WA routes: exactly ONE route's rendered list changes, and it is that
// one. Do not put `trailhead` back — the class is one today and the rule is wrong in general.
const SINGLETON = /^(summit|topout)$/i;
