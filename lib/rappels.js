...
export function rappelRopeNeed(route) {
  ...
  return { max, needs: max > SINGLE_70_REACH ? "double" : max > SINGLE_60_REACH ? "single70" : "single60" };
}
...
export function rappelHeaderLabel(route) {
  ...
  if (need && need.needs === "double") {
    return `RAPPELS · ${documented} station${documented !== 1 ? "s" : ""} · two ropes (longest ${need.max} m)`;
  }
