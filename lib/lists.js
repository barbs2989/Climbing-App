...
//     Colorado 14ers      48 of 53      Desert towers       27 of 27
//
// Four of the six are COMPLETE. Colorado's remaining FIVE are all known and none is a plain
// import (0143 adjudicated every one):
//   * Crestone Peak, Crestone Needle, Mount Wilson and Maroon Peak already live inside COMBINED
//     areas — co_crestones_the holds "The Prow (Kit Carson)", co_maroon_bells holds routes that
//     span both Maroon summits, and co_el_diente_peak_mount_wilson is named for two fourteeners
//     while holding a single El Diente route. Splitting those means deciding where existing
//     routes go, which is a different operation from adding an absent peak.
//   * Mount Bross is EXCLUDED ON PURPOSE. Its area exists holding 0 routes, so it looks exactly
//     like the routeless peaks 0143 fixed — but its SUMMIT IS STILL LEGALLY CLOSED (privately
//     owned; the March 2026 USFS acquisition covered the approach corridor, not the top), so
//     standing on it is trespassing. A tick list invites people to climb the thing, the same
//     rule that keeps Eagle Rock Spire off Desert Towers. Colorado tops out at 52 ACHIEVABLE.
//
