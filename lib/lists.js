// Named tick-lists, and how a route says it belongs to one.
//
// The Challenges screen asks inList("fifty"), inList("bulgers") and so on. The routes table
// answers with `lists`, and the two were never speaking the same language: the five Fifty
// Classics routes already tagged in the catalog carry the slug "fifty_classics", and the
// Bulger peaks carry whole sentences — "Washington Bulger List (100 highest peaks in
// Washington)", "Washington Bulgers (100 Highest Peaks) — tied for #51", and nine more
// spellings of the same list. So every one of those challenges rendered 0 of N while the data
// to fill them was sitting in the column. Nothing failed; the list was simply empty.
//
// LIST_ALIASES is the translation layer. A route matches a list if any entry in its `lists`
// array equals one of the list's slugs, or matches one of its prose patterns.

export const LIST_ALIASES = {
  fifty: {
    slugs: ["fifty", "fifty_classics", "50_classics", "fifty-classics"],
    // "Fifty Classic Climbs of North America", "50 Classics", "Steck & Roper"…
    re: /\b(?:fifty|50)\s+classic|steck\s*(?:&|and)?\s*roper|roper\s*(?:&|and)\s*steck/i,
  },
  bulgers: {
    slugs: ["bulgers", "bulger", "bulger_list"],
    re: /\bbulger|washington(?:'s)?\s+(?:top\s+)?100\s+(?:highest|peaks)|100\s+highest\s+peaks\s+in\s+washington/i,
  },
  ...
  state_hp: { slugs: ["state_hp", "state_highpoints"], re: /\bstate\s+high\s*point/i },
  np_hp: { slugs: ["np_hp"], re: /\bnational\s+park\s+high\s*point/i },
  cascade: { slugs: ["cascade", "cascade_volcanoes"], re: /\bcascade\s+volcano/i },
  adk46: { slugs: ["adk46"], re: /\badirondack\s+46|46er/i },
  ne4k: { slugs: ["ne4k"], re: /\bnortheast\b[^.]{0,20}4,?000/i },
  co_cent: { slugs: ["co_cent"], re: /\bcolorado(?:'s)?\s+100\s+highest/i },
  id12: { slugs: ["id12"], re: /\bidaho\b[^.]{0,20}12,?000/i },
  ...
  wasatch11: { slugs: ["wasatch11"], re: /\bwasatch\b[^.]{0,20}11,?000/i },
  triple: { slugs: ["triple", "triple_crown"], re: /\btriple\s+crown\b/i },
};

// Does this route belong to the named list?
export function routeInList(route, key) {
  ...
  const slugs = spec.slugs.map(s => s.toLowerCase());
...
export const FIFTY_CLASSICS = [
  ...
  { n: 17, route: "East Ridge", peak: "Devils Thumb", region: "British Columbia" },
