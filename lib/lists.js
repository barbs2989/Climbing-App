...
// peaks carry whole sentences in eleven wordings. Every one of those challenges rendered
// 0 of N with the data sitting in the column.
//
// The prose-to-slug decision itself belongs to lib/routeTags.js (#783), which normalises the
// column, keeps the rank out of the prose ("tied for #51") as `detail`, and is what
// check:route-tags asserts against. This module does NOT re-implement it — it maps the
// Challenges screen's key names onto those slugs, and carries its own patterns only for the
// lists routeTags has no slug for yet.

import { hasTag, listSlug } from "./routeTags.js";

// Challenge key -> routeTags slug, where routeTags knows the list.
const KEY_TO_SLUG = {
  fifty: "fifty_classics",
  bulgers: "bulger",
  state_hp: "state_hp",
  cascade: "volcano",
};

// Lists the Challenges screen declares that routeTags has no slug for. When one is added
// there, delete the entry here rather than keeping two vocabularies.
...
export function routeInList(route, key) {
  ...
  if (slug) return hasTag(route, slug);
  ...
  return entries.some(e => {
    ...
    // Never claim an entry that routeTags has already resolved to one of its own slugs.
    if (listSlug(e)) return false;
