...
export function gradeSystemForDiscipline(disc) {
  ...
  if (disc === "trad" || disc === "sport" || disc === "alpine" || disc === "toprope" || disc === "rock") return "yds"; // rock: the seed catalog's internal bucket
...
/* HIGHEST WINS. Scan EVERY match the branch's pattern makes and keep the largest, rather than
   returning the first.

   THIS REMOVES A DIALECT RATHER THAN ADDING ONE, which is the only reason it is allowed to
   happen at all — the standing objection recorded below is that "a fifth dialect is the problem,
   not the fix". The app already answered "how hard is a range grade" TWICE, in opposite
   directions: the grade FILTER (`routeBandIdx` in ClimbMatchCore.jsx) parses the same string and
   takes the MAXIMUM, deliberately — "Difficulty=crux, so a range route surfaces when you filter
   for its top grade" — while this parser took the FIRST match, so `"Class 3-4"` filtered as 4 and
   sorted as 3. One fact, two derivations, disagreeing on exactly the range rows. This makes the
   sortable column agree with the filter the app already ships.

   THE RANGE CAPTURE IS NOT OPTIONAL, and "just take the max" is the trap. `"5.4-5.6"` is already
   TWO separate 5.x matches, so YDS needs nothing extra — but `"Class 3-4"` is ONE match of
   /class\s*(\d)/ and a bare max over that pattern still scores 3. The systems that write a range
   as a single token (class, V, WI/AI, M, aid) capture BOTH ends.

   THE V BRANCH IS CASE-INSENSITIVE SINCE 2026-09-23, and that was a SEPARATE change with its own
   before/after — the note here used to say case sensitivity was "preserved deliberately" and that
   the lowercase rows were "reported separately", which stopped being true the day they were fixed.
   `v11`/`v6` scored null, so those routes sorted behind the WHOLE catalog and were dropped
   outright by any range filter. Measured over all 205,382 graded routes before it shipped:
   12 RESCUED, 0 CHANGED, 0 LOST — strictly additive, which is the only shape a widening here may
   have. See scripts/oneoff/{measure-unreadable-grades,verify-v-case-widening}.mjs. */
function maxOf(g, rx, val) {
  let m, best = null;
  rx.lastIndex = 0; // module-level /g literals carry lastIndex between calls.
...
/* EACH PATTERN IS THE ORIGINAL BRANCH PATTERN with a range tail appended and /g added — the same
   spacing, the same digit count, the same case flag. That exactness is the point: it makes the
   before/after provably "max instead of first" rather than a quiet widening, and `0 GAINED` in
   verify-highest-grade-equivalence a guarantee by construction rather than an accident of what
   the catalog happens to hold today.
   A first version injected `\s*` after every prefix, which reads as harmless and is not: it makes
   `"WI 2-3"` newly parseable, and `V\s*(\d+)` then reads "Grade V 5.9" as a V5 boulder problem —
   the roman-commitment-vs-technical conflation this column exists to avoid.
   DO NOT ADD THE SPACE. Measured 2026-09-23 across all five grade columns of all 205,543 routes:
   `"WI 2-3"` occurs EXACTLY ONCE catalog-wide and it is in `ice_grade`, not `grade`, on a route
   whose `grade` is "4th" and whose `grade_num` is a correct 4 — so this parser never sees it and
   the row is not mis-sorted. The widening rescues 0 rows, changes 0 and loses 0: a class of ZERO.
   That value already reaches a screen as a labelled ice grade, which needs no parsing at all.
   `\s*[-–—]\s*` — this catalog writes a range with a hyphen, an en dash and an em dash. */
const RX_YDS = /5\.(\d+)([a-d]?)/g;
const RX_V = /V(\d+)(?:\s*[-–—]\s*(\d+))?/gi;
const RX_WI = /WI(\d+)(?:\s*[-–—]\s*(\d+))?/gi;
const RX_M = /M(\d+)(?:\s*[-–—]\s*(\d+))?/g;
const RX_AID = /[AC](\d)(?:\s*[-–—]\s*(\d))?/g;
const RX_WIAI = /\b(?:WI|AI)(\d+)(?:\s*[-–—]\s*(\d+))?/gi;
const RX_VB = /\bV(\d+)(?:\s*[-–—]\s*(\d+))?/gi;
const RX_CLS = /class\s*(\d)(?:\s*[-–—]\s*(\d))?/gi;
const RX_CLS_ORD = /(\d)\s*(?:rd|th|nd)?\s*class/gi;
const RX_ORD = /\b(\d)(?:st|nd|rd|th)\b/gi;

/* V-BEGINNER, AND THE FIRST VALUE THIS COLUMN HAS EVER HELD BELOW ZERO.
   "VB"/"Vb" is a real Hueco grade sitting BELOW V0, and every branch above needs a digit, so
   those rows scored null — which means they sort behind the WHOLE catalog under `grade_asc`
   and `grade_desc` alike (both are `nulls last`), the same defect the lowercase-v fix removed.

   WHY -1 AND NOT 0. `V0` already maps to 0, so 0 is taken, and using it would say VB and V0 are
   the same grade. -1 is what the scale actually claims: one step below V0. Measured 2026-09-23
   by scripts/oneoff/measure-vb-grade-rows.mjs before choosing — the column holds min 0, max 17
   and ZERO negatives today, so this is a new class of value, and every consumer was read rather
   than assumed: the finder RPCs (0018/0019) declare `min_grade`/`max_grade` as `numeric default
   null` and sort `nulls last`, so a negative sorts correctly and is excluded only by a floor it
   is genuinely below; `lib/offline.js` goes through `numOrNull` + `cmpNullsLast`. NOTE that
   `techHrs(pitches,len,gradeNum)` in ClimbMatchCore.jsx is NOT a consumer of this column — its
   callers pass `gn(route.grade)`, a different parser over the grade STRING — so no time
   estimate is in frame here. Nothing does arithmetic on the stored number.

   SCOPED TO THE V SYSTEM, and that costs nothing: all 9 occurrences of a `VB` token in `grade`
   are `bouldering`, and there are 0 in rock/ice/alpine/aid_grade or `commitment`. So the safer
   scoping is also the complete one, and a system-agnostic fallback would buy no row while
   putting "VB" in reach of a grade string that meant something else.

   NO /g FLAG, deliberately: this is used with `.test()` rather than `maxOf`, and a /g regex
   carries `lastIndex` between calls — the trap `maxOf` resets for every other pattern here. */
/* "V-easy" (Mountain Project's name for the same beginner band) is VB too. It has no digit, so it
   used to fall through to the roman-numeral last resort, which read its leading "V" as commitment
   grade V and scored it 5 — 2,250 bouldering rows sorted between V4 and V6. */
const RX_VBEG = /\bV(?:B\b|-?\s*easy\b)/i;
const VBEG = -1;

export function gradeNumFrom(g, s) {
  if (!g) return null;
  let m, v;
  if (s === "yds" && (v = maxOf(g, RX_YDS, YDS_VAL)) != null) return v;
  if (s === "v" && (v = maxOf(g, RX_V, ENDS)) != null) return v;
  // AFTER the V branch, never before: "V3, VB start" is a V3, and highest-wins already says so.
  if (s === "v" && RX_VBEG.test(g)) return VBEG;
  // "V?" is an unknown V grade, not commitment grade V (7 rows scored 5 by the last resort below).
  if (s === "v" && /^\s*V\?\s*$/i.test(g)) return null;
  if (s === "wi" && (v = maxOf(g, RX_WI, ENDS)) != null) return v;
  if (s === "m" && (v = maxOf(g, RX_M, ENDS)) != null) return v;
  if (s === "aid" && (v = maxOf(g, RX_AID, ENDS)) != null) return v;
  if ((v = maxOf(g, RX_YDS, YDS_VAL)) != null) return v;
  if ((v = maxOf(g, RX_WIAI, ENDS)) != null) return v;
  if ((v = maxOf(g, RX_VB, ENDS)) != null) return v;
  if ((m = g.match(/\b(TD|PD|AD|ED)\b/)) || (m = g.match(/^(D|F)[+-]?$/))) return AGRADE[m[1]];
  {
    const a = maxOf(g, RX_CLS, ENDS);
    const b = maxOf(g, RX_CLS_ORD, (mm) => [parseInt(mm[1])]);
  ...
  if ((v = maxOf(g, RX_ORD, (mm) => [parseInt(mm[1])])) != null) return v;
