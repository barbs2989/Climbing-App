...
/* HIGHEST WINS. Scan EVERY match the branch's pattern makes and keep the largest, rather than
   ...
   CASE SENSITIVITY IS PRESERVED PER BRANCH, deliberately. Making the V branch case-insensitive
   would also fix ~12 catalog rows graded lowercase `v11`/`v6`, which currently score null and
   therefore sort behind the whole catalog — a real defect, and NOT this change. Widening two
   things at once makes a before/after unreadable; that one is reported separately. */
...
/* EACH PATTERN IS THE ORIGINAL BRANCH PATTERN with a range tail appended and /g added — the same
   ...
   `"WI 2-3"` newly parseable. That is arguably an improvement and it is NOT this change; it is
   reported alongside the ~12 rows graded lowercase `v11`, which the case-sensitive V branch still
   scores null. Widening two things at once makes a before/after unreadable.
   `\s*[-–—]\s*` — this catalog writes a range with a hyphen, an en dash and an em dash. */
const RX_YDS = /5\.(\d+)([a-d]?)/g;
const RX_V = /V(\d+)(?:\s*[-–—]\s*(\d+))?/g;
const RX_WI = /WI(\d+)(?:\s*[-–—]\s*(\d+))?/gi;
const RX_M = /M(\d+)(?:\s*[-–—]\s*(\d+))?/g;
const RX_AID = /[AC](\d)(?:\s*[-–—]\s*(\d))?/g;
const RX_WIAI = /\b(?:WI|AI)(\d+)(?:\s*[-–—]\s*(\d+))?/gi;
const RX_VB = /\bV(\d+)(?:\s*[-–—]\s*(\d+))?/g;
