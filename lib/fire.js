...
export async function fetchFireWeather(bbox) {
  const r = await arcgis(FIRE_WX, {
    ...
    // Soonest-expiring first. This layer was the one capped query still going out
    // unordered — found by writing check:fire, not by reading the code. Size is
    // meaningless for a weather zone, but "which of these ends first" is exactly what
    // you want to keep when a viewport holds more products than the cap.
    orderByFields: "ends ASC",
  }, CAP.wx);
