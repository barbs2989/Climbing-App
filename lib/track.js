...
// EXPORTED PER-POINT, because a GPX VERTEX can be manufactured the same way a pin can and the
// repair path needs to ask. Re-implementing the decimal test beside it is how this repo ended up
// with four grade parsers; one rule, two callers.
export function coordinateIsComputed(p) {
  const q = pointOf(p);
  if (!q) return false;
  const decimals = v => { const s = String(v); const i = s.indexOf("."); return i < 0 ? 0 : s.length - i - 1; };
  return Math.max(decimals(q.lat), decimals(q.lng)) > SURVEYED_DECIMALS;
}

export function someWaypointsAreComputed(waypoints) {
  return (Array.isArray(waypoints) ? waypoints : []).some(w => coordinateIsComputed(w));
}
...
export function trackCoverage(gpxPts, waypoints) {
  ...
  // ASKED BEFORE THE PARTIAL TEST, NOT AFTER — and it used to be after, which made it unreachable
  // in one of the two cases it exists for. "This line passes none of your waypoints" and "this line
  // is short at one end" are INDEPENDENT questions: a line can cover both ends of the route and
  // still pass none of the places the pins name. Sitting below the `!missingApproach &&
  // !missingSummit` early return, the check could only ever speak about a line that was ALSO
  // partial, so those routes returned null and said nothing. Same ordering defect CLAUDE.md records
  // for DbGuideApply: a guard placed after the branch that returns is unreachable in the case it
  // exists for.
  //
...
// A LINE THAT GOES NOWHERE IS A PLACEHOLDER, AND THE PAGE OFFERED IT AS THE ROUTE'S TRACK.
//
// The two caveats above cover a line drawn between waypoints and a line that is short at one end.
// Neither reaches a line that is short at BOTH ends because it barely exists: five routes store a
// "track" spanning 7 m to 451 m end to end — `wa_sky_mountain_s_route` is NINE points across SEVEN
// METRES — and the page draws each of them under a ROUTE TRACK heading with a Download GPX button
// beneath it, saying nothing. A climber who taps that gets a seven-metre file.
//
// Every other consumer of this data already knows: `trackCoverage` returns null below 500 m because
// measuring pins against a dot manufactures huge distances out of nothing, and the waypoint audits
// carry the same gate. The READER was the only place that did not, which is the split this file
// keeps recording — the data layer refuses to reason about a stub while the screen presents it.
//
// 500 m is that same existing gate, not a new number, and it lands in a real void: the silent
// stubs span 7, 18, 55, 69 and 451 m, and the next line up is well clear of it.
//
// EXCLUSIVE with the other two by construction rather than by the caller remembering. Ten of the
// fifteen sub-500 m lines are already captioned as waypoint joins, and stacking a second sentence
// on one section reads as two separate problems when it is one problem described twice — the rule
// `waypointCaveat` already follows.
const TRACK_STUB_M = 500;

export function trackStubCaveat(gpxPts, waypoints, fmt) {
  const line = (Array.isArray(gpxPts) ? gpxPts : []).map(pointOf).filter(Boolean);
  ...
  let mnLa = 90, mxLa = -90, mnLn = 180, mxLn = -180;
  for (const v of line) { mnLa = Math.min(mnLa, v.lat); mxLa = Math.max(mxLa, v.lat); mnLn = Math.min(mnLn, v.lng); mxLn = Math.max(mxLn, v.lng); }
  const ext = metresBetween({ lat: mnLa, lng: mnLn }, { lat: mxLa, lng: mxLn });
  // The distance is what makes this checkable rather than a bare assertion, and it needs a SHORT
  // formatter: the caller's usual one rounds to 0.1 km, which renders seven metres as "0 km" and
  // reads as a bug rather than as the point.
  const d = typeof fmt === "function" ? fmt(ext) : `${Math.round(ext)} m`;
  if (ext < TRACK_STUB_M)
    return `This line spans ${d} end to end — a placeholder, not this route's track. Do not navigate by it, and do not read the download below as the route.`;

  // TOO FEW POINTS TO BE A RECORDING OF ANYTHING, WHICH IS A CATEGORICAL CLAIM RATHER THAN A
  // THRESHOLD. Fifteen routes store a "track" of exactly TWO points — `wa_mount_rainier_edmunds_
  // headwall` is a single straight segment across 16.4 km of Mount Rainier — drawn under ROUTE
  // TRACK with Download GPX beneath it and no caption. N points make N-1 straight segments; at two
  // that is one chord, and there is no reading under which a chord is a recording of a walk.
  //
  // FOUR IS THIS FILE'S OWN FLOOR, not a new number: `trackCoverage` already returns null below it,
  // for the same reason — there is not enough line there to say anything about. And it is where the
  // data has a void: among the lines the app currently says nothing about, the vertex counts run
  // 2, 3, 4, 6, 7, 8, 10, 11 and then jump to 20, so nothing sits near the cut either way.
  //
  // DELIBERATELY NOT WIDER. A line of eight points across 18 km is equally implausible as a
  // recording, and saying so needs an argument about how coarse a SIMPLIFIED track may be —
  // measured, the spacing distribution is continuous, so that argument has no evidence behind it
  // (see measure-drawn-lines-posing-as-tracks.mjs). Two and three points need no argument at all.
  if (line.length < 4)
    return `This line is ${line.length === 2 ? "a single straight segment" : "two straight segments"} drawn across ${d} — not a recorded track of the route. Do not navigate by it, and do not read the download below as the route.`;
  return null;
<<<<<<< HEAD
}

// WHAT THE DOWNLOADED FILE SHOULD SAY, because every caveat above lives on a WEB PAGE and the file
// is the thing a climber carries where they cannot re-read it.
//
// The route page tells you a line is straight segments between waypoints, or a placeholder, or
// short at one end. Then it offers Download GPX, and `gpxDownload` wrote `<wpt>` entries and a
// `<trk><name>` with **no `<desc>` anywhere** — so the file loads into Gaia or CalTopo as a track
// named after the route, with nothing attached. The disclaimer stops at the browser, and the one
// place it matters is in the field.
//
// GPX 1.1 has `<desc>` on both `<metadata>` and `<trk>` for exactly this, and every mapping app
// shows it. Nothing here invents a claim: it is the same sentence the screen already renders.
//
// AT MOST ONE FIRES TODAY — the three are mutually exclusive by construction — but this returns an
// ARRAY rather than a string, because that exclusivity is a property of the current predicates and
// not something the file format cares about.
export function exportedTrackNotes(route, fmt) {
  const r = route || {};
  const notes = [];
  ...
  const cov = trackCoverage(r.gpxPts, r.waypoints);
  if (cov) notes.push(trackCoverageCaveat(cov, fmt));
  return notes;
=======
>>>>>>> origin/main
}
