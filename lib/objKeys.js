...
/* `weather` IS OFFERED AS ITS TWO SUB-KEYS, NOT AS THE BARE KEY, and the bare key was a
   live defect: it is an OBJECT on 498 of the 504 routes carrying this column, so the
   form's current-value line read "Weather: [object Object]" on 100% of them — proven
   with the app's own objStr in scripts/oneoff/prove-objstr-emits-object-object.mjs.
   A form that opens on [object Object] invites a climber to replace a real value with
   whatever they can see, which is worse than not offering the field at all.
   `typical` and `probability` are strings on 498 and 471 rows, so both are editable as
   text, and splitting them means one field per fact rather than a blob. */
export const SEASHAZ_KEYS=[["exposure","Exposure","e.g. the ridge is fully exposed above the col"],["weather.typical","Typical weather","e.g. afternoon convection builds over the ridge"],["weather.probability","Chance of it","e.g. thunderstorms most afternoons in July"],["crevasses","Crevasses","e.g. bridges thin from late July"],["avalanche.zone","Avalanche forecast zone","e.g. NWAC East Slopes Central"]];
