...
export const SEASHAZ_KEYS=[["exposure","Exposure","e.g. the ridge is fully exposed above the col"],["weather","Weather","e.g. afternoon convection builds fast in July"],["crevasses","Crevasses","e.g. bridges thin from late July"]];
/* `avalanche.zone` is DELIBERATELY NOT OFFERED, and check:contrib-fields is what stopped it going
   in. It is stored on 497 routes and read by NOTHING — the panel renders `avy.byMonth` and never
   the zone — so offering it would have been contributable-and-invisible, the very defect this
   sweep removes. The zone itself reaching no screen is a separate finding: on a safety panel it
   is the one thing that tells a party WHICH avalanche forecast to read. Adding that renderer is
   feature work, so it is reported rather than slipped in here. */
