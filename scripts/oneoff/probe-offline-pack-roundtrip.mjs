 ...
 *
 * NO NEW DEPENDENCY. There is no fake-indexeddb in node_modules and adding one for a single probe
 * is not worth it, so the shim below implements exactly the surface lib/offline.js uses and nothing
 * more — it cannot drift into a second implementation with its own opinions. It is deliberately
 * STRICT where the real API is strict: createObjectStore on an existing name throws, which is the
 * entire point of section 1.
 *
