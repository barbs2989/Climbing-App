...
const rich = R.filter(r => r.overview || r.approach || r.timing || r.itinerary || r.pitch_detail || r.waypoints || hints[r.id]);
const byArea = {};
...
batches.forEach((b, i) => fs.writeFileSync(`${T}/batches/b${String(i + 1).padStart(3, "0")}.json`, JSON.stringify(b, null, 1)));
