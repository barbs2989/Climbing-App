// Collect reader outputs (read/g*.json) -> research files (research/in/rNNN.json), grouped by area,
...
const reads = fs.readdirSync(`${T}/read`).filter(f => /^g\d+\.json$/.test(f) && (!only || only.test(f)));
const seen = new Set(), items = [];
let bad = 0;
for (const f of reads) {
  let j; try { j = JSON.parse(fs.readFileSync(`${T}/read/${f}`)); } catch (e) { console.log("UNPARSEABLE", f); bad++; continue; }
  for (const r of j.routes || []) {
    seen.add(r.id);
    const cs = (r.contradictions || []).filter(c => c.needs_research !== false || c.internal_resolution);
    if (cs.length && R[r.id]) items.push({ id: r.id, area: R[r.id].area_id, contradictions: cs });
  }
}
const exp = new Set();
for (const b of fs.readdirSync(`${T}/batches`)) {
  const g = Math.floor((+b.slice(1, 4) - 1) / 4) + 1;
  if (only && !only.test(`g${String(g).padStart(3, "0")}.json`)) continue;
...
files.forEach((f, i) => fs.writeFileSync(`${T}/research/in/r${String(start + i).padStart(3, "0")}.json`, JSON.stringify(f, null, 1)));
...
console.log(`routes with contradictions ${items.length}; contradictions ${total}`, sev, `-> research files r${String(start).padStart(3, "0")}..r${String(start + files.length - 1).padStart(3, "0")}`);
