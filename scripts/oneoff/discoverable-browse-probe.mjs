...
try {
  ...
  // 0192 makes listing opt-OUT (the user's decision; 0110 had made it opt-in), so a freshly
  ...
  if (freshDefault === true) {
    ...
    rec("a brand-new account is listed until it opts out", false, `a new account was created with discoverable=${JSON.stringify(freshDefault)} — migration 0192_discoverable_defaults_on.sql has not been applied, so no new climber appears in partner browse`);
  }
