...
//
// The guard's healthy output is "6 real payloads split correctly", which is exactly what a guard
// that has stopped looking prints. Each case edits RouteDetail.jsx in place, proves the edit
// LANDED by checksum, runs the guard, and restores the file byte-identically.
//
// Cases 1-3 are the original three re-run against the merged section — a guard that still catches
// everything it used to is half the claim. Case 4 is the property the merge ADDED. Cases 5 and 6
// are #1440's shortfall rule in both directions, carried over because a rule that only ever
...
const CASES = [
  ...
  {
    // It has to BYPASS pitchShortfall, not call it with a different `shown`: the `> described`
    // half of that predicate is inside the function, so handing it the roped count still refuses.
    // A first version did exactly that, landed by checksum and reproduced no defect at all.
    name: "5. #1440 reverted — shortfall measured against the ROPED count again",
    find: "const short=pitchShortfall(route,rows.length)?",
    repl: "const short=(route.pitches&&pitchCount&&route.pitches>pitchCount)?",
    expect: "fail",
  },
  {
    name: "6. #1440 satisfied by deletion — never claim a shortfall at all",
    find: "const short=pitchShortfall(route,rows.length)?",
    repl: "const short=false?",
    expect: "fail",
  },
];
