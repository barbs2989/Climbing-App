...
const CASES = [
  ...
  {
    name: "revert-stop",
    why: "the real historical STOP list — positional words stripped, so upper/lower become one key",
    find: "const STOP = /\\b(the|mount|mt|peak|spire|summit|standard|route)\\b/g;",
    repl: "const STOP = /\\b(the|mount|mt|peak|spire|summit|true|main|west|east|north|south|upper|lower|standard|route)\\b/g;",
    expect: "UPPER and LOWER of one feature are two places",
  },
  {
    name: "strip-lower-only",
    why: "half the revert — one positional word back is enough to collapse the pair",
    find: "const STOP = /\\b(the|mount|mt|peak|spire|summit|standard|route)\\b/g;",
    repl: "const STOP = /\\b(the|mount|mt|peak|spire|summit|standard|route|upper|lower)\\b/g;",
    expect: "UPPER and LOWER of one feature are two places",
  },
  {
    name: "no-name-merge",
    why: "gutting STOP entirely, so two spellings of ONE junction stop merging — the OVER-correction",
    find: "const STOP = /\\b(the|mount|mt|peak|spire|summit|standard|route)\\b/g;",
    repl: "const STOP = /\\b(?!)\\b/g;",
    expect: "two spellings of ONE junction still merge",
  },
  {
    name: "stop-reordered",
    why: "MUST STAY SILENT — the same STOP words in a different order is not a change",
    find: "const STOP = /\\b(the|mount|mt|peak|spire|summit|standard|route)\\b/g;",
    repl: "const STOP = /\\b(route|standard|summit|spire|peak|mt|mount|the)\\b/g;",
    expect: null,
  },
  {
