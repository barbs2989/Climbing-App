...
// The tour moved out of core with Help (lazy since the startup split); check:boot reads it there.
const HELP = path.join(ROOT, "lib", "Help.jsx");
...
const CASES = [
  { name: "1 my-crew", edits: [[HELP, '["","Crew","', '["","My Crew","']], expect: "fail", why: "the real defect: the tour called the Crew tab 'My Crew'" },
  { name: "2 no-home", edits: [[HELP, '["","Home","', '["","Home2","']], expect: "fail", why: "the real defect: Home, the landing tab, had no entry" },
  ...
  { name: "4 anchor", edits: [[HELP, "const feats=[", "const featsX=["]], expect: "fail", why: "a renamed tour must fail closed, never pass" },
  { name: "5 extra-section", edits: [[HELP, '["","Groups","', '["","Weather","Not a tab.",["a."]],["","Groups","']], expect: "pass", why: "MUST STAY SILENT — a non-tab section is legitimate" },
];
