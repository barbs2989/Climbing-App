...
for (const c of CASES) {
  ...
  if (!t.includes("ROUTE BREAKDOWN")) {
    // Distinguishing these is the whole point: one is a fixture too thin to render the
    // section at all, the other is the section rendering and getting the entries wrong.
    fail(`${c.name}: ROUTE BREAKDOWN never rendered — nothing below was actually checked`);
    continue;
  }
  // One row per pitch_detail entry, in the record's own order, each tagged with the kind it
  // was drawn as. Read from the markup rather than the text because with a single heading the
  // classification is no longer a word on the page — it is the badge shape, the accent and
  // the detail block, none of which survive tag-stripping.
...
// ── Injection-tested. Re-run these after any change to pitchEntryKind, breakdownRows or
// RouteBreakdown; a guard nobody has seen fail is not a guard. Cases 1-3 are the original
// three, re-run against the merged section; case 4 is the property the merge added.
//
//  1. Restore the original per-route verdict — insert `return isPitched(route)?"pitch":"stage";`
//     as the first line of pitchEntryKind.
//  2. Restore the glued badge — render {"P"+r._n} instead of {"P"+r._badge} in RouteBreakdown.
//  3. Drop one kind instead of rendering it — filter the stages out of breakdownRows' result.
//  4. Sort the merged list unconditionally — remove the `if(!nS)` guard around the numeric
//     sort, which is what re-separates a mixed route's two label spaces and puts the pitches
//     back in a block. Only the ORDER assertion can see this one.
