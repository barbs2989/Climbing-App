...
(function preflight() {
  ...
  const DISC = new Set(["alpine","mountaineering","scrambling","scramble","hike","glacier","snow","ice","mixed","rock","trad","sport","aid","bouldering","tr"]);
  const issues = [];
  A.forEach(a => {
...
function gradeSystem(d) { if (d === "bouldering") return "v"; if (["scrambling", "mountaineering", "hiking"].includes(d)) return "class"; if (["trad", "sport", "alpine", "toprope"].includes(d)) return "yds"; if (d === "ice") return "wi"; if (d === "mixed") return "m"; if (d === "aid") return "aid"; return null; }
...
const aRow = a => ({ id: a.id, name: a.name, parent_id: a.parentId, area_type: a.areaType, region: a.region, lat: a.lat, lng: a.lng, elevation_ft: ri(a.elevationFt), prominence_ft: ri(a.prominenceFt), parent_peak: a.parentPeak ?? null });
const rRow = r => { let disc = r.discipline === "rock" ? (r.style ? r.style.toLowerCase() : "alpine") : r.discipline; /* a styleless rock line on a peak is alpine (0197/0202) */ if (disc === "tr") disc = "toprope"; else if (disc === "scramble" || disc === "hike") disc = "scrambling"; else if (disc === "glacier" || disc === "snow") disc = "mountaineering"; const sys = r.gradeSystem || gradeSystem(disc); return { id: r.id, area_id: r.mountainId, name: r.name, discipline: disc, grade: r.grade, grade_system: sys, grade_num: gradeNum(r.grade, sys), pitches: ri(r.pitches), sort_order: r.sortOrder ?? null, length_m: r.routeFt != null ? Math.round(r.routeFt / 3.28084) : null, aspect: r.aspect ?? null, season: r.season ?? null, fa: r.fa ?? null, gain_ft: r.gainFt != null ? ri(r.gainFt) : (r.gainM!=null?Math.round(r.gainM*3.28084):null), loss_ft: r.lossFt != null ? ri(r.lossFt) : (r.lossM!=null?Math.round(r.lossM*3.28084):null), dist_km: r.distKm ?? null, max_angle: ri(r.maxAngle), rappels: r.rappels != null ? String(r.rappels) : null, commitment: r.commitment ?? null, face: r.face ?? null, permit: r.permit ?? null, comms: r.comms ?? null, descent: r.descent ?? null, obj_haz: r.objHaz ?? null, waypoints: r.waypoints ?? null, gpx: r.gpxPts ?? null, elev_pts: r.elevPts ?? null, overview: r.overview ?? null, beta: r.beta ?? null, turnaround: r.turnaround ?? null, auto_generated: r.autoGenerated ?? false, alpine_grade: r.alpineGrade ?? null, rock_grade: r.rockGrade ?? null, ice_grade: r.iceGrade ?? null, disciplines: Array.isArray(r.disciplines) ? [...new Set(r.disciplines.map(function(d){return (d==="scramble"||d==="hike")?"scrambling":((d==="glacier"||d==="snow")?"mountaineering":d);}))] : (r.disciplines ?? null), high_point_ft: ri(r.highPointFt), aid_grade: r.aidGrade ?? null, gear: r.gear ?? null, hazards: r.hazards ?? r.knownHazards ?? null, approach: r.approach ?? null, descent_text: r.descentText ?? null, bail: r.bail ?? null, pitch_detail: r.pitchDetail ?? null, itinerary: r.itinerary ?? null, access: r.access ?? null, road: r.road ?? null, climate: r.climate ?? null, emergency: r.emergency ?? null, lists: r.lists ?? null, timing: r.timing ?? null, detailed_rack: r.detailedRack ?? null, what_to_bring: r.whatToBring ?? null, pro_tips: r.proTips ?? null, watch_out: r.watchOut ?? null, pro_needs: r.proNeeds ?? null, best_season: r.bestSeason ?? null }; };
